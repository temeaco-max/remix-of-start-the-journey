...
export default function QrScannerScreen() {
  ...
  const handleBarcode = useCallback(async ({ data, type }: { data: string; type: string }) => {
    ...
    if (!context) { haptic.error(); setError("That QR code is not a valid Kurukoo context link."); return; }
    setHandled(true); setError(undefined); haptic.success(); await Linking.openURL(context.url); router.back();
  }, [handled, router]);
  return <ScreenContainer className="px-4 pt-3" edges={["top", "left", "right"]}><View style={styles.body}>
    ...
    {!permission?.granted ? <View style={[styles.permissionCard, { borderColor: colors.border, backgroundColor: colors.surface }]}>{permission?.canAskAgain !== false ? <><Text style={[styles.title, { color: colors.foreground }]}>Camera access is required</Text><Text style={[styles.detail, { color: colors.muted }]}>Kurukoo uses the camera only to read QR context links. Your camera feed is not uploaded.</Text><Pressable accessibilityRole="button" onPress={() => { haptic.light(); void requestPermission(); }} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><Text style={styles.primaryText}>Allow camera access</Text></Pressable></> : <><Text style={[styles.title, { color: colors.foreground }]}>Camera access is blocked</Text><Text style={[styles.detail, { color: colors.muted }]}>Enable camera access in system settings, then return to Kurukoo.</Text><Pressable accessibilityRole="button" onPress={() => { haptic.light(); void Linking.openSettings(); }} style={[styles.primaryButton, { backgroundColor: colors.primary }]}><Text style={styles.primaryText}>Open settings</Text></Pressable></>}</View> : <View style={styles.cameraWrap}><CameraView style={styles.camera} facing="back" onBarcodeScanned={handled ? undefined : handleBarcode} barcodeScannerSettings={{ barcodeTypes: ["qr"] }} /><View pointerEvents="none" style={styles.overlay}><View style={[styles.scanFrame, { borderColor: colors.primary }]} /><Text style={styles.overlayText}>Align the QR inside the frame</Text></View>{handled ? <View style={styles.processing}><ActivityIndicator color="#fff" /><Text style={styles.processingText}>Opening context…</Text></View> : null}</View>}
