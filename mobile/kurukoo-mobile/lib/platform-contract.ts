...
export const MOBILE_PLATFORM_CONTRACTS: readonly MobileCapabilityContract[] = [
  ...
  },
  {
    id: "commerce-network",
    area: "commerce",
    title: "Points, POS agents, provider lead charges and agent commissions",
    status: "client_ready",
    canonicalOwner: "src/services/pointsEngine.ts + src/services/agentNetworkCommerce.ts + src/services/commercialLedger.ts",
    nativeRoute: "/surface/commerce",
    api: ["/api/agent-network/*", "/api/commercial/*", "canonical Points routes"],
    notes: "Top-up settlement requires verified payment evidence; provider lead charging stays tied to Economic Request matching.",
  },
  {
    id: "catalogue",
    area: "catalogue",
    title: "Provider / business / WhatsApp / store / affiliate product inventory",
    status: "client_ready",
    canonicalOwner: "src/services/catalogueSourceRegistry.ts + src/services/catalogueInventoryMatcher.ts",
    nativeRoute: "/surface/catalogue",
    api: ["/api/catalogue/sources", "/api/catalogue/products", "/api/catalogue/products/search", "/api/discover/home", "/api/discover/category/:category"],
    notes: "All product sources converge into one catalogue authority; user declarations remain unverified until source evidence exists.",
  },
  {
    id: "provider-communications",
    area: "communications",
    title: "Masked provider calling and real-time provider session/tracking",
    status: "external_required",
    canonicalOwner: "src/services/providerCommunicationService.ts + privacyBridge.ts + webrtcSignalling.ts + trickbridgeTrackingAdapter.ts",
    nativeRoute: "/surface/provider-communication",
    api: ["/api/provider-communication/*", "/api/webrtc/*", "/api/voice/*"],
    notes: "Native UI can be built against canonical session/location state; live telephony and external tracking activate only through approved adapters.",
  },
] as const;
