...
const legacyRouter = read('src/services/legacyIntentRouter.ts');
const routingBoundary = `${router}\n${legacyRouter}`;
...
for (const [name, pattern] of requiredMandate) if (!pattern.test(skillFlows + routingBoundary + blueprint)) failures.push(`Missing mandate integration: ${name}`);
if (!routingBoundary.includes('classifyWithFastText')) failures.push('Action routing is not connected to FastText.');
if (!router.includes('queryUnifiedAI')) failures.push('Conversational routing is not connected to the unified AI engine.');
if (!routingBoundary.includes('getSkillFlow')) failures.push('Action routing bypasses the shared skill-flow service.');
if (!routingBoundary.includes('artist_booking') && !routingBoundary.includes('verified_artist')) failures.push('Creator/artist booking is not represented in the chat action layer.');

console.log(JSON.stringify({ seededSkillCount: uniqueSkills.length, actionIntentLiteralCount: new Set(actionIntents).size, serviceCount: serviceFiles.length, blueprintSectionCount: blueprintSections.length, mandateChecks: Object.fromEntries(requiredMandate.map(([name, pattern]) => [name, pattern.test(skillFlows + routingBoundary + blueprint)])), failures }, null, 2));
