...
if (missing.length) {
    throw new Error(`Public route module is missing: ${missing.join(', ')}`);
}

if (routes.length !== expected.length) {
    const unexpected = routes.filter((route: string) => !expected.includes(route));
    throw new Error(`Public route module has unexpected routes: ${unexpected.join(', ')}`);
}
