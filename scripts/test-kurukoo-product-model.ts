...
import { canonicalizeUrl } from '../src/services/canonicalUrlRegistry.js';

assert.equal(DESKTOP_AUTHENTICATED_HOME, '/desk');
...
assert.equal(canonicalizeUrl('/app/agent'), '/desk');
assert.equal(canonicalizeUrl('/app/discover'), '/discover');
assert.equal(canonicalizeUrl('/admin/?section=providers'), '/admin/providers');
