...
if (databaseMode === 'postgres' && !present(process.env.DATABASE_URL)) issues.push('KURUKOO_DATABASE_MODE=postgres requires DATABASE_URL.');
...
const result = { status: issues.length ? 'blocked' : warnings.length ? 'warning' : 'ready', production, databaseMode, jobMode, workers, issues, warnings };
