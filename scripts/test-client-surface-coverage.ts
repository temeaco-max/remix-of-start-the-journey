...
for (const forbidden of ['${r.status||\'unknown\'}${r.id?` · ${r.id}`:\'\'}', 'Continue my ${r.skill||r.category||\'request\'}']) if (appConvergence.includes(forbidden)) failures.push(`App-shell Requests fallback retains a raw identifier or generic continuation: ${forbidden}`);
...
for (const marker of ['waiting_on_dependency', 'Waiting for earlier work', 'dataset.objectiveState', 'A confirmed step was recorded.', 'resumeCanonicalContextOnLoad', "void sendMessage(input.value)"]) if (!primaryChat.includes(marker)) failures.push(`Chat Objective presentation or exact workspace continuation is missing ${marker}`);
