...
The deferred Request service also emits `economic_request.state_changed` after open-intention creation, state transitions, and bounded recheck scheduling. The reminder owner emits `reminder.state_changed` after creation, cancellation, due processing, and recurrence scheduling. The safety owner emits `safety.checkin.state_changed` after check-in start, completion, and expiration into `escalation_pending`; this is internal safety state and never claims that an emergency contact or external transport was reached. This keeps unavailable work resumable and observable without claiming that a provider, quote, dispatch, inventory item, or fulfilment result exists.
