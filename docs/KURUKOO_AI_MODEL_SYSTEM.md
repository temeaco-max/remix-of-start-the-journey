...
The local SmolLM2 boundary retains the configured 1.7B student target and supports a bounded constrained-host fallback to `HuggingFaceTB/SmolLM2-360M-Instruct`. Readiness telemetry distinguishes local enablement, successful local inference, fallback provenance, hosted configuration and unavailable runtime state. Successful fallback inference preserves the diagnostic that the primary checkpoint was unavailable. Student output is rejected when it leaks system policy, role, context, model, quota, prompt or internal contract language; the existing deterministic conversational fallback then supplies a safe clarification, recovery, correction, interruption or general-help response.
...
Mistral, Gemini, Groq, hosted Hugging Face inference, voice delivery, notifications, payments, channels, provider availability and external quotas remain optional readiness boundaries. No external capability is considered live without configured credentials, provider acceptance, evidence and independent runtime proof.
