...
Quick Ride remains a canonical Economic Request plus `economicDispatchCoordinator` broadcast over vehicle/capability constraints. It currently reuses the Economic Request, provider matching, participant selection, provider communication, location-consent, and review/evidence owners; it does **not** yet create a `physicalExecutionParticipant` profile or an `execution_request` for a ride. A future connector-backed ride participant must enter through the existing selected participant and execution authorization boundary rather than adding ride-specific physical-execution semantics. Taxi, bike, keke, future robot taxi, and future autonomous vehicle support remain capability declarations pending verified connector, safety, regulatory, and real-world evidence. Future external Agent-to-Agent work may nominate an authorized physical participant, but it must consume this boundary rather than become a second Kurukoo or a new agent runtime.
