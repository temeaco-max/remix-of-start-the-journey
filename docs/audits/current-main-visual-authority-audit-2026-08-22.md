...
The canonical `/call` entry redirects to the existing progressive-authentication surface in the local browser session. The unauthenticated identity shell remains visually aligned with the shared warm-card, terracotta-primary, 44px-input/action and Chat-return grammar, but the actual Kurukoo Call composition, permissions, device state, and call controls remain an authenticated and device-dependent validation boundary. No call was initiated and no identity information was supplied.

The repository retains `views/programmatic.ejs` and legacy stylesheet selectors, but current public routing has no registered Programmatic owner: the direct path returns `Cannot GET /programmatic`. This is a route/template inventory and ownership discrepancy, not a browser-rendered visual gap. It was not revived, aliased, or edited because current scope preserves route/backend ownership unless separately approved.

The live progressive-authentication entry retains correctly sized 48px name and primary controls and a 46px guest-return action. Its visible Terms and Privacy links initially measured 14px, and the canonical Back to chat continuation measured 21px. These were real interactive-target exceptions, not compact status or informational copy. The existing shared auth owner now gives the consent links and Chat return 44px geometry; rendered verification confirms Terms at 44px, Privacy Policy at 44px, and Back to chat at 44px while the 48px input/primary and 46px guest action preserve the current progressive identity flow. Consent wording, identity state, email/SMS flow, and guest behavior remain unchanged.

The live Legal & Privacy family now resolves its five in-page policy, support, and conversational continuations—Privacy Policy, Terms of Service, Cookie Policy, Help & support, and Ask Kurukoo—at 44px. The dark legal identity boundary, compact side navigation, white policy surface, and shared footer remain visually distinct from Marketing while using the same interaction grammar. No legal copy, policy state, or route behavior was changed.

The live Pricing family has no remaining visible sub-44px link, form, or action target. Its Web Chat primary action, Points, payment/escrow, provider continuations, and request-lifecycle paths retain the shared interaction grammar while the card copy and status language remain explicit that subscriptions, payment providers, escrow release, provider verification, and catalogue availability are not claimed by the public surface.

The live Help Centre family likewise has no remaining visible sub-44px link, form, action, or FAQ disclosure. Its support cards, contextual Help/Contact continuations, four native FAQ summaries, explanatory cards, and truthful channel/reminder/provider language retain the shared system while remaining a support composition rather than a duplicate Marketing or Chat screen.

The live Partner family has no remaining visible sub-44px role pathway, profile utility, evidence continuation, or onboarding/support control. Its deliberately information-dense opportunity and capability composition preserves 44px interactive targets, compact noninteractive status markers, a single terracotta Join/Invitation primary hierarchy, and explicit distinction between profile, evidence, opportunity, authorization, and earnings states.

The live Channels family has no remaining visible sub-44px connection action, disclosure, or recovery continuation. Its implemented, credentials-required, and activation-checklist states remain compact but text-plus-colour truthful, while every connection/workspace/support path and every native Activation and recovery summary uses the common 44px interaction grammar. No provider connection or activation was attempted.

The live Explore family has no remaining visible sub-44px search, capability-request, or disclosed-placement continuation. Its conversation-first search entry, capability cards, category language, and placement disclosures retain the canonical interaction grammar while preserving the distinction between a request prompt and provider, location, price, inventory, or fulfilment confirmation.

The live Contact support family retains correctly sized Chat primary actions and canonical channel-state language, but its contextual **Review channel readiness** continuation initially measured 16px. This was a genuine support-route interaction target, not compact status copy. The existing final authority now scopes only that muted support continuation to 44px; rendered verification confirms it remains transparent terracotta at 44px beside the 58px terracotta Open Web Chat primary. Contact content, channel state, and service claims remain unchanged.

The live Advertise family has no remaining visible sub-44px placement, disclosure, or business-discussion continuation. Its single conversational entry remains primary, while the Sponsored placement boundary, available placement options, asset/disclosure/destination checks, and explicit absence of self-service advertising preserve the shared system without claiming campaign activation or delivery.

The live Careers family has no remaining visible sub-44px role continuation. Its terracotta mission composition and role-card layout remain context-specific, while every Apply Now path follows the shared interaction rhythm. No application was started and the card copy does not claim a live application state beyond the existing open-role presentation.

The live Resources family has no remaining visible sub-44px guide, role-path, memory, or privacy continuation. Its editorial guidance groups remain quieter than Marketing, but all category and contextual pathways use the shared interaction grammar; the currently unavailable detailed-resource feed is rendered as an explicit content boundary rather than a fabricated guide state.

### Current-Main Kurukoo OS Family and State Authority Inventory

| Canonical screen family | Existing OS shell and navigation owner | Context-specific composition retained | Canonical state and transition authority | Evidence boundary |
|---|---|---|---|---|
| Public entry, Marketing, Help, Resources, Legal, Partners, Advertise and Careers | Shared public shell, late-loaded final authority, public-page/home owners, quiet text navigation, one terracotta conversation primary, shared footer. | Narrative, support, legal, partner, editorial and business surfaces retain distinct density and action emphasis. | Public text-plus-colour status, truthful disclosures, 44px route/form/disclosure controls, reduced motion and focus rules. | Representative routes rendered; provider, payment, campaign and hiring activation remain external. |
| Agent and Chat conversation | `kurukoo-chat.css`, final chat rules and canonical Chat contracts. | Three-region conversation rail, message thread, composer and contextual inspector rather than a generic dashboard/modal. | Sanitized streaming, attachments, stop generation, continuity, evidence cards and runtime banners remain owned by the canonical Chat/service contracts. | Guest Chat rendered; signed-in conversations and external channel delivery remain separate. |
| Desk, Requests, Tasks, Saved, Artifacts, Notifications, Points, Memory, Safety and commerce | Existing Workspace template and workspace authorities; clean URLs remain owned by app-surface routes and their services. | Durable operational rail/header, work cards, request/task detail, cart/checkout/confirmation, Points, memory, artifact and safety contexts remain purpose-specific. | Workspace task/request state, cart/order/checkout, reminders, notification and memory service owners determine empty, pending, success, error and recovery language. | Authenticated browser composition remains access-gated; payment/provider/notification outcomes remain evidence-gated. |
| Discover, Topics and Opportunities | Discover/topic/opportunity route and current discovery owners. | Source-led discovery, community/topic detail and opportunity review remain separate from Chat or Desk layouts. | Discovery data, nearby pulse, topic/community and opportunity services own availability, source, pending and unavailable semantics. | No live provider/location/availability claim is inferred without source evidence. |
| Connect, external channels, Agents and Call | Existing connection, agent-runtime, capability and voice owners. | Connection readiness, Agent detail, capability, prayer and Call surfaces retain contextual panels and evidence boundaries. | External readiness, agent runtime, device links and voice service own configured, pending, unavailable and activated states. | Call/auth/device and external activation remain unobserved. |
| Commerce and Quick Ride | Existing economic request/order/checkout and Quick Ride route/dispatch contracts; native Quick Ride contract defines vehicle, request, match, arrival, communication, completion and review state. | Checkout/confirmation and Ride remain request-lifecycle contexts, not separate visual product families. | Economic Request and dispatch services own payment, provider match, arrival, completion and review; UI must not claim them before evidence. | Payment, dispatch, provider, arrival and device proof remain external. |
| PWA/offline and standalone QR | Existing platform-state and QR owners. | Manual recovery card and contextual-entry form deliberately remain focused standalone compositions. | Service-worker/offline contracts and QR context owner define offline, retry, contextual-entry and unsupported-code states. | Installed-PWA and device camera proof remain external. |
| Native Agent, Discover, Requests, Tasks, Connect and specialist surfaces | Native root/tabs, `ScreenContainer`, `kurukoo-ui.tsx`, work-surface and specialist owners. | Five-tab mobile OS plus context-specific QR, capability, artifact, reminders, notifications and Go Live flows remain native-first. | `StatusPill`, `PlatformStateBanner`, `ContinuityBand`, `EvidenceRow`, empty state, ActionButton, offline sync and notification owners compose pending, success, error, unavailable and return-to-Chat states. | Physical safe areas, IME, camera, notifications and haptic proof remain device boundaries. |
| Admin control plane | Existing Admin convergence shell and Admin route/runtime owners. | Dense dark operational rail and role-specific data tables remain control-plane specific. | Admin route/audit/readiness owners determine provider, request, trust, notification, pricing and integration state language. | Live Admin rendering remains access-gated. |

The only new primitive-level divergence found during this inventory is the native tab-layout’s existing **Explore all Kurukoo features** overflow action, which measures 38px despite being a real `Pressable`. It is not a sixth product navigation system: it is the current overflow path from the five canonical native tabs to the existing feature compass. The correction must therefore stay in the native tab owner, retain that contextual overflow role, and establish the shared 44px interaction target without changing navigation, routes, or feature ownership.

The existing native tab owner now preserves the five primary Agent, Discover, Requests, Tasks and Connect tabs plus the same feature-compass overflow route, while resolving that overflow action to a 44px terracotta target. Its focused visual contract now guards the exact action and target geometry. TypeScript passes; lint has the same four pre-existing warnings; focused visual/continuity/work-surface coverage passes 17 assertions; and the native 15-record platform contract remains green. Physical mobile safe-area, tab overlap, haptic, touch and device rendering proof remain explicit device boundaries.

Live canonical Chat measurement confirmed the three-region composition, return pathways, composer, inspector and primary navigation remain distinct but system-aligned, while the compact **Pin**, **Copy**, **Retry** and **Delete** message actions were real visible `button`s at 30px. They are not informational marks: each acts on an individual message. Cascade tracing located the higher-specificity legacy message-row rule and the existing late Chat completion owner. The correction now scopes only `.message-actions .message-action-btn` in `kurukoo-chat-visual-completion.css` to a 44×44px flex target; the established PWA freshness policy now treats both that completion stylesheet and its loader as live assets, preventing stale ignore-search cache delivery.

Fresh canonical `/chat` verification at `http://localhost:3010` confirms **Pin**, **Copy**, **Retry**, and **Delete** each render at exactly **44×44px**. Their existing `title`/`aria-label` values and `data-action` contracts remain `pin`, `copy`, `regenerate`, and `delete`; their 15px icons stay transparent, muted, and compact rather than becoming primary actions. The Chat sidebar, conversation main region, contextual inspector, and composer remain present. This is browser-rendered desktop evidence only; authenticated conversations, installed-PWA lifecycle, and physical-device touch proof remain external boundaries.

The final shared-footer interaction audit found the footer’s brand link at 44px but every visible public navigation and policy link initially rendered as an 11px inline target. These are interactive route continuations across the canonical public family, not compact state labels or informational copy. The existing final authority now resolves each shared footer link as a muted, transparent `inline-flex` 44px target and removes legacy list spacing so the multi-column information architecture remains quiet and compact. Rendered verification confirms all fourteen Core, Support, Legal & Connect links measure 44px without becoming primary actions or changing any public route.

A subsequent full visible-control scan of the rendered Marketing family found no remaining sub-44px route, form, disclosure, or continuation control. The only remaining 24px element was the arrow icon inside the illustrative conversation mockup. Source tracing confirmed it has no behavior owner and is not the real conversation continuation. The existing homepage behavior owner now marks it disabled, removes it from keyboard order, and hides it from assistive interaction while preserving the visual mockup; the actual **See the flow** continuation remains keyboard reachable at 44px and points to `/how-it-works`.

## Controlled canonical-main reconciliation evidence — 22 August 2026

> **Evidence only.** `docs/architecture/CURRENT_PRODUCT_TRUTH.md` remains the single current-state authority. This section reports the controlled Git reconciliation and rendered visual evidence; it does not classify product capability as complete.

Before reconciliation, local `main` was `5f775ecf8f440ed621efb2a062a32fcf4f07a616`, `origin/main` was `79e89d054ace580132a7815df408ef78897ff6ed`, and their merge base was `4eed2f963a08a839e5ab97cdedefb8c5d4820f43`. The divergence was 46 local-only and 177 remote-only commits. The temporary annotated safety tag `safety/local-main-before-controlled-reconciliation-20260822` preserves the local pre-merge commit. A normal, non-force merge created `0f018669`; its two parents preserve both histories.

| Local-only commits | Classification | Evidence-led disposition |
|---|---|---|
| `a4fcec41`, `75bd075c`, `10aa67e2` | Genuinely unique and valid | Current-main public, developer/auth, and Desk visual-owner convergence was retained. |
| `fa299cc1` | Superseded implementation, history retained | Remote canonical Quick Ride weave extended the same contracts, dispatch lifecycle, QR/offline continuation, and regression coverage; its fuller implementation was selected at the add/add conflicts. |
| `236aa2ab`, `12086aa0` | Documentation-only/historical | Their branch-precedence and prior-agent reconciliation evidence remains in history; remote `BRANCH_CONTROL` and current product truth are now the applicable governance source. |
| `1a12ed4c`, `64168e79` | Genuinely unique and valid | Runtime/conversation contract work was retained; the combined server entry preserves local `stopBackgroundServices()` graceful shutdown plus remote Chat-page routing. |
| `813fe6ed`, `1d382846`, `59a66d96`, `c796e2b8`, `36930538` | Genuinely unique and valid | Existing Discover, Partners, Pricing, Channels, and Help visual owners were not replaced by remote code. |
| `c5477205`, `e3db0481`, `87c26460` | Documentation-only/historical evidence | These evidence records remain subordinate to current truth and production/contract evidence. |
| `d15cd92a`, `3d753409`, `0fb34707`, `9c7a2a15`, `2c5555f4` | Genuinely unique and valid | Native, Desk, and Admin target/contract corrections were retained. |
| `ab3eedee`, `07b1397c`, `a342e37`, `c67c9ca`, `db7a274`, `71fd374`, `489753f` | Genuinely unique and valid | FAQ, PWA fallback, QR, Blog, footer, API docs, and public-boundary corrections remain scoped to their existing owners. |
| `5e23d1f`, `5f348d9`, `05ff7ed`, `4d25818`, `6c5c4d4` | Genuinely unique and valid | About, home, and native message/consent/verification target corrections were retained. |
| `0b74bb3`, `86158cc`, `c0df911`, `6c33e30`, `fb20983`, `2431f58` | Genuinely unique and valid | Public nav, PWA recovery, progressive auth, footer, mockup semantics, and Contact corrections remain compatible with the remote visual layers. |
| `bbfe70c`, `f10d5fe`, `cf425b4`, `39298b4`, `0334dfd` | Documentation-only/historical evidence | The existing audit records remain evidence only and do not override canonical current truth. |
| `d657f25`, `5f775ec` | Genuinely unique and valid | Native feature-compass and Chat message-action target corrections were retained; fresh guest Chat browser proof still measures Pin, Copy, Retry, and Delete at 44×44px. |

The 177 remote-only commits were independently grouped as current core services/types/routing (58), Quick Ride/economic lifecycle (29), visual/PWA/native owners (29), verification/CI (22), governance/docs (13), communication/call (12), mobile offline/QR (11), and voice (3). Current remote capability, Quick Ride, browser speech, test, security, and branch-governance work was retained. Generated `data/audits/main-reconciliation.{json,md}` was removed because the canonical truth gate identifies generated reconciliation snapshots as obsolete; `CURRENT_PRODUCT_TRUTH.md` is the sole truth authority.

Seven predicted merge conflicts were current-owner overlaps. The remote Quick Ride, QR scanner, offline-sync, and OS-weave implementations were selected only after source comparison showed fuller canonical lifecycle coverage; `src/index.ts` combines remote `chatPageRoutes` with local graceful background-service shutdown. Post-merge type validation exposed a missing static-header quote in `src/index.ts`, an invalid provider-policy lifecycle literal (`accepted` rather than canonical `connected`), and one obsolete native offline replay option. Each was corrected through its existing owner. The platform visual-coverage matcher now recognizes the actual `/legal/:section?` route, and the merged Quick Ride/browser-speech/visual-owner tests are wired into the root manifest.

Repository validation passed: TypeScript lint and build; canonical product-truth gate; strict/full CSS audits; 56 public routes; public runtime; Chat DOM safety; PWA contract; browser SpeechSynthesis contract; voice, Memory provenance/self-service, Quick Ride, desktop visual-authority, platform visual-coverage, economic security, and staged-secret checks. Native TypeScript passed; focused native visual/Chat/voice coverage passed 19 assertions; the 15-record native platform contract passed. Native lint has four existing warnings only: two duplicate React Native imports and two existing effect-dependency warnings.

Runtime evidence is limited to the local reconciled server at `localhost:3010`: public home, Legal, offline fallback, browser-speech asset delivery, protected Desk redirect to login, and canonical guest Chat rendered. Chat retains the sidebar/main/inspector/composer composition and all four real message actions at 44×44px with preserved labels and datasets; the browser-speech adapter is injected and available. Authenticated conversation persistence, signed-in Memory continuity, Economic Request mutation/cancellation, provider communication/realtime voice, physical microphone/device safe areas/IME/camera/notifications/haptics, payment/payout/delivery/Quick Ride dispatch, and external provider activation remain **UNVERIFIED** or **BLOCKED_EXTERNAL** until direct runtime/device/provider evidence exists.
