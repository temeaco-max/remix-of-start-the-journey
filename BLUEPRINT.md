...
The Discovery & Opportunity Network is the single canonical spatial interface for Nearby Radar, Discover, provider discovery, opportunity discovery, contributor invitations, and attributable referral entry. `src/services/discoveryNetwork.ts` owns source-attributed discovery entities, the lifecycle `discovered → candidate → opportunity → invited → claimed → onboarded → verified → available → executing → completed`, freshness, evidence level, approximate-location policy, bounded viewport/radius queries, pagination, clustering inputs, owned-cache persistence, Pulse synchronization, and the provider abstraction seam for future cached or licensed geospatial sources. MapLibre/OpenStreetMap-compatible rendering remains a presentation layer; the map never becomes the source of truth and country-specific map implementations are not created.

A discovered business, place, event, community context or other entity is not a Kurukoo user, provider, or available service merely because it appears on a map. Provider identity, claim, onboarding, verification, availability, execution and completion remain canonical evidence-backed states. Exact coordinates and internal source references are never exposed to the browser. Public or systematic/bulk Nominatim discovery is not used. Pulse records expire from the owned discovery cache when their live evidence is no longer present. Contributor and referral invitations create attributable lifecycle records only; they do not create fake accounts or imply a completed claim.

Discover and Chat share the universal action protocol. Map/list selection creates an explicit `open_discovery_entity` action with the canonical entity ID and optional conversation context. The Chat boundary validates that exact ID and preserves it as context; it must not re-arbitrate an explicitly identified entity by recency or generic similarity. Subsequent user turns return to the Brain's normal context arbitration while the canonical Discovery Network remains the owner of entity state.
...
Production activation remains conditional on a real JWT secret of at least 32 characters, configured notification and channel providers, verified callback/webhook paths, payment and affiliate configuration, and an explicit operator decision to enable both runtime flags. WhatsApp and Telegram webhook authentication failures now return a non-2xx response through the shared channel boundary; invalid secrets/signatures are not acknowledged as successful delivery, while internal processing errors remain distinct from authentication failures. Missing provider credentials must remain visible as setup required or unconfigured; they must never be represented as connected.
...
| Server TTS | Existing voice/channel boundary when an approved adapter exists | Gemini 2.5 Flash TTS or approved equivalent | Enable the server voice/TTS boundary explicitly with a provider, model, server-side key, session/rate limits, privacy review, and runtime validation | Gemini adapter is implemented at `/api/voice/tts`; disabled and unavailable by default until explicitly configured; no audio is fabricated |
...
## Current Admin and Programmatic SEO Tranche

The admin area is served through two distinct but coherent layers. `/admin`, `/admin/`, and `/admin/login` now resolve to the static admin authentication surface at `public/admin/login.html`; successful authentication continues to redirect to the existing static console. The repository already contains the revamped admin pages under `public/admin/`, including dashboard, advertising, analytics, AI agents, content, SEO, users, pricing, revenue, referrals, commissions, partnerships, social, artists, celebrity, scam, marketing, and future surfaces. These pages consume protected `/api/admin/*` boundaries; exposing the HTML shell does not expose administrative records or credentials.

The shared public head now consumes the canonical SEO record fields and renders canonical URLs, robots directives, keywords where configured, Open Graph metadata, Twitter card metadata, alternate-language links where configured, WebPage/Organization schemas, and approved FAQPage JSON-LD. This fixes the previous mismatch where `canonical_url`, `schemas`, and `faqs` were assembled by the SEO service but not fully emitted by the shared view. Public route and runtime contracts cover the admin entry, core static admin pages, canonical metadata, and JSON-LD output.
...
### Current coordinator producer wiring

The canonical Chat persistence boundary emits `chat.turn.completed` after an assistant turn is written. Economic Request creation, requirement updates, and lifecycle transitions emit `economic_request.state_changed` after the canonical database write. Verified provider offer reception emits `provider.offer.received` only after request ownership, provider verification, and offer persistence succeed. The internal notification queue emits `notification.action_required` for queue insertion and terminal internal delivery-state changes; this must not be read as external FCM, SMS, WhatsApp, Telegram, or email delivery. The FCM registration route is mounted at `/api/fcm/register` behind authenticated owner identity, stores the device token on the canonical memory profile, and is ready to call Firebase HTTP v1 when deployment service-account secrets are supplied. Provider acceptance is not device delivery and requires real-device validation. Living Memory emits `memory.context.retrieved` using bounded retrieval metadata only, without raw prompts or memory text. The verified payment webhook boundary emits `payment.webhook.verified` only after signature verification, idempotency, quote matching, canonical request transition, and escrow lock success. Reminder creation, cancellation, due processing, and recurrence scheduling emit `reminder.state_changed`. Safety check-in start, completion, and expiration into `escalation_pending` emit `safety.checkin.state_changed`; this never claims that an emergency contact or external transport was reached.

The coordinator remains deterministic and local-first (`rules-v1`) and uses canonical services as the owners of truth. SmolLM2 is configurable through `SMOLLM2_MODEL` and runs through the existing Transformers.js boundary with CPU `q4` quantization by default. The deployment example retains `HuggingFaceTB/SmolLM2-1.7B-Instruct`, while constrained hosts may select `HuggingFaceTB/SmolLM2-360M-Instruct`. The repository test `test:smollm2-local` has verified direct local inference, unifiedAiEngine attribution, and canonical Chat attribution using the loaded 360M checkpoint; no second Chat or AI authority was created. The teacher/evaluation boundary remains opt-in and fail-closed. These events establish repository-side observability and resumability; they do not fabricate provider availability, stock, inventory, payment settlement, fulfilment, delivery, verification, quotas, or external connectivity. Persisted `economic_request.state_changed`, `provider.offer.received`, and `payment.webhook.verified` events are handed to the coordinator for one bounded asynchronous continuation pass. The dispatcher is limited by event type and suppressed when the coordinator itself persists an event, preventing recursive execution. The continuation uses only the existing bounded capabilities and never replaces the canonical owner. External capabilities are classified as **ready for external activation** until credentials, provider callbacks, privacy/retention decisions, quota review, activation flags, and independent runtime evidence exist.
...
## Personal WhatsApp linked-device connector

Kurukoo includes an optional local WhatsApp Web multi-device connector that is separate from the Meta WhatsApp Cloud API channel. It is disabled by default and requires `KURUKOO_WHATSAPP_LINKED_DEVICE_ENABLED=true`, `KURUKOO_WHATSAPP_LINKED_DEVICE_ALLOW=true`, and an explicit `KURUKOO_WHATSAPP_LINKED_DEVICE_OWNER_PHONE` binding. The connector exposes owner-authenticated status, start, pairing-QR, stop, and logout routes under `/api/whatsapp-linked-device` and can also be started manually with `npm run whatsapp:linked-device`.

The connector uses the existing canonical Chat boundary for inbound messages and replies. It does not create a second Chat engine or state owner. Direct messages are processed; group messages remain disabled unless separately enabled. Session credentials are stored under the ignored persistent auth directory and require a long-running local or deployment process. Feature flags alone do not prove a connection: readiness remains pending until the linked-device session is actually paired and open. This connector must not be represented as Meta Cloud API or as a WhatsApp Business number, and no external message delivery is claimed without live session evidence.


### Central workspace placement
...
## Outcome-completeness convergence

Kurukoo evaluates completeness by **real-world outcome journey**, not by isolated feature presence. The canonical 205-skill outcome matrix is generated by `npm run audit:outcome-completeness` into `data/audits/outcome-completeness.json` and `data/audits/outcome-completeness.md`, with a regression guard at `npm run test:outcome-completeness`.

Each canonical skill is mapped to its existing Chat entry, context requirements, actors, capabilities, canonical objects, applicable lifecycle, execution boundaries, evidence requirements, UI and channel representations, linked-device behavior, notification/continuation behavior, failure/recovery path, external activation dependencies, and missing repository implementation. The matrix is derived from `skillFlows.ts`, the feature registry, canonical Chat, Economic Request, discovery, evidence, notification and execution boundaries; it is not a parallel router, transaction engine, provider system, memory system or UI action system.

The default outcome path is `natural request → intent/context → clarification → actors → discovery → offer/provider → negotiation → selection → Economic Request → authorization/payment → execution → physical or logistical execution where required → evidence → notification/continuation → failure/recovery → completion → reputation → memory/follow-up`. Individual skills use only the applicable stages, but every required stage must be represented by a canonical owner or an explicit repository-ready external-activation boundary.

The iPhone-repair journey remains the reference economic outcome: problem, diagnosis, provider discovery, provider communication, quote, negotiation, selection, payment or authorization, repair, collection or delivery, tracking, evidence, completion, settlement, review and recovery. External APIs, local partners and Kurukoo-operated execution are adapters behind the same canonical lifecycle. The Brain interprets, arbitrates context and plans; canonical domain services alone authorize, mutate state, store evidence and perform irreversible actions. External completion is never claimed without independent provider evidence.
...
### Whole-platform assistance and recovery convergence

Kurukoo is not narrowed to economic fulfilment. The same Brain-mediated Chat relationship supports assistance, information, guidance, support, memory, reminders, safety, community content, discovery, economic problem solving, provider selection, negotiation, commitment, payment boundaries, execution, evidence, completion, recovery and escalation. Question-form guidance is projected from the existing content and Topic authorities and may offer a transition to verified help; it does not claim a provider, availability, neutral recommendation, payment, execution or completion. Explicit requests to find, hire, book, order or get someone remain in the canonical Economic Request flow. Native Help, Resources, Topics, Discover, Requests, Tasks, Notifications, product cards and support surfaces project canonical state rather than creating parallel state. The distinction between organic discovery, verified provider, sponsored placement, affiliate relationship, contributor referral and user-selected option remains explicit in Chat and native UI.


## Current Universal Capability Operating Contract

Natural language is Kurukoo's primary user interface. Kurukoo is one AI relationship in front of the operating system and opportunity network: the conversational layer interprets, presents and coordinates, while canonical domain services remain authoritative for identity, authorization, consent, canonical state, payment, subscriptions, provider truth, Economic Requests, evidence, notifications, memory, Points, QR, agent goals and irreversible actions.

The existing Chat capability/action protocol is the single machine-readable contract for Web Chat, PWA, linked devices, configured WhatsApp/Telegram/SMS/USSD/email/voice surfaces, first-class agents and future channels. Its shared descriptor covers capability, action, context, canonical object, required and optional inputs, permissions, owner, risk, consent, confirmation, lifecycle, canonical facts, execution status, evidence status, next actions, failure/retry/recovery, continuation context, external dependency state and activation state.

Model proposals are advisory. They must re-enter the canonical action boundary, which validates capability/action support, owner-scoped context and object identity, permissions, required inputs, lifecycle, risk, consent, idempotency and stale/foreign context. Explicit object identity is never re-arbitrated by recency or semantic similarity; missing, stale, foreign or unsupported actions fail closed.

Canonical results distinguish accepted, waiting, needs-user, confirmation-required, blocked, failed, completed, externally-pending and unavailable-external-dependency. Structured facts and cards remain owned by canonical services. The conversational layer may explain them in `generate`, `present` or `deterministic` mode but may not invent provider availability, payment, evidence, prices, inventory, delivery or completion. Failure results preserve the canonical object and expose retry, resume, cancel, wait or escalation paths through existing owners.

The implementation seams are `src/services/universalCapabilityProtocol.ts`, which adapts the existing 205-skill/46-family metadata and bounded `agentToolRegistry` into the shared catalog and result contract, and `src/services/canonicalCapabilityExecutor.ts`, which is exposed to authenticated Chat through `POST /api/chat/action`. The executor validates owner-scoped conversation and exact canonical object identity, applies confirmation and idempotency policy, dispatches only to existing canonical services, and returns a universal result with canonical facts/card projections. Current universal adapters include reminders, agent goals, notifications, memory revocation and selected Economic Request decisions (update, cancellation and provider selection). Other payment, provider, channel, product, subscription and fulfilment actions remain repository-ready behind their existing canonical boundaries until external evidence or a dedicated universal adapter exists; no click or response claims external completion. This does not create another Brain, router, capability registry, transaction engine, provider engine, memory system, agent runtime or UI action system.


## Emergency interrupt contract — current implementation

Emergency assistance is a first-class interruptive outcome. The canonical turn boundary evaluates emergency language before onboarding, authentication, Economic Requests, provider discovery, reminders, memory, agents, products, notifications and ordinary conversation. A guest may receive initial emergency information and a permitted dial option without registration; authentication may be requested later only for optional persistence or follow-up and must not delay initial assistance.

Emergency handling uses the universal capability vocabulary `emergency.assess`, `emergency.location`, `emergency.dial`, `emergency.connect`, `emergency.end` and `emergency.followup`, owned by the existing canonical Chat, emergency, voice and WebRTC boundaries. The canonical directory currently contains Nigeria's national 112 route for ambulance, police and fire, sourced from the Nigerian Communications Commission. Local alternatives require an authoritative source record and are never inferred from OpenStreetMap or generated by the model.

Location may be supplied by the user, trusted profile, permitted device location or manual selection. Approximate or unknown location does not block the initial emergency path. Dial state is explicit (`dial_requested`, `dialing`, `ringing`, `connected`, `ended`, `failed`, or `unavailable`). Kurukoo may expose a browser `tel:` action where supported, but it never claims a call, dispatch, responder acceptance, medical diagnosis or emergency-service availability without actual evidence from an activated telephony boundary. Emergency state is visible in Chat and can end or return to the prior conversation without corrupting prior contexts.


## Interaction-policy convergence — 17 August 2026
The existing interaction-policy service is now the first semantic decision boundary of the canonical Chat turn. Every turn is evaluated for priority, interruption, guest access, authentication timing, exact-context preservation and resumability before onboarding, generic intent routing or model generation. The policy result is persisted in Chat telemetry; it does not replace the Brain, context arbitration, Intent Router, FastText, SmolLM2, unifiedAiEngine, canonical services or agent runtime.

Critical emergency help remains available to a guest before ordinary identity collection. Security interruptions such as a stolen phone, account-access concern or unrecognized payment receive immediate protective guidance without silently changing ownership, cancelling a payment, issuing a refund or claiming recovery. Mutating security, payment, account, device, memory, public or channel state requires the existing owner, identity, confirmation, evidence and canonical-service boundaries.

The registered `emergency` universal capability owns emergency assessment, location, dial, connection, ending and follow-up. The older `safety/emergency_dispatch` proposal is retained only as an exact compatibility alias into that descriptor and executor; it is not a second safety or emergency engine. The universal policy matrix covers emergency interruption, security interruption, reminder and agent switches, payment and remote external-unavailable outcomes, memory provenance, public draft-versus-commit, exact identity, confirmation and resumable contexts.

Repository-side readiness remains distinct from external activation. Browser `tel:` links, provider adapters, channel boundaries and payment boundaries may be presented as available-to-configure or waiting, but Kurukoo must not claim connection, dispatch, delivery, settlement, provider availability, recovery or fulfilment without external evidence.
