...
Historical validation notes are retained rather than hidden, but they are not current defects: the earlier report of 217 legacy inline-style occurrences has been superseded by `npm run audit:css:strict`, which now reports no inline CSS attributes or frontend `<style>` tags. The production dependency audit still reports the upstream `sharp`/`@huggingface/transformers` advisory path with no currently safe npm fix; do not force an untested override merely to change audit output.
...
The reusable `scripts/provider-outcome-scenario-lab.ts` derives a deterministic provider-led scenario universe from the canonical skill-flow and service registries. It does not create per-skill engines or scenario-specific business logic. The current default corpus contains **24,000 retained scenarios** from a 28,700-instance full universe. It covers all 205 canonical skills, all 46 families, seven market configurations, six locale/dialect groups, ten channel/context surfaces, six actor types, six provider types, linked-device and QR entry, and 20 lifecycle, failure, recovery, identity, safety, legal, negotiation, interruption, duplicate, stale-action, and notification variants. The generated trajectories span horizons from 5 to 81 turns, totaling 943,188 turns with an average of 39.30 turns per scenario.

Run `npm run scenario-lab:generate` to regenerate the deterministic JSONL scenario corpus and its SmolLM2-labelled corpus. Run `KURUKOO_SCENARIO_EXECUTION_LIMIT=1000 npm run scenario-lab:execute` to execute a stratified 1,000-scenario sample through the canonical `routeIntent` semantic boundary in an isolated temporary `DB_PATH`; the run includes at least one probe for every canonical skill and does not mutate the normal development database or claim provider availability or external fulfilment. Set `KURUKOO_SCENARIO_EXECUTION_LIMIT` lower only for a quick local smoke run. Run `KURUKOO_REQUIRE_SCENARIO_EXECUTION=true npm run test:scenario-lab` to require canonical probe results. The semantic probe records the FastText runtime state honestly: a missing CLI uses the configured deterministic fallback rather than fabricating local-model availability.
...
The readiness vocabulary is explicit: `LOCALLY_COMPLETE`, `DEVELOPMENT_FIXTURE_COMPLETE`, `REPOSITORY_READY_EXTERNAL_ACTIVATION_REQUIRED`, `STRUCTURALLY_INCOMPLETE`, `BLOCKED_BY_EXTERNAL_DEPENDENCY`, and `INTENTIONALLY_UNSUPPORTED`. The laboratory is synthetic-only, does not feed scenario data into normal runtime authority, and never promotes an adapter into live, active, verified, connected, delivered, or externally completed status. `npm run test:repository-convergence` is the broad repository-side gate: it composes all-domain Chat/model checks, the complete route and lifecycle suite, the deterministic scenario laboratory, and the long-horizon trajectory harness.
