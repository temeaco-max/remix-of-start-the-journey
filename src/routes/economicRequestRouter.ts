...
/**
 * Universal economic request boundary.
 * Categories describe requirements and capabilities; they do not get bespoke
 * transport or conversation systems. Skills may add stricter requirements
 * without bypassing this lifecycle.
 */
...
const ALLOWED_STATUSES = new Set(['requested', 'awaiting_match', 'partially_matched', 'fulfilled', 'abandoned']);
...
function cleanRequirements(value: unknown): Record<string, unknown> {
  ...
  return Object.fromEntries(Object.entries(input).filter(([key]) => /^[a-zA-Z][a-zA-Z0-9_]{0,63}$/.test(key)));
}

function validateRequirements(category: string, requirements: Record<string, unknown>, allowPartial: boolean): string[] {
...
router.post('/', async (req: AuthRequest, res) => {
  ...
  try {
    const request = await createEconomicRequest({
      ...
      skill,
      requirements,
      amount: typeof req.body?.amount === 'number' && Number.isFinite(req.body.amount) ? req.body.amount : undefined,
    });
