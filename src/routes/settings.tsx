...
export const Route = createFileRoute("/settings")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Control how Kurukoo behaves, remembers, communicates and works for you.",
      },
    ],
  }),
...
function ProfileEditor() {
  ...
  useEffect(() => {
    void api<{
      profile?: { name?: string | null; location?: string | null; email?: string | null };
    }>("/api/profile")
      .then(({ profile }) => {
    ...
    <div className="flex flex-col items-stretch gap-2 sm:items-end">
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <button
          ...
          onClick={() => void save()}
          disabled={saving}
          className="h-9 rounded-lg bg-primary px-3 text-[12px] font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50"
        >
          {saving ? "Saving…" : "Save"}
        </button>
      </div>
...
function PersistentToggle({
  ...
  return (
    <button
      ...
      className={`relative h-6 w-10 shrink-0 rounded-full transition-colors ${on ? "bg-primary" : "border border-border bg-elevated"}`}
    >
      <span
        className={`absolute top-1/2 size-[18px] -translate-y-1/2 rounded-full bg-surface shadow-[var(--shadow-soft)] transition-all ${on ? "left-[19px]" : "left-[3px]"}`}
      />
    </button>
  );
}
        ...
        {tab === "Account" && (
          <>
            <SettingIntro
              ...
              title="Make Kurukoo feel like yours"
              description="Your account profile is stored with Kurukoo and used to personalise your experience."
            />
            <Rows>
              <SettingsRow
                title="Profile"
              ...
              <SettingsRow
                ...
                control={
                  <Link to="/trust" className={actionClass()}>
                    Security
                  </Link>
                }
              />
              <SettingsRow
                title="Security"
                description="Credentials, sessions and trusted execution"
                control={
                  <Link to="/trust" className={actionClass()}>
                    Manage
                  </Link>
              ...
              <SettingsRow
                title="Memory"
                description="What Kurukoo remembers about you"
                control={
        ...
        {tab === "Privacy" && (
          <>
            <SettingIntro
              ...
              title="Control what Kurukoo may use"
              description="These preferences take effect in this browser. Consequential actions still use Kurukoo's approval and trust controls."
            />
            <Rows>
              <SettingsRow
                title="Ask before contacting anyone"
                description="Require approval before outbound contact"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-ask-before-contact"
                    label="Ask before contacting"
                    defaultOn
                  />
                }
              />
              <SettingsRow
                title="Share location for discovery"
                description="Allow nearby recommendations to use your location"
                control={
                  <PersistentToggle storageKey="kurukoo-share-location" label="Share location" />
                }
              />
              <SettingsRow
                title="Personalised recommendations"
                description="Use activity and interests to improve suggestions"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-personalised"
              ...
              <SettingsRow
                ...
                description="Export your account data as JSON"
                control={<DataExport />}
              />
            </Rows>
          </>
        ...
        {tab === "Notifications" && (
          <>
            <SettingIntro
              ...
              title="Only hear when it matters"
              description="Keep approvals, replies and important activity close without turning Kurukoo into a noisy feed."
            />
            <Rows>
              <SettingsRow
                title="Approvals"
                description="When Kurukoo needs a decision"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-notifications-approvals"
                    label="Approvals"
                    defaultOn
                  />
                }
              />
              <SettingsRow
                title="Network replies"
                description="When someone responds to a request"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-notifications-replies"
              ...
              />
              <SettingsRow
                title="Followed topics and people"
                description="Updates from things you follow"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-notifications-following"
              ...
              <SettingsRow
                ...
                description="New Kurukoo features and improvements"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-notifications-product"
        ...
        {tab === "Services" && (
          <>
            <SettingIntro
              ...
              title="Choose what Kurukoo can work with"
              description="Manage live service connections and resources from Connect."
            />
            <Rows>
              <SettingsRow
                title="Connected services"
                description="Storage, messaging, email, calendar and other resources"
                control={
              ...
              />
              <SettingsRow
                title="Outcomes and files"
                description="Connected resources and completed outputs"
                control={
                  <Link to="/artifacts" className={actionClass()}>
                    Open Artifacts
                  </Link>
                }
              />
        ...
        {tab === "Voice" && (
          <>
            <SettingIntro
              icon={Waves}
              eyebrow="Voice"
              title="Talk to Kurukoo naturally"
              description="Use voice as another way to tell Kurukoo what needs doing. Voice features fail clearly when the configured provider or browser capability is unavailable."
            />
            <Rows>
              <SettingsRow
                title="Voice input"
                description="Use voice in conversations"
                control={
                  <PersistentToggle
                    storageKey="kurukoo-voice-input"
                    label="Voice input"
                    defaultOn
                  />
                }
              ...
              <SettingsRow
                ...
                description="Browser speech output"
                control={
                  <PersistentToggle storageKey="kurukoo-voice-output" label="Read replies aloud" />
                }
              />
              <SettingsRow
                title="Voice wake phrase"
                description="Listen for the phrase inside the open voice screen"
                control={
                  <PersistentToggle storageKey="kurukoo-voice-wake" label="Voice wake phrase" />
                }
              />
              <SettingsRow
                title="Voice preferences"
                description="Language and voice style are managed in Chat"
                control={
        ...
        {tab === "Money" && (
          <>
            <SettingIntro
              icon={ShieldCheck}
              eyebrow="Money"
              title="Keep payments under your control"
              description="Payment instruments and secure execution controls live in Trust."
            />
            <Rows>
              <SettingsRow
                title="Wallet"
                description="Canonical money state when exposed"
                control={
              ...
              />
              <SettingsRow
                title="Points and usage"
                description="Keep Points separate from money"
                control={
                  <Link to="/usage" className={actionClass()}>
                    Open Usage
                  </Link>
              ...
              <SettingsRow
                ...
                description="Protected payment instruments"
                control={
                  <Link to="/trust" className={actionClass()}>
                    Manage in Trust
                  </Link>
              ...
              />
              <SettingsRow
                title="Network, business and creator accounts"
                description="Account types connected to Kurukoo"
                control={
        ...
        {tab === "Appearance" && (
          <>
            <SettingIntro
              icon={Sparkles}
              eyebrow="Appearance"
              title="Make Kurukoo comfortable everywhere"
              description="The shell follows your system appearance and keeps the same Kurukoo visual language across surfaces."
            />
            <Rows>
              <SettingsRow
                title="Theme"
                description="Use the appearance control in the shell"
                control={
                  <span className="text-[12px] text-muted-foreground">System / Light / Dark</span>
                }
              />
              <SettingsRow
                title="Reduce motion"
                description="Follows your system setting"
                control={<span className="text-[12px] text-muted-foreground">System</span>}
              />
              <SettingsRow
                title="Text size"
                description="Follows your browser setting"
                control={<span className="text-[12px] text-muted-foreground">Default</span>}
              />
            </Rows>
          </>
        )}
