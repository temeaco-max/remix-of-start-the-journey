...
function WorkDetail() {
  ...
  useEffect(() => {
    ...
    void fetchEconomicRequest(workId)
      .then(setRequest)
      .catch((error) => setRequestError(error instanceof Error ? error.message : ""));
  }, [workId]);
  ...
  return (
    <>
      ...
      {canonical ? (
        <>
          <section className="mt-8">
            <SectionHeader title="Request details" subtitle="The information Kurukoo is carrying for this request." />
            {request ? (
              <Rows>
              ...
              <EmptyState title="Loading the canonical request" body="Kurukoo is checking the live request record before showing its details." />
            )}
          </section>

          {request ? (
            <section className="mt-8">
              <SectionHeader title="Commercial state" subtitle="Only values returned by the canonical request are shown." />
              <Rows>
            ...
            <Rows>
              {entities.slice(0, 2).map((e) => (
                <ContactRow
                  key={e.id}
                  entity={e}
                  right={<Link to="/messages" className="text-[13px] text-muted-foreground hover:underline">Message</Link>}
                />
              ))}
            </Rows>
          </section>
      ...
      <div className="mt-6 flex gap-2">
        ...
        {!canonical ? (
          <Link to="/artifacts"><Action>Open files</Action></Link>
        ) : null}
      </div>

      {request ? (
        <section className="mt-8">
          <SectionHeader title="Canonical request" subtitle="Live Economic Request state from Kurukoo." />
          <div className="rounded-2xl border border-border bg-surface p-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-elevated px-2.5 py-1 text-[10.5px] font-medium">{request.status.replace(/[_-]/g, " ")}</span>
              <span className="text-[10.5px] text-muted-foreground">{request.skill.replace(/[_-]/g, " ")}</span>
            </div>
            <button type="button" onClick={() => send("Review the current state of this request and tell me what needs my approval.")} className="mt-3 rounded-lg bg-primary px-3 py-2 text-[11.5px] font-medium text-primary-foreground">Ask Kurukoo about this request</button>
          </div>
        </section>
      ) : requestError ? (
        <p className="mt-5 text-[11px] text-muted-foreground">{requestError}</p>
      ) : null}
