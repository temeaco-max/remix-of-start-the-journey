...
function ExplorePage() {
  ...
  const results = useMemo(() => {
    ...
    return entities.filter((e) => {
      const kindOk = cat === "All" || (cat === "Providers" && e.kind === "provider") || (cat === "Businesses" && e.kind === "business") || (cat === "Creators" && e.kind === "creator");
  ...
  return <div className="space-y-8">
    ...
    <section className="relative overflow-hidden rounded-[24px] border border-border bg-surface p-5 shadow-[var(--shadow-soft)] md:p-6">
      ...
      <div className="relative mt-5"><SearchField label="Search Kurukoo" placeholder="Search providers, topics, creators…" value={q} onChange={setQ} /></div>
    ...
    {showEntities ? <section><SectionHeader title={cat === "All" ? "People and places" : cat} subtitle={`${results.length} results`} /><div className="grid gap-3">{results.map((e) => <EntityCard key={e.id} entity={e} />)}</div><div className="mt-3"><AdSlot placement="Explore feed" headline="Sponsored result" body="Sponsored results are visually separated from organic discovery." advertiser="Example advertiser" /></div></section> : null}
    ...
    <div className="flex items-center gap-2 text-[12px] text-muted-foreground"><Sparkles className="size-3.5" />Discovery runs on prototype data — live directory, ranking and availability arrive with the backend.</div>
    <IntegrationGap>Nothing discovered here should be presented as confirmed availability until Kurukoo has verified it.</IntegrationGap>
  </div>;
}
