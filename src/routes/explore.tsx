...
function ExplorePage() {
  ...
  return <div className="space-y-8">
    ...
    {showEntities ? <section><SectionHeader title={cat === "All" ? "People and places" : cat} subtitle={`${results.length} results`} /><div className="grid gap-3 xl:grid-cols-3">{results.map((e) => <EntityCard key={e.id} entity={e} />)}</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><AdSlot placement="Explore feed" headline="Sponsored result" body="Sponsored results are clearly separated from organic discovery." advertiser="Example advertiser" /><AdSlot placement="Explore feed" headline="Sponsored result" body="Sponsored results are clearly separated from organic discovery." advertiser="Example advertiser" /></div></section> : null}
