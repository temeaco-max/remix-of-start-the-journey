...
function ExplorePage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section><SectionHeader title="Start a request" subtitle="Hand a useful task straight to Kurukoo." /><div className="flex flex-wrap gap-2">{starters.map((starter) => <button key={starter} type="button" onClick={() => { send(starter); void navigate({ to: "/chat" }); }} className="rounded-xl border border-border bg-surface px-3.5 py-2 text-[11.5px] font-medium transition-colors hover:bg-elevated">{starter}</button>)}</div></section>

      <section><div className="flex items-center justify-between gap-3"><SectionHeader title="Live discovery" subtitle="Current discovery results are shown with their evidence and availability state." /><button type="button" onClick={() => void loadDiscovery()} className="inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-border px-2.5 text-[10.5px] font-medium hover:bg-elevated"><RefreshCw className="size-3" /> Refresh</button></div>{discoveryError ? <IntegrationGap title="Live discovery unavailable" description={discoveryError} /> : discoveryLoading ? <div className="rounded-[18px] border border-border bg-surface p-5 text-[12px] text-muted-foreground">Checking nearby discovery…</div> : liveResults.length ? <div className="grid gap-3 md:grid-cols-2">{liveResults.slice(0, 6).map((entity) => <DiscoveryEntityCard key={entity.id} entity={entity} />)}</div> : <div className="rounded-[18px] border border-border bg-surface p-5 text-[12px] text-muted-foreground">No current discovery results matched this search.</div>}</section>

      {exploreAd ? <section><SectionHeader title="Sponsored" subtitle="Paid placement is kept separate from organic discovery." /><SponsoredExploreCard campaign={exploreAd} /></section> : null}

      <section><SectionHeader title="Community context" subtitle="Moderated topics can add local context; they do not establish provider availability." />{topicError ? <IntegrationGap title="Community context unavailable" description={topicError} /> : topicLoading ? <div className="rounded-[18px] border border-border bg-surface p-5 text-[12px] text-muted-foreground">Loading moderated topics…</div> : filteredTopics.length ? <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{filteredTopics.slice(0, 6).map((topic) => <ExploreTopicCard key={topic.id} topic={topic} />)}</div> : <div className="rounded-[18px] border border-border bg-surface p-5 text-[12px] text-muted-foreground">No topics matched this search.</div>}</section>

      <section><SectionHeader title="People, places and opportunities" subtitle="The existing network view stays available alongside goal-first exploration." /><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{results.slice(0, 6).map((entity) => <EntityCard key={entity.id} entity={entity} />)}</div></section>

      <section><SectionHeader title="Watch" subtitle="Useful community media without turning Explore into an uncontrolled feed." /><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{videos.slice(0, 3).map((video) => <VideoCard key={video.id} video={video} />)}</div></section>
