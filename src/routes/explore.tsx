...
export const Route = createFileRoute("/explore")({ head: () => ({ meta: [{ title: "Explore — Kurukoo" }, { name: "description", content: "Discover people, businesses, services, creators, topics, opportunities and useful content." }, { property: "og:title", content: "Explore — Kurukoo" }, { property: "og:description", content: "Discover useful people, places, ideas, services and opportunities." }] }), component: ExplorePage });
...
function ExploreTopicCard({ topic }: { topic: CanonicalTopic }) {
  ...
  return <Link to="/topics/$slug" params={{ slug: topic.slug }} className="group rounded-[18px] border border-border bg-surface p-4 transition-colors hover:bg-elevated/50">
    <div className="flex items-center justify-between gap-3"><div className="flex flex-wrap gap-1.5 text-[11px] text-muted-foreground"><span className="rounded-full bg-elevated px-2 py-1 font-medium text-foreground/80">{pretty(topic.type)}</span>{topic.category ? <span className="rounded-full bg-elevated px-2 py-1">{pretty(topic.category)}</span> : null}{locality ? <span className="rounded-full bg-elevated px-2 py-1">{locality}</span> : null}</div><ArrowUpRight className="size-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5" /></div>
    <h3 className="mt-4 text-[15.5px] font-medium leading-snug">{topic.title}</h3>
    <p className="mt-1.5 line-clamp-2 text-[12.5px] leading-relaxed text-muted-foreground">{topic.body}</p>
    <div className="mt-3 flex items-center gap-2 text-[11px] text-muted-foreground"><span>{topic.replyCount} {topic.replyCount === 1 ? "moderated reply" : "moderated replies"}</span><span>·</span><span>{topic.authorLabel}</span></div>
  </Link>;
}
...
function ExplorePage() {
  ...
  async function loadTopics(refresh = false) {
    ...
    try { setTopicData(await fetchCanonicalTopics(6)); setTopicError(""); }
    catch (error) { setTopicData([]); setTopicError(error instanceof Error ? error.message : "Community context is unavailable right now."); }
    finally { setTopicLoading(false); setTopicRefreshing(false); }
  }
  ...
  const results = useMemo(() => {
    ...
    return entities.filter((e) => {
      const kindOk = cat === "All" || (cat === "Network" && e.kind === "provider") || (cat === "Businesses" && e.kind === "business") || (cat === "Creators" && e.kind === "creator") || (cat === "Opportunities" && ["partner", "contributor"].includes(e.kind));
      return kindOk && (!term || (e.name + e.tagline + e.topics.join(" ")).toLowerCase().includes(term));
    });
  }, [q, cat]);
  const filteredTopics = useMemo(() => { const term = q.trim().toLowerCase(); return topicData.filter((topic) => !term || [topic.title, topic.body, topic.type, topic.category ?? "", ...topic.skills, topic.city ?? "", topic.lga ?? ""].join(" ").toLowerCase().includes(term)); }, [q, topicData]);
  const showEntities = cat === "All" || cat === "Network" || cat === "Businesses" || cat === "Creators" || cat === "Opportunities";
  ...
  return <div className="space-y-8"><PageHeader title="Explore" subtitle="Find people, places, ideas and opportunities — then hand the useful part to Kurukoo." />
    ...
    {cat === "All" || cat === "Opportunities" ? <section><SectionHeader title="Opportunities" subtitle="Useful ways to participate, earn or collaborate." action={<Link to="/opportunities" className="inline-flex items-center gap-1 text-[13px] text-muted-foreground">See opportunities <ArrowUpRight className="size-3.5" /></Link>} /><div className="grid gap-3 sm:grid-cols-2"><Link to="/opportunities" className="group rounded-[18px] border border-border bg-surface p-4 transition-colors hover:bg-elevated/50"><span className="grid size-9 place-items-center rounded-xl bg-elevated"><BriefcaseBusiness className="size-4" /></span><h3 className="mt-4 text-[14px] font-semibold">Find work and collaborations</h3><p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">See opportunities that can become useful requests or introductions.</p><span className="mt-3 inline-flex items-center gap-1 text-[11.5px] font-medium">Open opportunities <ArrowUpRight className="size-3.5" /></span></Link><Link to="/how-it-works" className="group rounded-[18px] border border-border bg-surface p-4 transition-colors hover:bg-elevated/50"><span className="grid size-9 place-items-center rounded-xl bg-elevated"><Sparkles className="size-4" /></span><h3 className="mt-4 text-[14px] font-semibold">Understand the system</h3><p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">See how discovery becomes coordination and where you stay in control.</p><span className="mt-3 inline-flex items-center gap-1 text-[11.5px] font-medium">How it works <ArrowUpRight className="size-3.5" /></span></Link></div></section> : null}
    <div className="flex items-center gap-2 text-[12px] text-muted-foreground"><Sparkles className="size-3.5" />Community context comes from real public Topics; verified availability remains a separate discovery signal.</div><IntegrationGap>Availability is only shown as confirmed when Kurukoo can verify it.</IntegrationGap>
  </div>;
}
