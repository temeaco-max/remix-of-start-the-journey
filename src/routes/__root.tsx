...
import { PageHeader } from "../components/kurukoo/page-header";
import { KurukooProvider } from "@/lib/kurukoo-store";

const publicSurfacePrefixes = ["/about", "/blog", "/contributors", "/help", "/login", "/partners", "/pricing", "/signup", "/use-cases"];
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1, initial-scale=1, viewport-fit=cover" },
      { title: "Kurukoo" },
...
function RootComponent() {
  ...
  return <QueryClientProvider client={queryClient}><KurukooProvider>{isPublicSurface ? <Outlet /> : <AppShell><PageHeader /><Outlet /></AppShell>}</KurukooProvider></QueryClientProvider>;
}
