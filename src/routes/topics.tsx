...
function TopicsPage() {
  ...
  async function loadTopics(refresh = false) {
    ...
    try {
      ...
      setTopics([]);
      setError(
        nextError instanceof Error ? nextError.message : "Public Topics are unavailable right now.",
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const topicSummary = useMemo(
    () =>
      loading
        ? "Loading community context…"
        : error ||
          (!topics.length
            ? "No public Topics match this view yet."
            : `${topics.length} public ${topics.length === 1 ? "Topic" : "Topics"} in this view`),
    ...
    <div className="w-full max-w-5xl space-y-0">
      <section className="max-w-3xl">
        ...
        <p className="mt-4 text-[16px] leading-relaxed text-muted-foreground">
          Questions, experiences, local reports and opinions become shared context. Kurukoo can use
          that context to help you think and act, but community posts never become proof of a
      ...
      <section className="mt-6" aria-live="polite">
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <Link
            to="/explore"
            className="inline-flex items-center gap-1 text-[12.5px] font-medium text-primary hover:opacity-80"
          >
        ...
        {loading ? (
          <div className="grid gap-3 sm:grid-cols-2">
            {[0, 1, 2, 3].map((item) => (
              <div
                key={item}
                className="h-44 animate-pulse rounded-[18px] border border-border bg-surface"
              />
            ))}
          </div>
        ) : topics.length ? (
          ...
          <Panel className="p-6">
            ...
            <p className="mt-1.5 max-w-2xl text-[13px] leading-relaxed text-muted-foreground">
              Kurukoo keeps this surface truthful: it shows real public Topics from the community
              and does not manufacture activity to make the feed look busy.
            </p>
            <Link
              to="/topics/create"
              className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-border px-3.5 py-2 text-[12.5px] font-medium hover:bg-elevated"
            >
              <Plus className="size-3.5" />
              Start the first Topic <ArrowUpRight className="size-3.5" />
            </Link>
          </Panel>
        )}
      </section>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div>
            ...
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">
              Questions, opinions and experiences can accumulate into a useful public record.
            </p>
          </div>
          <div>
            <p className="text-[13.5px] font-medium">Context</p>
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">
              Topics can carry category, skill and broad local context without exposing precise
              private details.
            </p>
          </div>
          <div>
            ...
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-muted-foreground">
              When something matters to you, take it into Chat and let Kurukoo handle the verified
              next step.
            </p>
          </div>
        </div>
