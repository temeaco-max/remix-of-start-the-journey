  ...
  if (!entity) {
    return (
      <>
        ...
        <Link to="/explore" className="mt-4 inline-block text-[14px] underline">Back to Explore</Link>
      </>
    );
  }
    ...
    <>
      <header className="flex items-start gap-4 pb-6 pt-2">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="flex items-center gap-1.5 text-[24px] font-semibold">{entity.name}{entity.verified ? <BadgeCheck className="size-5 text-primary" aria-label="Verified" /> : null}</h1>
          <p className="mt-1 text-[14.5px] text-muted-foreground">{entity.tagline}</p>
          <p className="mt-1 text-[13px] text-muted-foreground">{kindLabel[entity.kind]}{entity.location ? ` · ${entity.location}` : ""}</p>
        </div>
      </header>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatTile label="Rating" value={entity.rating ? `${entity.rating} / 5` : "—"} note={entity.rating ? "Community rating" : "Not rated yet"} />
      ...
      {entity.services?.length ? (
        <section className="mt-8">
          ...
          <Rows>{entity.services.map((s) => <li key={s} className="flex items-center justify-between gap-4 px-4 py-3.5"><span className="text-[15px]">{s}</span><Link to="/" className="text-[13.5px] underline" onClick={() => send(`${s} with ${entity.name}.`)}>Request</Link></li>)}</Rows>
        </section>
      ) : null}
      ...
      {theirVideos.length ? (
        <section className="mt-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2">{theirVideos.map((v) => <VideoCard key={v.id} video={v} creatorName={entity.name} />)}</div>
        </section>
      ) : null}

      <IntegrationGap>Availability and ratings are shown here only when Kurukoo has reliable information.</IntegrationGap>
    </>
  );
}
