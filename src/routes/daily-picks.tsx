    ...
    <section className="rounded-[22px] border border-border bg-surface p-5 md:p-6">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-primary">Live pick</p>
          <h2 className="mt-1.5 font-serif text-[28px] leading-tight tracking-[-0.035em]">
            {pick.title}
          </h2>
...
function DailyPicksPage() {
  ...
  return (
    <>
      ...
      {loading ? (
        <div className="h-32 animate-pulse rounded-[22px] bg-elevated/40" />
      ) : pick ? (
        <LivePick pick={pick} />
      ) : (
        <section className="rounded-[22px] border border-dashed border-border px-5 py-7">
          <p className="text-[12.5px] font-medium">No live pick is available right now.</p>
          <p className="mt-1 text-[11px] text-muted-foreground">
            The examples below keep the experience populated while live recommendation inventory is
            unavailable.
          </p>
          <button
            type="button"
            ...
            className="mt-3 inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-border px-3 text-[11px] font-medium"
          >
            <RefreshCw className="size-3.5" /> Try live pick again
          </button>
        </section>
      )}
      <section className="mt-7">
        <SectionHeader
          ...
          subtitle="Example content for the OS surface; live recommendations remain separate."
        />
        <DailyPicksGrid />
      </section>
