...
async function finalizeAuthenticatedSession(phone: string, res: any, options: { name?: string; email?: string; goal?: string; guestPhone?: string; deviceId?: string; credentialType?: string; pushCapable?: boolean; clearGuest?: boolean } = {}) {
  ...
  if (!userPhone) throw new Error('Authenticated phone is required');
  await upsertProfile(userPhone, options.name, options.email, options.goal);
  const token = issueUserToken(userPhone);
  const deviceId = String(options.deviceId || '').trim();
  if (deviceId && !isProvisionalEmailSubject(userPhone) && !isGuestSubject(userPhone)) {
    await registerTrustedDevice({ phone: userPhone, deviceId, credentialType: (options.credentialType || 'web') as any, label: options.name ? `${options.name} browser` : 'Kurukoo browser', pushCapable: options.pushCapable === true });
  }
  const clearGuest = options.clearGuest === true || String(options.guestPhone || '').startsWith('anon_');
  setAuthCookie(res, token, clearGuest);
  if (String(options.guestPhone || '').startsWith('anon_') && !isProvisionalEmailSubject(userPhone)) {
    try {
      await migrateGuestSessionToAccount(String(options.guestPhone), userPhone);
      await applyQrReferralAttribution(String(options.guestPhone), userPhone);
    } catch (error) {
      console.error('[Auth] guest migration during session finalization failed:', error);
    }
  }
  return { token, phone: userPhone, deviceRegistered: Boolean(deviceId) && !isProvisionalEmailSubject(userPhone) };
}
...
router.post('/request-otp', authRateLimit, async (req, res) => {
  try {
    ...
    if (testStatus.active && isDevelopmentTestIdentity(phone)) return res.json({ success: true, testMode: true, message: 'Development test authentication is active. Use the displayed test code.', devCode: developmentTestOtpLabel() });
...
router.post('/request-email-otp', authRateLimit, async (req, res) => {
  try {
    ...
    res.json(result);
  } catch (e) { console.error('request-email-otp error:', e); res.status(500).json({ success: false, message: 'Failed to issue email verification code' }); }
});
...
router.post('/login', authRateLimit, async (req, res) => {
  try {
    ...
    if (authHeader?.startsWith('Bearer ')) {
      try { const token = authHeader.slice(7).trim(); const decoded = jwt.verify(token, getJwtSecret(), { algorithms: ['HS256'] }) as any; if (decoded?.phone) { await upsertProfile(decoded.phone, name, email, goal); setAuthCookie(res, token); return res.json({ success: true, phone: decoded.phone, name: name || 'Kurukoo User', token }); } } catch { /* continue */ }
    }
    if (process.env.OTP_LEGACY_LOGIN === 'true') { const userPhone = String(phone); await upsertProfile(userPhone, name, email, goal); const token = issueUserToken(userPhone); setAuthCookie(res, token); return res.json({ success: true, phone: userPhone, name: name || 'Kurukoo User', token, warning: 'Legacy login enabled — disable OTP_LEGACY_LOGIN in production' }); }
...
router.get('/identity', (_req, res) => res.json({ success: true, ...progressiveAuthReadiness() }));
router.get('/identity/me', authenticateUser, async (req: AuthRequest, res) => { try { const phone = String(req.user?.phone || ''); res.json({ success: true, identity: await assessProgressiveIdentity(phone) }); } catch (error) { res.status(500).json({ success: false, error: 'Unable to assess identity' }); } });

router.post('/request-magic-link', authRateLimit, async (req, res) => {
  try {
    const email = String(req.body?.email || '').trim();
    const guestPhone = String(req.body?.guestPhone || req.headers['x-kurukoo-guest-id'] || '').trim();
    const result = await requestMagicLink({ email, name: String(req.body?.name || '').trim() || undefined, guestPhone: guestPhone || undefined, returnPath: sanitizeReturnPath(req.body?.returnPath) });
    if (!result.success) return res.status(502).json(result);
    res.json(result);
  } catch (error) { console.error('[Auth] request-magic-link failed:', error); res.status(500).json({ success: false, message: 'Unable to request sign-in link', delivery: 'none' }); }
});

router.post('/complete-challenge', authRateLimit, async (req, res) => {
  try {
    const token = String(req.body?.token || '').trim();
    if (!token) return res.status(400).json({ success: false, message: 'Challenge token is required' });
    const consumed = await consumeAuthChallengeToken(token);
...
router.post('/request-push-approval', authRateLimit, async (req, res) => {
  try {
    const phone = String(req.body?.phone || '').trim();
    const result = await requestPushLoginApproval({ phone, name: String(req.body?.name || '').trim() || undefined, guestPhone: String(req.body?.guestPhone || '').trim() || undefined, returnPath: sanitizeReturnPath(req.body?.returnPath) });
    if (!result.success) return res.status(409).json(result);
    res.json(result);
  } catch (error) { console.error('[Auth] request-push-approval failed:', error); res.status(500).json({ success: false, message: 'Unable to request push approval', delivery: 'none' }); }
});

router.get('/logout', (_req, res) => res.redirect(302, '/chat'));
router.post('/logout', (_req, res) => { const options = { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax' as const, path: '/' }; res.clearCookie(AUTH_COOKIE, options); res.clearCookie('kurukoo_guest_id', options); res.cookie(AUTH_COOKIE, '', { ...options, maxAge: 0 }); res.cookie('kurukoo_guest_id', '', { ...options, maxAge: 0 }); res.json({ success: true }); });

router.get('/me', authenticateUser, async (req: AuthRequest, res) => { const phone = String(req.user?.phone || ''); res.json({ success: true, user: req.user, identity: await assessProgressiveIdentity(phone).catch(() => null), developmentTestAccount: isDevelopmentTestIdentity(phone), testAuth: getDevelopmentTestAuthStatus(), sessionContext: { operator: req.user?.operatorSession === true, testActor: req.user?.testActor === true, actorRole: req.user?.actorRole || null, actorContextId: req.user?.actorContextId || null, operatorPhone: req.user?.operatorPhone || null } }); });
