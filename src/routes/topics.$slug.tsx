...
export const Route = createFileRoute("/topics/$slug")({
  head: () => ({
    meta: [
      { title: "Topic — Kurukoo" },
      { name: "description", content: "Community context, moderated discussion and useful Kurukoo pathways around one subject." },
...
function TopicPage() {
  ...
  async function loadTopic(refresh = false) {
    ...
    try {
      const nextTopic = await fetchCanonicalTopic(slug);
      setTopic(nextTopic);
      setReplyStatus("");
      try {
        setRelationship(await fetchTopicRelationship(nextTopic.id));
  ...
  async function handleFollow() {
    ...
    try {
      if (relationship) {
        ...
        setRelationship(await followCanonicalTopic(topic.id));
      }
    } catch (error) {
      setReplyStatus(error instanceof Error ? error.message : "Unable to update Topic follow status");
    } finally {
  ...
  async function handleMute() {
    ...
    setRelationshipBusy(true);
    try {
      setRelationship(await setTopicNotificationPreference(topic.id, relationship.notificationPreference === "muted" ? "all" : "muted"));
    } catch (error) {
      setReplyStatus(error instanceof Error ? error.message : "Unable to update Topic notifications");
    } finally {
      setRelationshipBusy(false);
    }
  }
  ...
  async function handleReport() {
    if (!topic) return;
    const reason = window.prompt("Why does this Topic need review? For example: safety, privacy, impersonation, or misinformation.");
    if (!reason?.trim()) return;
    try {
      await reportCanonicalTopic(topic.id, reason.trim());
      setReplyStatus("Reported for review. Thank you for helping keep Topics useful.");
    } catch (error) {
      setReplyStatus(error instanceof Error ? error.message : "Unable to submit the report");
    }
  }
  ...
  if (loading) return <div className="h-96 animate-pulse rounded-[24px] border border-border bg-surface" aria-label="Loading Topic" />;

  if (!topic) {
    return <><PageHeader title="Topic" /><EmptyState title="Topic not found" body={replyStatus || "Browse public Topics to find community context."} /><Link to="/topics" className="mt-4 inline-flex items-center gap-1.5 text-[14px] underline"><ArrowLeft className="size-3.5" /> All Topics</Link></>;
  }
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <section className="mt-4 max-w-3xl">
        ...
        <p className="mt-4 text-[15.5px] leading-relaxed text-muted-foreground">Shared by {topic.authorLabel}. This is community context: it can help you understand a situation, but it does not verify providers, prices, stock, availability, booking, payment or delivery.</p>
      ...
      <section className="mt-6 rounded-[20px] border border-border bg-surface p-5 shadow-[var(--shadow-soft)]">
        <div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-elevated"><ShieldCheck className="size-4" /></span><div><p className="text-[13px] font-medium">Community context, not fulfilment proof</p><p className="mt-1 text-[12.5px] leading-relaxed text-muted-foreground">Use the discussion to learn what people are saying. Ask Kurukoo when you need evidence, current options or an actual next step.</p></div></div>
        <button type="button" onClick={continueWithKurukoo} className="mt-4 inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-primary px-3.5 text-[12.5px] font-medium text-primary-foreground hover:opacity-90">Discuss with Kurukoo <ArrowUpRight className="size-3.5" /></button>
      </section>

      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[12px] font-medium text-muted-foreground">Community discussion</p><h2 className="mt-1 text-[20px] font-semibold tracking-tight">What people are saying</h2></div><button type="button" onClick={() => void loadTopic(true)} disabled={refreshing} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-border px-3 text-[12px] text-muted-foreground hover:bg-elevated disabled:opacity-50"><RefreshCw className={refreshing ? "size-3.5 animate-spin" : "size-3.5"} /> Refresh</button></div>
        <div className="space-y-3">
          <article className="rounded-[18px] border border-border bg-surface p-5"><div className="flex items-center gap-2 text-[11.5px] text-muted-foreground"><MessageCircle className="size-3.5" /> Topic statement · {topic.authorLabel}</div><p className="mt-3 whitespace-pre-wrap text-[14.5px] leading-relaxed">{topic.body}</p><p className="mt-3 text-[11.5px] text-muted-foreground">Community statement · not independently verified</p></article>
          {topic.replies?.length ? topic.replies.map((item) => <article key={item.id} className="rounded-[18px] border border-border bg-surface p-5"><div className="flex items-center gap-2 text-[11.5px] text-muted-foreground"><MessageCircle className="size-3.5" /> {item.authorLabel}</div><p className="mt-3 whitespace-pre-wrap text-[14px] leading-relaxed">{item.body}</p><p className="mt-3 text-[11.5px] text-muted-foreground">Moderated reply · community context</p></article>) : <div className="rounded-[18px] border border-dashed border-border p-5 text-[13px] text-muted-foreground">No moderated replies yet. This stays quiet rather than being padded with synthetic activity.</div>}
        </div>
      </section>
      ...
      <div className="mt-8"><AdSlot placement="Topic feed" headline="Relevant sponsored discovery" body="Sponsored placements stay visually separate from community discussion and are not evidence of availability." advertiser="Sponsored placement" /></div>
      <IntegrationGap>Topic discussion is community context. Kurukoo's verified discovery and fulfilment layers remain the source of truth for real-world action.</IntegrationGap>
    </div>
  );
}
