...
function MessagesPage() {
  ...
  return (
    <div className="space-y-7">
      <PageHeader title="Messages" subtitle="Your conversations stay close to the work they belong to." />
        ...
        <div className="p-5 sm:p-6">
          <div className="flex items-start gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <h2 className="mt-1 text-[22px] font-semibold tracking-tight">Talk to the people doing the work.</h2>
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[12px] text-muted-foreground">
                <StatusPill>{threads.length} conversations</StatusPill>
                <StatusPill tone={unread ? "peach" : "green"}>{unread ? `${unread} unread` : "Up to date"}</StatusPill>
              </div>
            </div>
          </div>
          ...
          <div className="relative mt-5">
            ...
            <input id="message-search" type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search conversations" className="min-h-11 w-full rounded-xl border border-border bg-background pl-10 pr-4 text-[15px] outline-none placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring" />
          </div>
        </div>
      </Panel>
        ...
        {visibleThreads.length === 0 ? (
          ...
          <Rows>
            {visibleThreads.map((t) => {
              ...
              return (
                <li key={t.id}>
                  <Link to="/messages/$threadId" params={{ threadId: t.id }} className="group flex items-center gap-3 px-4 py-4 transition-colors hover:bg-elevated sm:px-5">
                    ...
                    <span className="flex shrink-0 items-center gap-1.5 text-[12px] text-muted-foreground">{t.when}<ArrowUpRight className="size-3.5 opacity-0 transition-opacity group-hover:opacity-100" /></span>
                  </Link>
                </li>
              );
            })}
      ...
      <IntegrationGap>Threads are currently prototype content. Real delivery will connect approved messaging channels and provider conversations without losing their work context.</IntegrationGap>
    </div>
  );
}
