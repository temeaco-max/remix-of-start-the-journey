...
router.post('/catalogue/sources', authenticateUser, async (req: AuthRequest, res) => {
  try {
    ...
    const source=await registerCatalogueSource({sourceType,name:req.body?.name,ownerPartyId,providerPhone:String(req.body?.providerPhone||req.user?.phone||''),sourceUrl:req.body?.sourceUrl,connected:false,verified:false});
    res.status(201).json({success:true,source,verification:'pending'});
...
router.post('/catalogue/products', authenticateUser, async (req: AuthRequest, res) => {
  try { const product=await upsertCatalogueProduct({sourceId:String(req.body?.sourceId||''),title:req.body?.title,description:req.body?.description,sku:req.body?.sku,category:req.body?.category,location:req.body?.location,price:req.body?.price,currency:req.body?.currency,available:req.body?.available,stock:req.body?.stock,sourceUrl:req.body?.sourceUrl,validUntil:req.body?.validUntil,verified:false}); res.status(201).json({success:true,product,verification:'source-controlled'}); }
...
router.get('/catalogue/products/search', authenticateUser, async (req: AuthRequest, res) => {
  try { const products=await searchCatalogueProducts({query:String(req.query.q||''),category:req.query.category?String(req.query.category):undefined,location:req.query.location?String(req.query.location):undefined,limit:Number(req.query.limit||10),verifiedOnly:req.query.verified!=='false'}); res.json({success:true,products}); }
