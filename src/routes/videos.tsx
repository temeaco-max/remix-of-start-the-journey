  ...
  head: () => ({
    meta: [
      { title: "Watch — Kurukoo" },
      {
        name: "description",
        content:
          "Useful creator videos connected to Topics and things people are trying to get done.",
      },
    ],
  }),
  ...
  return (
    <div className="space-y-7 pb-12">
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 md:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1.5 font-serif text-[28px] leading-tight">
              Learn something useful, then take the next step.
            </h2>
            <p className="mt-2 max-w-2xl text-[12.5px] leading-relaxed text-muted-foreground">
              Watch practical creator content, open its Topic or ask Kurukoo to turn what you
              learned into an action.
            </p>
          </div>
        </div>
      ...
      <section className="mt-7">
        <SectionHeader
          title="Recent videos"
          subtitle="Example creator content keeps the watch surface populated while live content inventory grows."
        />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {videos.map((video) => (
            <div key={video.id} className="relative">
              <VideoCard video={video} creatorName={entityById(video.creatorId)?.name ?? ""} />
              <span className="absolute right-3 top-3 rounded-full bg-background/90 px-2 py-1 text-[9px] uppercase tracking-[0.08em]">
                Example
              </span>
            </div>
          ))}
      ...
      <section className="mt-7 rounded-2xl border border-border bg-elevated/35 p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[14px] font-semibold">Have something worth teaching?</p>
