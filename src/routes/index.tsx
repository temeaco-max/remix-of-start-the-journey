...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Home — Kurukoo" },
...
function HomePage() {
  ...
  return <div className="space-y-6">
    <header className="flex items-start justify-between gap-4 border-b border-border pb-5">
      ...
      <div className="hidden items-center gap-2 sm:flex"><span className="rounded-full bg-elevated px-3 py-1.5 text-[12px] text-muted-foreground">☀ 23°C · Osu</span><span className="grid size-9 place-items-center rounded-full border border-border bg-surface"><BellDot /></span></div>
    </header>
    ...
    <section className="relative overflow-hidden rounded-[28px] border border-border bg-surface p-5 shadow-[var(--shadow-soft)] md:p-7">
      ...
      <div className="relative"><div className="flex items-center gap-2 text-[12px] font-medium text-muted-foreground"><Sparkles className="size-3.5" />Kurukoo is ready</div><h2 className="mt-2 max-w-2xl text-[24px] font-semibold tracking-[-0.03em] md:text-[30px]">What needs your attention?</h2><p className="mt-1 max-w-xl text-[13.5px] text-muted-foreground">Ask plainly. Kurukoo can help coordinate the next useful step.</p><div className="mt-5"><Composer onSend={send} placeholder="What needs your attention?" /></div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => send("Find me a plumber.")} className="rounded-full border border-border bg-background px-3 py-1.5 text-[12px] text-muted-foreground hover:bg-elevated">Find a plumber</button><button onClick={() => send("Book me a dentist.")} className="rounded-full border border-border bg-background px-3 py-1.5 text-[12px] text-muted-foreground hover:bg-elevated">Book a dentist</button><button onClick={() => send("What am I waiting for?")} className="rounded-full border border-border bg-background px-3 py-1.5 text-[12px] text-muted-foreground hover:bg-elevated">What am I waiting for?</button></div></div>
    </section>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section><SectionTitle title="Active requests" action="View all" /><Panel className="p-0"><div className="divide-y divide-border">{(active.length ? active : [{ id:'demo', title:'No active requests', detail:'New requests will appear here as Kurukoo works on them.', stage:'done' as const, updated:'', steps:[] }]).map((item) => <div key={item.id} className="flex items-center gap-3 px-4 py-3.5"><IconTile><Compass className="size-4" /></IconTile><div className="min-w-0 flex-1"><p className="truncate text-[13.5px] font-medium">{item.title}</p><p className="mt-0.5 truncate text-[12px] text-muted-foreground">{item.detail}</p></div>{item.stage !== 'done' && <StatusPill tone="peach">{item.stage === 'needs_you' ? 'Needs you' : 'Working'}</StatusPill>}</div>)}</div><Link to="/work" className="block border-t border-border px-4 py-2.5 text-[12px] text-muted-foreground hover:bg-elevated/60">View all requests <ArrowUpRight className="ml-1 inline size-3" /></Link></Panel></section>

      <section><SectionTitle title="Tasks & reminders" action="View all" /><Panel className="p-0"><div className="divide-y divide-border">{[['Share updated mockups','Today, 15:00',true],['Reply to Kojo','Today, 17:00',false],['Pick up groceries','Tomorrow',false]].map(([title,time,done]) => <button key={String(title)} className="flex w-full items-center gap-3 px-4 py-3.5 text-left hover:bg-elevated/50"><span className={cn("grid size-5 place-items-center rounded-full border",done ? "border-transparent bg-primary text-primary-foreground" : "border-border")}>{done && <Check className="size-3" />}</span><span className={cn("min-w-0 flex-1 text-[13.5px]",done && "text-muted-foreground line-through")}>{title}</span><span className="shrink-0 text-[11.5px] text-muted-foreground">{time}</span></button>)}</div><button className="flex w-full items-center gap-2 border-t border-border px-4 py-2.5 text-[12px] text-muted-foreground hover:bg-elevated/60"><Plus className="size-3.5" />Add reminder</button></Panel></section>
    </div>

    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <Panel className="p-4"><SectionTitle title="Opportunity radar" /><IconTile className="bg-[#edf2e9] text-[#55705a]"><Gift className="size-4" /></IconTile><p className="mt-3 text-[13.5px] font-medium">Local design meetup</p><p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">A new event near Osu matches your interests.</p><button className="mt-3 text-[12px] font-medium">Review →</button></Panel>
      <Panel className="p-4"><SectionTitle title="Points" /><p className="text-[27px] font-semibold tracking-tight">1,240</p><p className="mt-0.5 text-[12px] text-muted-foreground">+80 this week</p><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-elevated"><div className="h-full w-[68%] rounded-full bg-primary" /></div><p className="mt-2 text-[11px] text-muted-foreground">260 to next level</p></Panel>
      <Panel className="p-4"><SectionTitle title="Topics for you" /><div className="mt-1 flex flex-wrap gap-1.5"><span className="rounded-full bg-elevated px-2.5 py-1 text-[11px]">Design</span><span className="rounded-full bg-elevated px-2.5 py-1 text-[11px]">Accra</span><span className="rounded-full bg-elevated px-2.5 py-1 text-[11px]">AI</span><span className="rounded-full bg-elevated px-2.5 py-1 text-[11px]">Mobility</span></div><Link to="/explore" className="mt-4 inline-block text-[12px] font-medium">Explore topics →</Link></Panel>
      <Panel className="p-4"><SectionTitle title="Guide videos" /><div className="relative overflow-hidden rounded-xl bg-elevated"><div className="flex aspect-[16/8] items-center justify-center"><CirclePlay className="size-8 text-muted-foreground" /></div><span className="absolute bottom-2 right-2 rounded bg-background/80 px-1.5 py-0.5 text-[10px]">2:14</span></div><p className="mt-3 text-[13px] font-medium">Get more from Kurukoo</p><p className="mt-0.5 text-[11.5px] text-muted-foreground">A quick tour of handing things over.</p></Panel>
    </div>

    <section><SectionTitle title="Connected channels" action="Manage" /><Panel className="p-0"><div className="grid divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="flex items-center gap-3 p-4"><IconTile><MessageCircle className="size-4" /></IconTile><div><p className="text-[13px] font-medium">Web chat</p><StatusPill tone="green">Connected</StatusPill></div></div><div className="flex items-center gap-3 p-4"><IconTile><Mic2 className="size-4" /></IconTile><div><p className="text-[13px] font-medium">WhatsApp</p><StatusPill>Not connected</StatusPill></div></div><div className="flex items-center gap-3 p-4"><IconTile><Users className="size-4" /></IconTile><div><p className="text-[13px] font-medium">Telegram</p><StatusPill>Not connected</StatusPill></div></div></div></Panel></section>
