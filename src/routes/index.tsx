...
function IconTile({ children, className }: { children: ReactNode; className?: string }) {
  return <ContextIconTile className={className}>{children}</ContextIconTile>;
}
  ...
  return (
    <div className="flex min-w-0 items-center gap-2.5 px-3.5 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[10.5px] text-muted-foreground">{timing}</p>
      </div>
    </div>
  );
}
...
const servedCards = [
  {
    title: "People",
    body: "Ask for help, discover useful options and keep life moving.",
    to: "/chat",
    Icon: UsersRound,
  },
  {
    title: "Providers",
    body: "Offer your capabilities, stay available and receive relevant work.",
    to: "/providers",
    Icon: UsersRound,
  },
  {
    title: "Businesses",
    body: "Reach demand, manage services and coordinate customer work.",
    to: "/businesses",
    Icon: Building2,
  },
  {
    title: "Creators",
    body: "Publish useful content and build a channel around what you know.",
    to: "/creators",
    Icon: Sparkles,
  },
  {
    title: "Agents",
    body: "Give bounded agents goals and capabilities that operate through Kurukoo's governed system.",
    to: "/agents",
    Icon: Bot,
  },
  {
    title: "Contributors",
    body: "Contribute useful evidence, discovery and community work through clear tasks and rewards.",
    to: "/contributors",
    Icon: UsersRound,
  },
  {
    title: "Partners",
    body: "Connect organisations, referral paths and services to the Kurukoo network.",
    to: "/partners",
    Icon: Handshake,
  },
  {
    title: "Advertisers",
    body: "Promote useful products, services and content through clearly labelled discovery.",
    to: "/advertising",
    Icon: Megaphone,
  },
] as const;
    ...
    <section className="mt-10">
      <div className="mb-4">
        ...
        <h2 className="mt-1.5 text-[22px] font-semibold tracking-tight">
          One network, different roles.
        </h2>
        <p className="mt-1.5 max-w-2xl text-[12.5px] leading-relaxed text-muted-foreground">
          Kurukoo keeps one identity and one coordination layer underneath, while the experience
          changes around what each participant needs to do.
        </p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {servedCards.map(({ title, body, to, Icon }) => (
          <Link
            key={title}
            to={to as never}
            className="group rounded-[18px] border border-border bg-surface p-4 transition-colors hover:bg-elevated"
          >
...
export function HomePage() {
  ...
  useEffect(() => {
    void Promise.all([
      fetchProactiveFeed()
        ...
        .catch(() => {}),
      fetchAuthenticatedAd("desk-content")
        .then(setDeskAd)
        .catch(() => {}),
    ]);
  }, []);
  ...
  const flow = active.map((item) => ({
    ...
    title: item.title,
    detail: item.detail,
    icon: item.stage === "needs_you" ? Bell : Focus,
  }));
  ...
  return (
    <div className="min-w-0 space-y-5">
      <section
        ...
        className="relative overflow-hidden pb-2 pt-4 md:pt-6"
      >
        ...
        <div className="relative z-10 max-w-4xl">
          ...
          <h1
            id="home-hero-title"
            className="font-serif text-[40px] leading-[1.02] tracking-[-0.045em]"
          >
            Tell Kurukoo what needs doing
          </h1>
          <p className="mt-2 max-w-2xl text-[14px] leading-relaxed text-muted-foreground">
            Ask plainly and Kurukoo will help carry the next useful step.
          </p>
          <div className="mt-5 max-w-[920px]">
            <Composer onSend={send} placeholder="What needs your attention?" />
          </div>
        </div>
      </section>
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-2 xl:grid-cols-[minmax(0,.65fr)_minmax(0,.9fr)_minmax(0,1.08fr)]">
        <Panel className="overflow-hidden p-0">
          <CardHeader title="What needs your attention" count={active.length} />
          {active.length ? (
            ...
            <div className="px-4 py-5 text-[12px] leading-relaxed text-muted-foreground">
              Nothing is in motion right now. Start a request and Kurukoo will keep the useful state
              here.
            </div>
          )}
          <div className="border-t border-border/60 px-4 py-3">
            <SectionAction to="/work">Open Work</SectionAction>
          </div>
        </Panel>
        <Panel className="overflow-hidden p-0">
          <CardHeader title="What’s moving" />
          <div className="pb-1">
            {flow.length ? (
              flow.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex items-start gap-2.5 px-4 py-2.5">
                    <span className="grid size-7 place-items-center rounded-full bg-elevated">
              ...
              <div className="px-4 py-5 text-[12px] leading-relaxed text-muted-foreground">
                Your active requests and attention items will appear here as they become active.
              </div>
            )}
          </div>
          <div className="px-4 py-3">
            <SectionAction to="/activity">Open Activity</SectionAction>
          </div>
        </Panel>
        <Panel className="overflow-hidden p-0">
          <CardHeader title="Radar" count={opportunities.slice(0, 2).length} />
          {opportunities.slice(0, 2).length ? (
            opportunities.slice(0, 2).map((item) => <OpportunityCard key={item.id} item={item} />)
          ) : (
            <div className="px-4 py-5 text-[12px] leading-relaxed text-muted-foreground">
              Radar is quiet until Kurukoo has a supported signal worth bringing to your attention.
            </div>
          )}
          <div className="px-4 py-3">
            <SectionAction to="/discover">Open Nearby</SectionAction>
          </div>
        </Panel>
      </div>
      <div className="grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Panel className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[14px] font-semibold">Topics</h2>
            <SectionAction to="/topics">Open</SectionAction>
          </div>
          <p className="text-[12px] leading-relaxed text-muted-foreground">
            Followed and relevant community context stays separate from verified local availability.
          </p>
        </Panel>
        <Panel className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[14px] font-semibold">Wallet</h2>
            <SectionAction to="/wallet">Open</SectionAction>
          </div>
          <p className="text-[12px] leading-relaxed text-muted-foreground">
            Points and money remain separate, with balances shown from canonical account state when
            connected.
          </p>
        </Panel>
        <Panel className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[14px] font-semibold">Connected channels</h2>
            <SectionAction to="/connect">Manage</SectionAction>
          </div>
          <div className="space-y-2 text-[11.5px] text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              ...
              WhatsApp · Provider setup required
            </div>
            <div className="flex items-center gap-2">
              <Wallet className="size-3.5" />
              Account services · Readiness-aware
            </div>
          </div>
        </Panel>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[14px] font-semibold">Useful context</h2>
            <SectionAction to="/explore">Explore</SectionAction>
          </div>
          <p className="text-[12px] leading-relaxed text-muted-foreground">
            Explore the wider network when you want to look deliberately. Nearby is the local world.
            Radar is the attention layer.
          </p>
        </Panel>
        <Panel className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-[14px] font-semibold">Guide videos</h2>
            <SectionAction to="/explore">Browse</SectionAction>
          </div>
          <div className="flex items-center gap-3">
            ...
            <p className="text-[12px] text-muted-foreground">
              Learn how conversation, discovery, trust and fulfilment fit together.
            </p>
          </div>
        </Panel>
      </div>
      <p className="pb-4 text-center text-[11px] text-muted-foreground">
        Kurukoo keeps you in control. Nothing is committed without your approval.
      </p>
    </div>
  );
}
