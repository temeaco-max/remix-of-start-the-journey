...
function ActiveHomeRequest({ id, title, detail, timing, needsYou = false }: { id: string; title: string; detail: string; timing: string; needsYou?: boolean }) { return <Link to="/work/$workId" params={{ workId: id }} className="group flex min-w-0 items-center gap-2.5 px-3.5 py-2.5 hover:bg-elevated/70"><span className="grid size-7 shrink-0 place-items-center rounded-[10px] bg-brand-tint text-brand-ink">{needsYou ? <Bell className="size-3.5" /> : <ShoppingBag className="size-3.5" />}</span><div className="min-w-0 flex-1"><p className="truncate text-[13px] font-medium group-hover:text-primary">{title}</p><p className="truncate text-[11.5px] text-muted-foreground">{detail}</p><p className="mt-0.5 truncate text-[10.5px] text-muted-foreground">{timing}</p></div><ArrowUpRight className="size-3.5 shrink-0 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100" /></Link>; }
