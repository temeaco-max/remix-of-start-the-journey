  ...
  return (
    <div className="flex min-w-0 items-center gap-2.5 px-3.5 py-2.5">
      <span className="grid size-7 shrink-0 place-items-center rounded-[10px] bg-brand-tint text-brand-ink">
        {needsYou ? <Bell className="size-3.5" /> : <ShoppingBag className="size-3.5" />}
      </span>
    ...
    <section className="mt-10">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {servedCards.map(({ title, body, to, Icon }) => (
          <Link key={title} to={to as never} className="group rounded-[18px] border border-border bg-surface p-4 transition-colors hover:bg-elevated">
...
export function HomePage() {
  ...
  useEffect(() => {
    void Promise.all([
      fetchProactiveFeed().then(setOpportunities).catch(() => {}),
      fetchAuthenticatedAd("desk-content").then(setDeskAd).catch(() => {}),
    ]);
  }, []);
