...
router.post('/commercial/checkout', authenticateUser, async (req: AuthRequest, res) => {
  ...
  try{const product=await getCommercialProduct(code);if(!product||!Number(product.active))return res.status(404).json({error:'Commercial product is unavailable.'});const amountMinor=Math.floor(Number(product.price_minor||0));const currency=String(product.currency||'NGN').toUpperCase();if(amountMinor<=0)return res.status(409).json({error:'This commercial product does not have an active configured price.'});const key=`checkout:${phone}:${code}:${Date.now()}`;await recordCommercialEvent({eventType:product.product_type==='agent_subscription'?'agent_subscription':product.product_type==='sports_fee'?'sports_fee':product.product_type==='sponsorship'?'sponsorship':product.product_type==='money_circle_fee'?'money_circle_fee':'adjustment',direction:'inbound',status:'authorized',currency,grossMinor:amountMinor,platformFeeMinor:amountMinor,providerAmountMinor:0,payer:phone,payee:'KURUKOO',representedParty:phone,idempotencyKey:key,metadata:{productCode:code,productType:product.product_type,billingPeriod:product.billing_period}});const intent=await createStripePaymentIntent({amountMinor,currency,economicRequestId:`commercial:${key}`,idempotencyKey:`kurukoo:${key}`});res.json({success:true,provider:'stripe',productCode:code,paymentIntentId:intent.id,clientSecret:intent.clientSecret,amountMinor,currency,billingPeriod:product.billing_period,status:intent.status});}
...
router.post('/commercial/donations/intents', authenticateUser, async (req: AuthRequest, res) => {
  ...
  try {
    await ensureCommercialSchema(); const db=await getDb(); const stmt=db.prepare(`SELECT d.agent_id,d.owner_phone,d.skill,p.allow_donations,p.donation_recipient,d.status FROM agent_delegations d JOIN agent_permissions p ON p.delegation_id=d.id WHERE d.agent_id=? AND d.status='active'`); stmt.bind([agentId]); if(!stmt.step()){stmt.free();return res.status(404).json({error:'Donation-enabled agent not found.'});}
