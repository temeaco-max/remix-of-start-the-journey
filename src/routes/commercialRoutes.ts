...
router.get('/commercial/me', authenticateUser, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const db = await getDb(); const stmt = db.prepare(`SELECT id,event_type,status,currency,gross_minor,platform_fee_minor,provider_amount_minor,processing_fee_minor,tax_minor,net_minor,payee,represented_party,agent_id,skill,economic_request_id,external_reference,created_at,settled_at FROM commercial_ledger WHERE payer=? OR represented_party=? ORDER BY id DESC LIMIT 100`); stmt.bind([phone,phone]); const rows:any[]=[]; while(stmt.step()) rows.push(stmt.getAsObject()); stmt.free(); res.json({ success:true, events:rows });
...
router.post('/commercial/donations/intents', authenticateUser, async (req: AuthRequest, res) => {
  ...
  try {
    await ensureCommercialSchema(); const db=await getDb(); const stmt=db.prepare(`SELECT d.agent_id,d.skill,p.allow_donations,p.donation_recipient,d.status FROM agent_delegations d JOIN agent_permissions p ON p.delegation_id=d.id WHERE d.agent_id=? AND d.status='active'`); stmt.bind([agentId]); if(!stmt.step()){stmt.free();return res.status(404).json({error:'Donation-enabled agent not found.'});} const row=stmt.getAsObject() as any; stmt.free(); if(!Number(row.allow_donations)) return res.status(403).json({error:'This agent has not enabled donations/offering.'}); const recipient=String(row.donation_recipient||'').trim(); if(!recipient) return res.status(409).json({error:'Donation recipient has not been canonically configured.'});
...
router.get('/commercial/revenue/summary', authenticateUser, async (_req: AuthRequest, res) => {
...
router.post('/commercial/affiliate/conversions', authenticateUser, async (req: AuthRequest, res) => {
