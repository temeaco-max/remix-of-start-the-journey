...
import executionOverviewRoutes from './routes/executionOverviewRoutes.js';
import quickRideRoutes from './routes/quickRideRoutes.js';
...
import agentDelegationRoutes from './routes/agentDelegationRoutes.js';
...
import outcomeContextRoutes from './routes/outcomeContextRoutes.js';
...
import identityContactRoutes from './routes/identityContactRoutes.js';
import savedRoutes from './routes/savedRoutes.js';
import relationshipRoutes from './routes/relationshipRoutes.js';
import mcpAppRoutes from './routes/mcpAppRoutes.js';
import authenticatedAdvertisingRoutes from './routes/authenticatedAdvertisingRoutes.js';
...
import { closeCanonicalStore } from './services/canonicalStore.js';
import { observabilityMiddleware } from './middleware/observability.js';
import { apiV1Bridge } from './middleware/apiV1Bridge.js';
...
if (process.env.KURUKOO_EXTERNAL_AUTO_ACTIVATE === 'true' || (production && process.env.KURUKOO_EXTERNAL_AUTO_ACTIVATE !== 'false')) { void activateConfiguredExternalProviders().then(result => { for (const item of result.results) console.log(`[Kurukoo External Activation] ${item.provider}: ${item.activated ? 'active' : 'not-active'} — ${item.detail}`); }).catch(error => console.error('[Kurukoo External Activation] startup activation failed:',error)); }
...
app.use('/',systemRoutes); app.use('/',authChallengePublicRoutes); app.use('/',mcpAppRoutes); app.use('/api/v1',apiV1Bridge); app.use('/api/v1',executionOverviewRoutes);
app.use('/',pageContentRoutes); app.use('/',channelRoutes); app.use('/api',channelRoutes); app.use('/api',circleRoutes); app.use('/api/economic-requests',economicRequestRouter); app.use('/api/admin/platform',adminPlatformRoutes);
