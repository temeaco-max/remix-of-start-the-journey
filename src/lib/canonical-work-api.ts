...
export async function approveEconomicRequest(id: string) {
  const payload = await readJson<{ success: boolean; request?: EconomicRequest }>(
    `/api/economic-requests/${encodeURIComponent(id)}/transition`,
    {
      method: "POST",
...
export async function lockEconomicEscrow(id: string) {
  return readJson<{
    ...
    message?: string;
    error?: string;
  }>(`/api/economic-requests/${encodeURIComponent(id)}/escrow`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "{}",
  });
...
export async function completeEconomicRequest(id: string, evidence?: Record<string, unknown>) {
  const payload = await readJson<{ success: boolean; request?: EconomicRequest }>(
    `/api/economic-requests/${encodeURIComponent(id)}/complete`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function broadcastEconomicRequest(input: {
  requestId: string;
  skill: string;
  vehicleType?: string;
  location?: string;
  ...
  const payload = await readJson<{
    success: boolean;
    requestId: string;
    offers?: Array<{ id: string; providerPhone: string; skill: string; status: string }>;
    providers?: unknown[];
    error?: string;
  }>(`/api/economic-requests/${encodeURIComponent(input.requestId)}/dispatch/broadcast`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      skill: input.skill,
...
export async function confirmDispatchCompletion(leadId: string) {
  const payload = await readJson<{ success: boolean; lead?: { status?: string } }>(
    `/api/dispatch-leads/${encodeURIComponent(leadId)}/confirm-completion`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{}",
...
export async function submitEconomicReview(input: {
  requestId: string;
  ...
  if (!input.providerPhone.trim())
    throw new Error("A provider is required before submitting a review.");
  const payload = await readJson<{ success: boolean; review?: Record<string, unknown> }>(
    `/api/economic-requests/${encodeURIComponent(input.requestId)}/review`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
