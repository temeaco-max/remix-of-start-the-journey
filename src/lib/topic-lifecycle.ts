...
export async function fetchMyTopics(limit = 50) {
  const payload = await request<{ topics?: OwnerTopic[] }>(
    `/api/topics/mine/list?limit=${Math.min(100, Math.max(1, limit))}`,
  );
  return Array.isArray(payload.topics) ? payload.topics : [];
}

export async function fetchMyTopic(id: string) {
  const payload = await request<{ topic?: OwnerTopic }>(
    `/api/topics/mine/${encodeURIComponent(id)}`,
  );
...
export async function submitTopic(input: TopicInput) {
  const payload = await request<{ topic?: OwnerTopic }>("/api/topics", {
    ...
    headers: {
      ...
      "Idempotency-Key": input.idempotencyKey ?? randomKey(),
    },
...
export async function removeTopic(id: string) {
  return request<{ id: string; removed: boolean }>(`/api/topics/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
...
export async function fetchTopicSuggestions(title: string, category?: string) {
  ...
  const payload = await request<{ candidates?: TopicSuggestion[] }>(
    `/api/topics/suggestions?${params.toString()}`,
  );
  return Array.isArray(payload.candidates) ? payload.candidates : [];
}

export async function reportTopicReply(id: string, reason: string, detail?: string) {
  const payload = await request<{ report?: { id: string } }>(
    `/api/topics/replies/${encodeURIComponent(id)}/report`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function fetchFollowedTopics() {
  const payload = await request<{ topics?: FollowedTopic[] }>(
    "/api/relationships/topics?limit=100",
  );
  return Array.isArray(payload.topics) ? payload.topics : [];
}
...
export function typeGuidance(type: string) {
  ...
  const guidance: Record<string, string> = {
    ...
    review:
      "Describe your experience with the subject. Keep claims grounded in what you actually experienced.",
    local_report:
      "Share a local observation with broad place context. Do not post a precise address.",
    price_report:
      "Describe the price you saw, when you saw it and the broad context. Treat it as reported, not independently verified.",
    recommendation:
      "Explain what you recommend and why; other people can add their own experiences.",
    ...
    alert:
      "Use this for potentially important local information. Distinguish what you know from what you have not verified.",
    opportunity:
      "Describe the opportunity and useful context. Kurukoo can help people explore a verified next step separately.",
    experience:
      "Share what happened and what you learned, without exposing someone else's private information.",
  };
  return (
    guidance[key] ?? "Share useful community context and keep claims grounded in what you know."
  );
}
