...
export type EconomicParticipant = {
  id?: number | string;
  role?: string;
...
export type EconomicCoordination = {
  offer: EconomicOffer | null;
  participants: EconomicParticipant[];
};
...
export async function fetchTopicsAdSlots() {
  const payload = await readJson<{ slots?: Record<string, SponsoredAd[]> }>(
    "/api/advertising/topics-slots",
  );
  return payload.slots && typeof payload.slots === "object" ? payload.slots : {};
}
...
export async function fetchEconomicRequests() {
  const payload = await readJson<{ requests?: EconomicRequest[] }>(
    "/api/v1/chat/economic-requests",
  );
  return Array.isArray(payload.requests) ? payload.requests : [];
}
export async function fetchEconomicRequest(id: string) {
  const payload = await readJson<{ request?: EconomicRequest }>(
    `/api/v1/chat/economic-requests/${encodeURIComponent(id)}`,
  );
  if (!payload.request) throw new Error("Request not found");
  return payload.request;
}
export async function fetchEconomicCoordination(id: string) {
  const payload = await readJson<{
    offer?: EconomicOffer | null;
    participants?: EconomicParticipant[];
  }>(`/api/economic-requests/${encodeURIComponent(id)}/participants`);
  return {
    offer: payload.offer ?? null,
    participants: Array.isArray(payload.participants) ? payload.participants : [],
  };
}
export async function fetchEconomicExecution(id: string) {
  const payload = await readJson<{ ok?: boolean; executionRequests?: ExecutionRequest[] }>(
...
export async function fetchDeliveryCandidates(id: string) {
  const payload = await readJson<{ providers?: EconomicParticipant[] }>(
    `/api/economic-requests/${encodeURIComponent(id)}/delivery-candidates`,
  );
  return Array.isArray(payload.providers) ? payload.providers : [];
}
export async function selectDeliveryCandidate(id: string, providerPhone: string) {
  return readJson<{ success: boolean; participant?: EconomicParticipant }>(
    `/api/economic-requests/${encodeURIComponent(id)}/delivery-selection`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function submitServiceReview(
  ...
  const payload = await readJson<{ success: boolean; review?: Record<string, unknown> }>(
    `/api/economic-requests/${encodeURIComponent(requestId)}/review`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ providerPhone, rating, feedback }),
    },
  );
  if (!payload.review) throw new Error("Review was not recorded.");
  return payload.review;
}
export async function fetchNotifications() {
  return readJson<{ notifications?: Notification[] }>("/api/notifications");
}
export async function markNotificationRead(id: string | number) {
  const numericId = Number(id);
  if (!Number.isInteger(numericId) || numericId <= 0) throw new Error("Invalid notification id");
  return readJson<{ success: boolean }>(`/api/notifications/${numericId}/read`, { method: "POST" });
}
...
export async function controlAgentGoal(id: string, action: "pause" | "resume" | "cancel") {
  return readJson<{ success?: boolean; goal?: AgentGoal }>(
    `/api/agent/goals/${encodeURIComponent(id)}/${action}`,
...
export async function revokeConnectedResource(id: string) {
  return readJson<{ success: boolean; status?: string }>(
    `/api/connect/resources/${encodeURIComponent(id)}`,
    { method: "DELETE" },
  );
}
...
export async function fetchDailyPick(): Promise<DailyPick | null> {
  const payload = await readJson<{ success: boolean; pick?: DailyPick | null }>(
...
export async function fetchQuickReplies(): Promise<QuickReply[]> {
  const payload = await readJson<{ success: boolean; quickReplies?: QuickReply[] }>(
    "/api/orphan-wire-back/quick-replies",
  );
...
export async function fetchSurveyPrompt(): Promise<SurveyPrompt | null> {
  const payload = await readJson<{ success: boolean; prompt?: SurveyPrompt | null }>(
    "/api/orphan-wire-back/survey-prompt",
  );
...
export async function submitSurveyResponse(
  ...
  const payload = await readJson<{ success: boolean; result?: SurveyResponse }>(
    "/api/orphan-wire-back/survey-response",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
