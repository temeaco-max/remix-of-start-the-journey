...
export async function encryptSecret(plaintext: string, passphrase: string): Promise<EncryptedSecret> {
  ...
  if (typeof crypto === "undefined" || !crypto.subtle) throw new Error("This browser cannot encrypt a secret safely, so nothing was stored.");
...
export async function decryptSecret(ciphertext: string, iv: string, salt: string, passphrase: string): Promise<string> {
  if (typeof crypto === "undefined" || !crypto.subtle) throw new Error("This browser cannot decrypt a secret locally.");
