...
function apiUrl(path: string) {
  return `${API_BASE}/api/provider-communication${path}`;
}
...
export async function fetchProviderRequests(limit = 20) {
  const payload = await readJson<{ requests?: ProviderRequest[] }>(
    `/requests?limit=${Math.min(50, Math.max(1, limit))}`,
  );
...
export async function submitProviderQuote(
  ...
  }>(`/requests/${encodeURIComponent(id)}/quote`, {
    method: "POST",
...
export async function acceptProviderRequest(id: string) {
  return readJson<{ request: EconomicRequest; participant: EconomicParticipant; started: boolean }>(
    `/requests/${encodeURIComponent(id)}/accept`,
    { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" },
  );
}
export async function declineProviderRequest(id: string, reason?: string) {
  return readJson<{ request: EconomicRequest; participant: EconomicParticipant }>(
    `/requests/${encodeURIComponent(id)}/decline`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function submitProviderEvidence(
  id: string,
  status: string,
  evidence: Record<string, unknown>,
) {
  ...
  }>(`/requests/${encodeURIComponent(id)}/evidence`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function startProviderCommunication(id: string) {
  return readJson<{ session: { id: string; state: string; conversationId?: string } }>(
    `/requests/${encodeURIComponent(id)}/communication`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export type ProviderSessionMessage = {
  id: number;
  sender: string;
  content: string;
  created_at?: string;
...
export async function fetchProviderSessionMessages(sessionId: string) {
  const payload = await readJson<{ messages?: ProviderSessionMessage[] }>(
...
export async function sendProviderSessionMessage(sessionId: string, content: string) {
  return readJson<{ message: ProviderSessionMessage }>(
...
export async function updateProviderSessionState(sessionId: string, state: string) {
  return readJson<{ session: Record<string, unknown> }>(
