...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok || payload?.success === false) {
    throw new Error(typeof payload?.error === "string" ? payload.error : `Integration request failed (${response.status})`);
  }
  return payload as T;
}
...
export async function getIntegrationStatus(integration: Integration): Promise<IntegrationConnectionState> {
  ...
  const readPath = integration.slug === "google-drive"
    ? "/api/artifacts"
  ...
  return {
    ...
    connected: Boolean(source.connected ?? source.status === "connected" || source.status === "active"),
    status: typeof source.status === "string" ? source.status : undefined,
    reason: typeof source.reason === "string" ? source.reason : undefined,
    featureFlagState: typeof source.featureFlagState === "string" ? source.featureFlagState : undefined,
  };
}
...
export async function startIntegrationConnection(integration: Integration) {
  ...
  const payload = await request<{ authorizationUrl?: string; connected?: boolean }>(integration.endpoint.path, {
