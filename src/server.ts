...
import { registerFcmDevice } from './services/fcmDeviceRegistry.js';
const router = Router();

/**
 * @route POST /api/fcm/register
 * @desc Receive and store owner-scoped device registration tokens for Firebase Cloud Messaging.
 */
router.post('/register', authenticateUser, async (req: AuthRequest, res) => {
    ...
    try {
        ...
        const registration = await registerFcmDevice({
            ...
            credentialType: req.body?.credentialType || 'fcm',
            label: req.body?.label || 'FCM-capable device',
        });
        ...
        return res.json({
            success: true,
            message: 'Firebase Cloud Messaging device registration updated successfully.',
