...
function PersonalIdentityDock({ collapsed }: { collapsed: boolean }) {
  ...
  useEffect(() => {
    const read = () => setLoggedIn(localStorage.getItem("kurukoo-authenticated") === "true");
    read();
    window.addEventListener("storage", read);
  ...
  return <div className={cn("relative mt-auto border-t border-border pt-4", collapsed ? "px-1" : "px-2")}>
    ...
    {!collapsed && menuOpen ? <div role="menu" aria-label="Account menu" className="absolute bottom-[76px] right-2 z-50 w-48 rounded-2xl border border-border bg-surface p-1.5 shadow-[var(--shadow-lift)]">
      ...
      <Link role="menuitem" to="/subscriptions" onClick={closeMenu} className="block rounded-xl px-3 py-2 text-[12.5px] hover:bg-elevated">Subscriptions</Link>
      <Link role="menuitem" to="/settings" onClick={closeMenu} className="block rounded-xl px-3 py-2 text-[12.5px] hover:bg-elevated">Settings</Link>
      <button role="menuitem" type="button" onClick={logout} className="w-full rounded-xl px-3 py-2 text-left text-[12.5px] text-muted-foreground hover:bg-elevated hover:text-foreground">Log out</button>
    </div> : null}
    <div className={cn("flex w-full items-center rounded-xl p-2", collapsed ? "flex-col justify-center gap-1" : "gap-3")}>
      <Link to="/settings" className={cn("flex min-w-0 items-center rounded-xl text-left transition-colors hover:bg-elevated", collapsed ? "justify-center" : "flex-1 gap-3")} aria-label="Open personal profile" title={collapsed ? "Profile" : undefined}>
        <span className="relative grid size-9 shrink-0 place-items-center overflow-hidden rounded-full bg-[#ead7c7] text-[12px] font-semibold text-[#684f3e]">{initial}<span className="absolute bottom-0 right-0 size-2.5 rounded-full border-2 border-surface bg-[var(--color-success)]" /></span>
        {!collapsed && <span className="min-w-0 flex-1"><span className="block truncate text-[13.5px] font-medium">{profileName}</span></span>}
      </Link>
      {!collapsed && <button type="button" aria-haspopup="menu" aria-expanded={menuOpen} aria-label="Open profile menu" title="Profile menu" onClick={() => setMenuOpen((v) => !v)} className="grid size-9 shrink-0 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-elevated hover:text-foreground"><MoreHorizontal className="size-4" /></button>}
      {collapsed && !loggedIn ? <Link to="/login" className="text-[10.5px] font-medium text-muted-foreground hover:text-foreground">Sign In</Link> : null}
    </div>
  </div>;
}
...
function TrustedContextRail({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <aside aria-label="Trusted context rail" aria-hidden={!open} className={cn("hidden shrink-0 flex-col overflow-y-auto border-l border-[#e7e0d7] bg-[#f5f1eb] py-4 transition-[width,padding] duration-200 lg:flex dark:border-border dark:bg-background", open ? "w-[224px] px-3" : "w-[48px] px-1.5")}>
    ...
    {open ? <div className="space-y-3">
      ...
      <ContextSection title="Current work" icon={Briefcase} action="Open work" to="/work"><div className="space-y-0.5">{focus ? <ContextRow icon={Target} title={focus.title} detail={focus.stage === "needs_you" ? "Needs you" : focus.stage === "working" ? "In progress" : "Understanding"} /> : <p className="px-1.5 py-2 text-[10.5px] leading-relaxed text-muted-foreground">No active work right now.</p>}</div></ContextSection>
      <ContextSection title="Attention" icon={Bell} action="Open activity" to="/activity"><div className="space-y-0.5"><ContextRow icon={Bell} title={unread ? `${unread} unread` : "Nothing urgent"} detail={unread ? "Worth a look" : "You're caught up"} /></div></ContextSection>
      ...
      {isExplore ? <div className="rounded-xl bg-elevated/45 px-3 py-2 text-[10.5px] text-muted-foreground">Explore connects people, places, ideas and opportunities back to your active context.</div> : null}
    </div> : null}
  </aside>;
}
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background"><aside aria-label="OS navigation" className={cn("fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-border bg-surface/90 py-6 backdrop-blur md:flex transition-[width] duration-200", collapsed ? "w-[76px] px-3" : "w-[200px] px-3")}>
    <div className={cn("flex items-center pb-6", collapsed ? "justify-center" : "justify-between px-2")}>
      <Link to="/" className={cn("flex items-center", collapsed ? "justify-center" : "gap-2")} aria-label="Kurukoo Home"><span className="grid size-7 shrink-0 place-items-center rounded-[10px] bg-[#f4e6dc] text-[13px] font-semibold tracking-[-0.02em] text-[#765443]" aria-hidden>K</span>{!collapsed && <span className="text-[17px] font-semibold tracking-[-0.025em]">Kurukoo</span>}</Link>
