...
function MobileNavigation({ pathname, open, onClose }: { pathname: string; open: boolean; onClose: () => void }) {
  ...
  return <>
    ...
    <aside aria-label="Mobile navigation" className="fixed inset-y-0 left-0 z-50 flex w-[284px] flex-col border-r border-border bg-surface px-4 py-5 shadow-2xl md:hidden">
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-border bg-background/90 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden">{nav.map(({ to, label, icon: Icon }) => { const active = to === "/" ? pathname === "/" : pathname === to || pathname.startsWith(`${to}/`); return <Link key={to} to={to} aria-label={label} className={cn("flex flex-col items-center gap-1 py-2.5 text-[10px]", active ? "text-primary" : "text-muted-foreground")}><Icon className="size-[19px]" strokeWidth={active ? 2.2 : 1.8} />{label}</Link>; })}</nav>
  </div>;
}
