...
function TrustedContextSearch({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => { const onKeyDown = (event: KeyboardEvent) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") { event.preventDefault(); onOpenChange(true); window.setTimeout(() => document.getElementById("trusted-context-search")?.focus(), 0); } if (event.key === "Escape") { setQuery(""); setSubmitted(""); onOpenChange(false); } }; window.addEventListener("keydown", onKeyDown); return () => window.removeEventListener("keydown", onKeyDown); }, [onOpenChange]);
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={cn("min-h-screen bg-background md:grid", grid)}>
    <aside aria-label="OS navigation" className={cn("sticky top-0 z-40 hidden h-screen flex-col border-r border-border bg-surface/90 py-6 backdrop-blur transition-[width] duration-200 md:flex", collapsed ? "w-[76px] px-3" : "w-[200px] px-3")}>
      ...
      {!collapsed && <><p className="px-3 pb-1 pt-6 text-[10px] font-medium uppercase tracking-[0.14em] leading-4 text-muted-foreground">More</p><nav aria-label="More navigation" className="space-y-1 overflow-y-auto">{more.map(({ to, label, icon: Icon }) => to === "/network" ? <a key={to} href={to} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] leading-5 transition-colors", pathname.startsWith("/network") ? "bg-elevated font-medium text-foreground" : "text-muted-foreground hover:bg-elevated hover:text-foreground")}><Icon className="size-[18px] shrink-0" strokeWidth={1.8} />{label}</a> : <Link key={to} to={to} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] leading-5 transition-colors", pathname.startsWith(to) ? "bg-elevated font-medium text-foreground" : "text-muted-foreground hover:bg-elevated hover:text-foreground")}><Icon className="size-[18px] shrink-0" strokeWidth={1.8} />{label}</Link>)}</nav></>}
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 backdrop-blur md:hidden"><div className="mx-auto flex max-w-lg items-center justify-around px-2 py-2">{nav.slice(0, 4).map(({ to, label, icon: Icon }) => <Link key={to} to={to} className={cn("grid place-items-center rounded-xl px-3 py-2 text-[10px] leading-4", pathname === to ? "bg-elevated font-medium text-foreground" : "text-muted-foreground")}><Icon className="size-[17px]" strokeWidth={1.8} />{label}</Link>)}</div></div>
  </div>;
}
