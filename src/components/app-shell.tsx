  ...
  return (
    <div className={cn("mt-auto border-t border-border pt-4", collapsed ? "px-1" : "px-2")}>
      ...
      <Link
        to="/settings"
        className={cn(
          "flex w-full items-center rounded-xl p-2 text-left transition-colors hover:bg-elevated",
      ...
      >
        ...
        {!collapsed && (
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[13.5px] font-medium">Personal</span>
            <span className="block truncate text-[11.5px] text-muted-foreground">Identity & preferences</span>
          </span>
        )}
  ...
  return (
    <aside aria-label="Trusted context rail" className="hidden w-[292px] shrink-0 flex-col gap-3 overflow-y-auto border-l border-border bg-background/55 px-4 py-5 xl:flex">
      ...
      <ContextCard title="Current work" icon={Target} action="Open work" to="/work">
        {focus ? (
          <div>
            <p className="text-[13.5px] font-medium">{focus.title}</p>
            <p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">{focus.detail}</p>
            <div className="mt-3 flex items-center gap-2">
              <span className="size-1.5 rounded-full bg-[var(--color-success)]" />
              <span className="text-[11px] text-muted-foreground">{focus.stage === "needs_you" ? "Needs your attention" : "In progress"}</span>
            </div>
          </div>
        ) : (
          <p className="text-[13px] leading-relaxed text-muted-foreground">Nothing is currently in progress.</p>
        )}
      </ContextCard>

      <ContextCard title="Places & memory" icon={MapPin} action="View memory" to="/memory">
        <div className="space-y-3 text-[13px]">
          <div><p className="font-medium">Personal context</p><p className="text-muted-foreground">{memory.length ? `${memory.length} saved item${memory.length === 1 ? "" : "s"}` : "Nothing saved yet"}</p></div>
          <div><p className="font-medium">Location</p><p className="text-muted-foreground">Available when you choose to share it</p></div>
        </div>
      </ContextCard>

      <ContextCard title="Attention" icon={CalendarDays} action="View activity" to="/activity">
        <div className="space-y-3">
          {activeWork.length ? activeWork.map((item) => (
            <div key={item.id} className="flex gap-3">
              <span className="mt-1 size-1.5 shrink-0 rounded-full bg-primary" />
              <div><p className="text-[13px] font-medium">{item.title}</p><p className="text-[12px] text-muted-foreground">{item.updated}</p></div>
            </div>
          )) : <p className="text-[13px] text-muted-foreground">No active items need attention.</p>}
        </div>
      </ContextCard>
      ...
      <ContextCard title="Safety" icon={ShieldCheck}>
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-[13.5px] font-medium">All good</p><p className="text-[12px] text-muted-foreground">{unread ? `${unread} item${unread === 1 ? "" : "s"} waiting for you.` : "Nothing needs your attention."}</p></div>
        </div>
      </ContextCard>
    </aside>
  );
...
function NavLinks({ pathname, collapsed = false, onNavigate }: { pathname: string; collapsed?: boolean; onNavigate?: () => void }) {
  return (
    <>
      <nav aria-label="Primary OS navigation" className="space-y-1">
        {nav.map(({ to, label, icon: Icon }) => {
          const active = to === "/" ? pathname === "/" : pathname === to || pathname.startsWith(`${to}/`);
          return <Link key={to} to={to} onClick={onNavigate} title={collapsed ? label : undefined} className={cn("flex items-center rounded-xl py-2.5 text-[13.5px] transition-colors", collapsed ? "justify-center px-2" : "gap-3 px-3", active ? "bg-[#f4e6dc] font-medium text-foreground" : "text-muted-foreground hover:bg-elevated hover:text-foreground")}><Icon className="size-[18px]" strokeWidth={active ? 2.2 : 1.8} />{!collapsed && label}</Link>;
        })}
      </nav>
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <nav aria-label="Mobile primary navigation" className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-border bg-surface/95 px-2 py-2 backdrop-blur md:hidden">
      {nav.map(({ to, label, icon: Icon }) => {
        const active = to === "/" ? pathname === "/" : pathname === to || pathname.startsWith(`${to}/`);
        return <Link key={to} to={to} className={cn("flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px]", active ? "bg-[#f4e6dc] font-medium text-foreground" : "text-muted-foreground")}><Icon className="size-[18px]" strokeWidth={active ? 2.2 : 1.8} /><span>{label}</span></Link>;
      })}
    </nav>
  </div>;
}
