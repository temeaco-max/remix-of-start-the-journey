...
import { fetchCanonicalTopics, fetchSponsoredAds, type CanonicalTopic, type SponsoredAd } from "@/lib/kurukoo-api";
...
function PublicRailAd() {
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchSponsoredAds().then((campaigns) => { if (!cancelled) setAd(campaigns[0] ?? null); }).catch(() => { if (!cancelled) setAd(null); });
