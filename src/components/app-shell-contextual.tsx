  ...
  return (
    <div className="relative hidden sm:block">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-label={live ? "Live on Nearby Pulse" : "Go Live on Nearby Pulse"}
        className={cn(
          "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1.5 text-[10.5px] font-medium",
          live
            ? "border-primary/25 bg-brand-tint text-brand-ink"
            : "border-border bg-surface text-muted-foreground hover:bg-elevated",
        )}
      >
        <span
          className={cn("size-1.5 rounded-full", live ? "bg-primary" : "bg-muted-foreground/40")}
        />
        {live ? "Live" : eligible ? "Go Live" : "Nearby"}
      </button>
      {open ? (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-64 rounded-2xl border border-border bg-surface p-3 shadow-[var(--shadow-lift)]">
          <p className="text-[11px] font-semibold">
            {live ? "Your Pulse is live" : "Nearby Pulse"}
          </p>
          <p className="mt-1 text-[10.5px] leading-4 text-muted-foreground">
            {live
              ...
              : eligible
                ...
                : "Radar is available for local attention. Go Live requires provider readiness and verified availability."}
          </p>
          {live ? (
            <button
              type="button"
          ...
          <p className="mt-2 text-[9.5px] leading-4 text-muted-foreground" aria-live="polite">
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur">
      <div className="relative flex h-[50px] items-center px-5 md:px-7 lg:px-8">
        <Link to="/desk" className="flex shrink-0 items-center gap-2" aria-label="Kurukoo For You">
          <KurukooLogo />
        ...
        <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-3 md:flex">
          ...
          <form onSubmit={submit} className="hidden w-[390px] md:block">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5">
          ...
          <Link
            ...
            aria-label="Wallet"
            title="Wallet"
            className="hidden size-10 place-items-center rounded-full text-muted-foreground hover:bg-elevated sm:grid"
          >
          ...
          <Link
            ...
            className="relative grid size-10 place-items-center rounded-full text-muted-foreground hover:bg-elevated"
          >
            <Bell className="size-[19px]" />
            {unread ? (
              <span className="absolute right-[8px] top-[7px] grid min-w-2.5 place-items-center rounded-full bg-destructive px-1 text-[7px] text-destructive-foreground">
                {unread > 9 ? "9+" : unread}
              </span>
            ) : null}
          </Link>
          ...
          <div className="relative">
            ...
            {open ? (
              <div
                ...
                className="absolute right-0 top-[calc(100%+8px)] z-50 w-52 rounded-2xl border border-border bg-surface p-1.5 shadow-[var(--shadow-lift)]"
              >
                <Link
                  role="menuitem"
                  to="/memory"
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-background">
      <Header />
      <div className={cn("h-[calc(100vh-50px)] min-h-0 overflow-hidden md:grid", grid)}>
        <aside
          className={cn(
        ...
        >
          <button
            ...
            className="absolute right-[-18px] top-5 z-[80] grid size-9 place-items-center rounded-full border border-border bg-surface text-muted-foreground shadow-[var(--shadow-soft)] hover:bg-elevated"
          >
            {collapsed ? (
          ...
          <nav aria-label="OS navigation" className="scrollbar-none space-y-1 pt-5">
