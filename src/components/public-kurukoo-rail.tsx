...
export function PublicRail({
  ...
  return (
    <aside
      ...
      className={`relative z-40 hidden min-h-0 overflow-visible flex-col border-r border-border bg-surface/95 px-3 py-0 backdrop-blur md:sticky md:top-[50px] md:flex md:h-[calc(100vh-50px)] md:self-start ${collapsed ? "w-[76px]" : "w-[200px]"}`}
    >
      <button
        type="button"
        onClick={onToggle}
        ...
        className="absolute right-[-15px] top-5 z-[80] grid size-7 place-items-center rounded-full border border-border bg-surface text-muted-foreground shadow-[var(--shadow-soft)] hover:bg-elevated hover:text-foreground"
      >
        {collapsed ? (
      ...
      {!collapsed && (
        <div className="pb-4">
          ...
          <div className="mt-5">
            <ReferralCard variant="public" onPublicAuth={() => onAuth("login")} />
          </div>
        </div>
      )}
      {collapsed && (
