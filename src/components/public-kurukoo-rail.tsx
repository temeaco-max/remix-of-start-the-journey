...
export function PublicRail({ collapsed, onToggle, onAuth }: { collapsed: boolean; onToggle: () => void; onAuth: (mode: AuthMode) => void }) {
 ...
 return <aside aria-label="Public Kurukoo navigation" className={`relative z-40 hidden h-full min-h-[calc(100vh-50px)] flex-col border-r border-border bg-surface/90 px-3 py-0 backdrop-blur md:flex ${collapsed ? "w-[76px]" : "w-[200px]"}`}>
  ...
  {collapsed && <div className="mb-2 flex justify-center"><button type="button" onClick={() => { window.location.href = "/explore"; }} aria-label="Search Kurukoo" title="Search Kurukoo" className="grid size-10 place-items-center rounded-xl text-muted-foreground hover:bg-elevated hover:text-foreground"><Search className="size-[18px]" /></button></div>}
  <nav aria-label="Public primary navigation" className="space-y-1">{publicNav.map(([to,label,Icon])=><Link key={to} to={to as never} className={`flex items-center rounded-xl px-3 py-2.5 text-[13.5px] leading-5 transition-colors ${collapsed ? "justify-center px-2" : "gap-3"} ${to === "/" ? "bg-elevated font-medium text-foreground" : "text-muted-foreground hover:bg-elevated hover:text-foreground"}`} title={collapsed ? label : undefined}><Icon className="size-[18px] shrink-0" strokeWidth={1.8}/>{!collapsed&&label}</Link>)}</nav>
