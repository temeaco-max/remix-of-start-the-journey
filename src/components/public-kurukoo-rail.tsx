...
export function RotatingPublicPrompt() {
  const rotatingPrompts = userJobs.filter((job) =>
    [
      "Get a Ride",
      "Order Food",
      "Set Reminders",
  ...
  return (
    <Link
      ...
      title={prompt}
      className={`inline-flex h-7 max-w-[118px] items-center justify-center overflow-hidden rounded-full border border-border bg-background/80 px-2.5 text-[9.5px] font-medium shadow-[var(--shadow-soft)] transition-colors hover:border-primary/30 hover:bg-brand-tint/40 md:max-w-[136px] ${promptVisible ? "opacity-100" : "opacity-0"}`}
    >
...
function PublicAdvert() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-brand-tint/40 p-3.5 shadow-[var(--shadow-soft)]">
...
export function PublicRail({
  ...
  return (
    <aside
      aria-label="Public Kurukoo navigation"
      className={`relative z-40 hidden h-full min-h-0 overflow-visible flex-col border-r border-border bg-surface/95 px-3 py-0 backdrop-blur md:flex ${collapsed ? "w-[76px]" : "w-[200px]"}`}
    >
      <button
        type="button"
        onClick={onToggle}
        ...
        className="absolute right-[-18px] top-5 z-[80] grid size-9 place-items-center rounded-full border border-border bg-surface text-muted-foreground shadow-[var(--shadow-soft)] hover:bg-elevated hover:text-foreground"
      >
        {collapsed ? (
      ...
      <nav aria-label="Public primary navigation" className="space-y-1 pt-5">
      ...
      <div className="mt-auto shrink-0 px-1 pb-4 pt-4">
        {collapsed ? (
          <button
            ...
            className="flex w-full items-center justify-center rounded-xl border border-border px-2 py-2 text-[10px] font-medium leading-4 text-muted-foreground hover:bg-elevated"
            title="Log In"
          >
            Log In
          </button>
