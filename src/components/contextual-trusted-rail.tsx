...
export function ContextualTrustedRail({
    ...
    <>
      <Section title="Trusted context" icon={Brain} to="/memory">
        ...
        <Row
          icon={Briefcase}
          title="Current work"
          detail={focus?.title ?? "No active work"}
          to="/work"
        />
        <Row
          icon={Brain}
          title={memory.length ? "Active memory" : "Memory ready"}
          detail="Private continuity"
          to="/memory"
        />
      </Section>
      ...
      <Section title="Places & nearby" icon={MapPin} to="/discover">
        ...
        <Row
          icon={Zap}
          title="Radar"
          detail={live ? "Your Pulse is live" : "Local attention layer"}
          live
        />
      </Section>
  ...
  const nearbyContent = (
    <>
      ...
      <Section title="Local world" icon={Compass} to="/discover">
        <Row icon={MapPin} title="Nearby" detail="Live and stationary discovery" />
        <Row icon={Tags} title="Offers & Daily Picks" detail="Relevant commercial discovery" />
      </Section>
      <Section title="Your work" icon={Briefcase} to="/work">
        {workRow}
      </Section>
      {ad ? <RailAd campaign={ad} /> : null}
    </>
  ...
  const topicsContent = (
    <>
      <Section title="Topic context" icon={Tags} to="/topics">
        <Row icon={Tags} title="Community discussion" detail="Context, not fulfilment proof" />
        <Row icon={MapPin} title="Nearby" detail="Local businesses and activity" to="/discover" />
      </Section>
      <Section title="Opportunities" icon={Target} to="/opportunities">
        <Row icon={Sparkles} title="Ways to participate" detail="Useful network signals" />
      </Section>
      {ad ? <RailAd campaign={ad} /> : null}
    </>
  );
  const workContent = (
    <>
      ...
      <Section title="Availability" icon={MapPin} to="/discover">
        <PulseControl readiness={readiness} onChange={setReadiness} />
      </Section>
      <Section title="Messages & attention" icon={MessageSquare} to="/messages">
        <Row
          icon={Bell}
          title={unread ? `${unread} unread` : "Caught up"}
          detail="Notifications and conversations"
        />
      </Section>
      {ad ? <RailAd campaign={ad} /> : null}
    </>
  ...
  const businessContent = (
    <>
      <Section title="Your business" icon={Briefcase} to="/businesses">
        <Row icon={StoreIcon} title="Products & offers" detail="What people can discover" />
        <Row
          icon={MapPin}
          title={live ? "Live nearby" : "Availability"}
          detail={live ? "People can discover you" : "Go Live when ready"}
          to="/discover"
        />
      </Section>
      <Section title="Your work" icon={Briefcase} to="/work">
  ...
  else if (pathname.startsWith("/businesses") || pathname.startsWith("/providers"))
  ...
  return (
    <aside
      ...
      className={cn(
        "relative min-h-0 h-full hidden overflow-hidden border-l border-border bg-surface py-4 lg:flex lg:flex-col",
    ...
    >
      <button
        ...
        className="absolute left-0 top-5 z-20 grid size-9 -translate-x-1/2 place-items-center rounded-full border border-border bg-surface text-muted-foreground shadow-[var(--shadow-soft)] hover:bg-elevated"
      >
