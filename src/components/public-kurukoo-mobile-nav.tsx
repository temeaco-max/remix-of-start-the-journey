...
import { Menu, X, Compass, MessageCircle, Sparkles, Wallet, Zap } from "lucide-react";
import { useState } from "react";
import type { AuthMode } from "@/components/kurukoo/auth";

const items = [["/", "Home", Sparkles], ["/explore", "Explore", Compass], ["/topics", "Topics", MessageCircle], ["/opportunities", "Opportunities", Zap], ["/pricing", "Plans", Wallet]] as const;
...
export function PublicMobileNavigation({ onAuth }: { onAuth: (mode: AuthMode) => void }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} aria-label="Open navigation" title="Open navigation" className="fixed left-3 top-2 z-50 grid size-9 place-items-center rounded-full bg-background/80 text-foreground shadow-none backdrop-blur md:hidden">
    ...
    {open ? <>
      ...
      <aside aria-label="Mobile public navigation" className="fixed inset-y-0 left-0 z-[70] flex w-[284px] flex-col border-r border-border bg-surface px-4 py-5 shadow-2xl md:hidden">
        ...
        <nav className="space-y-1">{items.map(([to,label,Icon], index) => { const className = `flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] ${index === 0 ? "bg-[#f4e6dc] font-medium" : "text-muted-foreground hover:bg-elevated"}`; const content = <><Icon className="size-[18px]" />{label}</>; return to === "/opportunities" ? <a key={to} href={to} onClick={close} className={className}>{content}</a> : <Link key={to} to={to} onClick={close} className={className}>{content}</Link>; })}</nav>
