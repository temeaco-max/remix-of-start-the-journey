...
export function PublicKurukooShell({ children }: { children: ReactNode }) {
  const pathname = useRouterState({ select: (state) => state.location.pathname }); const navigate = useNavigate(); const routeMode: AuthMode | null = pathname === "/login" ? "login" : pathname === "/signup" ? "signup" : null; const [authMode, setAuthMode] = useState<AuthMode | null>(routeMode); const [signedIn, setSignedIn] = useState(false); const [railCollapsed, setRailCollapsed] = useState(false); useEffect(() => { setAuthMode(routeMode); }, [routeMode]); useEffect(() => { const read = () => setSignedIn(localStorage.getItem("kurukoo-authenticated") === "true"); read(); window.addEventListener("kurukoo-auth-updated", read); return () => window.removeEventListener("kurukoo-auth-updated", read); }, []); const openAuth = (mode: AuthMode) => setAuthMode(mode); const closeAuth = () => { setAuthMode(null); if (routeMode) navigate({ to: "/" }); };
