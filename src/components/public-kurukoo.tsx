...
const nav = [["/explore", "Explore"], ["/discover", "Nearby"], ["/how-it-works", "How it works"], ["/capabilities", "Capabilities"], ["/topics", "Topics"]] as const;
const footer = [["/about", "About"], ["/blog", "Blog"], ["/help", "Help"], ["/legal", "Legal"]] as const;
const routerDestinations = new Set(["/explore", "/discover", "/capabilities", "/topics", "/about", "/blog", "/help", "/login", "/signup", "/how-it-works", "/contributors", "/partners", "/pricing", "/contact", "/use-cases", "/opportunities", "/advertising", "/topics"]);
...
function PublicDestinationLink({ to, children, className, active = false }: { to: string; children: ReactNode; className: string; active?: boolean }) { if (!routerDestinations.has(to)) return <a href={to} className={className}>{children}</a>; return <Link to={to as never} activeProps={active ? { className: "text-foreground" } : undefined} className={className}>{children}</Link>; }
export function PublicKurukooShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen overflow-hidden bg-background">
    ...
    <div className={`h-[calc(100vh-50px)] min-h-0 overflow-hidden md:grid ${railCollapsed ? "md:grid-cols-[76px_minmax(0,1fr)]" : "md:grid-cols-[200px_minmax(0,1fr)]"}`}>
      ...
      <div className="min-h-0 h-full overflow-y-auto scrollbar-none"><div className="grid min-h-full lg:grid-cols-[minmax(0,1fr)_224px]"><main className="min-w-0"><div className="mx-auto w-full max-w-[1120px] px-5 py-9 md:px-8 md:py-12">{children}</div><footer className="mx-auto flex w-full max-w-[1120px] flex-wrap items-center justify-between gap-3 border-t border-border/60 px-5 py-7 text-[11px] text-muted-foreground md:px-8"><p>Kurukoo · conversation-first coordination</p><nav aria-label="Information"><div className="flex flex-wrap gap-x-4 gap-y-2">{footer.map(([to,label])=><PublicDestinationLink key={to} to={to} className="hover:text-foreground">{label}</PublicDestinationLink>)}</div></nav></footer></main><aside className="hidden min-w-0 lg:block border-l border-border/45"><PublicContextRail/></aside></div></div>
    </div>
