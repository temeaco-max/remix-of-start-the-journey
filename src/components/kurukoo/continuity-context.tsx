...
export function ContinuityContext({ requestId, status }: { requestId: string; status: string }) {
  ...
  return <section className="rounded-2xl border border-border bg-surface p-4" aria-label="Work continuity context">
    <div className="flex items-start justify-between gap-4">
      <div>
        <div className="flex items-center gap-2"><Network className="size-4 text-primary"/><h2 className="text-[15px] font-semibold">Continuous OS context</h2></div>
        <p className="mt-1 text-[11.5px] leading-5 text-muted-foreground">Memory, trust, network participants, execution state and evidence stay attached to this Work item.</p>
      </div>
      {error ? <AlertTriangle className="size-4 shrink-0 text-muted-foreground" aria-label="Some continuity sources are unavailable"/> : null}
    </div>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] text-muted-foreground">
      <Link to="/agents" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 hover:bg-elevated"><Network className="size-3"/>My Agents</Link>
      <Link to="/contacts" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 hover:bg-elevated"><Users className="size-3"/>Contacts</Link>
      <Link to="/messages" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 hover:bg-elevated"><MessageCircle className="size-3"/>Messages</Link>
      <Link to="/businesses" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 hover:bg-elevated">Businesses</Link>
      <Link to="/providers" className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1.5 hover:bg-elevated">Providers</Link>
    </div>

    <div className="mt-4 border-t border-border pt-3">
      {recoveryNeeded ? <div className="flex items-start gap-2 rounded-xl bg-elevated/60 p-3"><RefreshCw className="mt-0.5 size-3.5 shrink-0"/><div><p className="text-[11px] font-semibold">Recovery is part of the same Work item.</p><p className="mt-1 text-[10px] leading-4 text-muted-foreground">The current execution state is {executionState.replace(/[_-]/g, " ")}. Use the canonical controls below to retry, cancel or resolve the request; no success is inferred.</p></div></div> : <div className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-muted-foreground"/><p className="text-[10px] leading-4 text-muted-foreground">This request remains the source of truth. Coordination, execution, approval, evidence and recovery should return here rather than creating a separate task.</p></div>}
    </div>
  </section>;
}
