...
export function Rows({ children }: { children: ReactNode }) {
  return (
    <ul className="divide-y divide-border overflow-hidden rounded-2xl border border-border bg-surface shadow-[var(--shadow-soft)]">
      {children}
    </ul>
  );
}
...
export function StatTile({ label, value, note }: { label: string; value: string; note?: string }) {
  return (
    <div className="rounded-2xl border border-border bg-surface px-4 py-3.5 shadow-[var(--shadow-soft)]">
  ...
  return (
    <span
      className={cn(
        ...
        tone === "green"
          ? "bg-[color-mix(in_oklab,var(--color-success)_13%,transparent)] text-[var(--color-success)]"
          : tone === "peach"
...
export function ContextIconTile({
  children,
  className,
}: {
  children: ReactNode;
  ...
  return (
    <span
      className={cn(
        "grid size-8 shrink-0 place-items-center rounded-[10px] bg-elevated text-muted-foreground",
        className,
      )}
    >
      {children}
...
export function ContextualSectionAction({
  children,
  className,
}: {
  children: ReactNode;
...
export function SafetyStateIndicator({
  ...
  state?: "good" | "attention" | "action" | "blocked";
}) {
  const labels = {
    good: "Good",
    attention: "Attention",
  ...
  return (
    <span
      aria-label={`Safety state: ${labels[state]}`}
      className={cn(
  ...
  return (
    <div className="relative flex items-start gap-2.5 px-4 py-3">
      <div className="relative flex w-3 shrink-0 justify-center pt-1.5">
        <span className="z-10 size-2 shrink-0 rounded-full border-2 border-primary/60 bg-background" />
        {!isLast ? (
          <span
            aria-hidden
            className="absolute left-1/2 top-3.5 h-[calc(100%+0.75rem)] w-px -translate-x-1/2 bg-primary/20"
          />
        ) : null}
      </div>
      ...
      <div className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-full bg-elevated text-muted-foreground">
        {Icon ? <Icon className="size-3.5" /> : null}
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[13.5px] font-medium">{title}</p>
        <p className="mt-0.5 truncate text-[12.5px] text-muted-foreground">{detail}</p>
      </div>
      {kind ? (
        <span className="hidden rounded-full bg-elevated px-2 py-1 text-[10px] text-muted-foreground sm:inline">
          {kind}
        </span>
      ) : null}
    </div>
...
export function SuggestionRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      <button
        type="button"
        className="shrink-0 rounded-full border border-border px-3 py-1.5 text-[11.5px] font-medium hover:bg-elevated"
      >
        {action}
      </button>
    </div>
...
export function TrustedPersonNode({ name, state = "Online" }: { name: string; state?: string }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="relative grid size-9 place-items-center rounded-full bg-elevated text-[12px] font-medium">
        {name.slice(0, 1)}
