...
export function WorkspaceHome() {
  ...
  return <div className="min-w-0 space-y-10 pb-10">
    <section className="relative overflow-hidden border-b border-border/80 pb-8 pt-2 md:pb-10">
      ...
      <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-end">
        <div>
          ...
          <h1 className="mt-4 max-w-3xl font-serif text-[44px] leading-[.96] tracking-[-0.055em] md:text-[62px]">Your Kurukoo,<br /><span className="text-muted-foreground/75">at work.</span></h1>
          <p className="mt-5 max-w-2xl text-[14px] leading-7 text-muted-foreground">The private place where your conversations, active work, approvals and trusted context come together.</p>
        </div>
        <div className="border-l border-border/80 pl-5 lg:mb-1"><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground">Control</p><p className="mt-2 text-[13px] leading-6">Kurukoo can move things forward, but consequential actions remain yours to approve.</p><Link to="/memory" className="mt-4 inline-flex items-center gap-1 text-[11px] font-medium hover:text-primary">Review trusted context <ArrowRight className="size-3.5" /></Link></div>
      </div>
    </section>

    <section className="grid gap-px overflow-hidden border border-border/80 bg-border/80 md:grid-cols-3">
      <Link to="/chat" className="group bg-background p-5 transition-colors hover:bg-surface"><MessageCircle className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Conversation</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">Continue the relationship with your personal AI.</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">Open chat <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
      <Link to="/work" className="group bg-background p-5 transition-colors hover:bg-surface"><CheckCircle2 className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Work</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">See what is moving, waiting or complete.</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">Open Work <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
      <Link to="/memory" className="group bg-background p-5 transition-colors hover:bg-surface"><ShieldCheck className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Trusted context</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">{needsYou.length ? `${needsYou.length} item${needsYou.length === 1 ? "" : "s"} need${needsYou.length === 1 ? "s" : ""} your attention.` : "Nothing needs your attention right now."}</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">Review context <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
    </section>
    ...
    </section> : <section className="border border-border/80 px-5 py-8"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Nothing in motion</p><h2 className="mt-2 font-serif text-[27px] tracking-[-0.035em]">Tell Kurukoo what you want to move forward.</h2><Link to="/chat" className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-[11px] font-medium text-primary-foreground">Start a conversation <ArrowRight className="size-3.5" /></Link></section>}
