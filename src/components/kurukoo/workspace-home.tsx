...
export function WorkspaceHome() {
  ...
  return <div className="min-w-0 space-y-10 pb-10">
    <section className="relative overflow-hidden border-b border-border/80 pb-8 pt-2 md:pb-10">
      ...
      <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-[14px] leading-7 text-muted-foreground">A private place to see the things you have asked Kurukoo to handle, review important context, and return to the conversation.</p>
    ...
    <section className="grid gap-px overflow-hidden border border-border/80 bg-border/80 md:grid-cols-3">
      <Link to="/chat" className="group bg-background p-5 transition-colors hover:bg-surface"><MessageCircle className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Conversation</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">Your main place to tell Kurukoo what needs doing.</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">Open Chat <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
      <Link to="/work" className="group bg-background p-5 transition-colors hover:bg-surface"><CheckCircle2 className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Things being handled</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">{active.length ? `${active.length} active item${active.length === 1 ? "" : "s"}${needsYou.length ? ` · ${needsYou.length} need${needsYou.length === 1 ? "s" : ""} you` : ""}.` : "Nothing is currently in motion."}</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">See Work <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
      <Link to="/memory" className="group bg-background p-5 transition-colors hover:bg-surface"><ShieldCheck className="size-4 text-muted-foreground" /><p className="mt-9 text-[14px] font-semibold">Trusted context</p><p className="mt-1.5 text-[11.5px] leading-5 text-muted-foreground">{needsYou.length ? `${needsYou.length} item${needsYou.length === 1 ? "" : "s"} need${needsYou.length === 1 ? "s" : ""} your attention.` : "Your saved context is available when you want to review it."}</p><span className="mt-5 inline-flex items-center gap-1 text-[10.5px] font-medium">Review context <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span></Link>
    </section>
