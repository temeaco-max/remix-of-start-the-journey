...
function detectPlaceFromTimezone(timeZone: string) {
  const known: Record<string, string> = {
    "Africa/Accra": "Accra", "Africa/Lagos": "Lagos", "Africa/Abidjan": "Abidjan",
    "Africa/Nairobi": "Nairobi", "Africa/Cairo": "Cairo", "Europe/London": "London",
    "Europe/Dublin": "Dublin", "Europe/Paris": "Paris", "Europe/Berlin": "Berlin",
    "America/New_York": "New York", "America/Los_Angeles": "Los Angeles", "America/Chicago": "Chicago",
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        if (Date.now() - parsed.at < 6 * 60 * 60 * 1000) setWeather({ temperature: parsed.temperature, place: parsed.place });
    ...
    navigator.geolocation.getCurrentPosition(async ({ coords }) => {
      try {
        const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${coords.latitude}&longitude=${coords.longitude}&current=temperature_2m&timezone=auto`);
        if (!response.ok) throw new Error("Weather unavailable");
        const data = await response.json() as { current?: { temperature_2m?: number }; timezone?: string };
        if (cancelled) return;
        const temperature = typeof data.current?.temperature_2m === "number" ? String(Math.round(data.current.temperature_2m)) : "23";
        const place = detectPlaceFromTimezone(data.timezone ?? fallbackPlace);
        setWeather({ temperature, place });
        localStorage.setItem("kurukoo-location-weather", JSON.stringify({ at: Date.now(), temperature, place }));
      } catch {
...
function useLocalDateTime() {
  ...
  const date = new Intl.DateTimeFormat(locale, { weekday: "short", day: "numeric", month: "short", year: "numeric" }).format(now);
  return { greeting: getGreeting(now.getHours()), date };
}
    ...
    <header className="flex h-12 min-h-12 shrink-0 items-center justify-between gap-4 border-b border-border/70 pb-3" aria-label="Page header">
      <div className="flex min-w-0 items-center gap-2 text-[12px] text-muted-foreground">
        ...
        <span className="inline-flex min-w-0 items-center gap-1.5 truncate">
          <SunMedium className="size-4 shrink-0 text-[#c58d62]" />{weather.temperature}°C · {weather.place}
        </span>
      </div>
