...
export function FAQSection({
  ...
  useEffect(() => {
    ...
    fetch(`/api/content/page?path=${encodeURIComponent(pagePath)}`, { credentials: "same-origin" })
      .then((response) =>
        response.ok ? response.json() : Promise.reject(new Error("FAQ content unavailable")),
      )
      ...
      .catch(() => {
        if (!cancelled) setCanonicalItems(null);
      })
      .finally(() => {
  ...
  const resolvedItems = (() => {
    ...
    const managedByQuestion = new Map(
      canonicalItems.map((item) => [item.question.toLowerCase(), item]),
    );
    ...
    return [
      ...merged,
      ...canonicalItems.filter((item) => !fallbackQuestions.has(item.question.toLowerCase())),
    ];
  })();
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="max-w-2xl">
          ...
          <h2
            id="faq-heading"
            className="mt-1.5 font-serif text-[27px] leading-[1.05] tracking-[-0.035em] md:text-[31px]"
          >
            {title}
          </h2>
          {pagePath ? (
            <p className="mt-2 text-[11.5px] leading-relaxed text-muted-foreground">
              Answers are maintained in Kurukoo’s content system when managed content is available;
              built-in role answers keep the page useful when it is not.
            </p>
          ) : null}
        </div>
        {loading ? (
          <RefreshCw
            ...
            aria-label="Loading managed FAQs"
          />
        ) : null}
      </div>
      <div className="mt-5 divide-y divide-border/70 border-y border-border/70">
        {resolvedItems.map((item) => (
          <details key={item.question} className="group py-4">
            ...
            <p className="mt-2.5 max-w-3xl pr-8 text-[12.5px] leading-relaxed text-muted-foreground">
              {item.answer}
            </p>
          </details>
        ))}
