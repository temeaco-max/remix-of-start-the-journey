  ...
  return (
    <Panel className="overflow-hidden p-0">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {SKILL_OPTIONS.map((option) => (
              <button
                ...
                className="rounded-xl border border-border bg-surface px-3 py-2.5 text-left text-[12.5px] text-foreground hover:border-primary/30 hover:bg-elevated"
              >
                {option.label}
              </button>
            ))}
      ...
      {step === "details" ? (
        <div className="px-4 py-4">
          <p className="text-[11.5px] text-muted-foreground">
            Give Kurukoo the booking manager details so the artist can be verified.
          </p>
          <div className="mt-3 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Full name of the booking manager"
                className="mt-0.5 w-full rounded-xl border border-border bg-surface px-3 py-2 text-[13px] outline-none placeholder:text-muted-foreground"
              />
            </div>
            <div>
              <label className="text-[11.5px] font-medium text-muted-foreground">
              ...
              <div className="mt-0.5 flex gap-2">
                ...
                <input
                  ...
                  placeholder={
                    contactType === "phone" ? "+234 801 234 5678" : "manager@example.com"
                  }
                  className="min-w-0 flex-1 rounded-xl border border-border bg-surface px-3 py-2 text-[13px] outline-none placeholder:text-muted-foreground"
                />
              </div>
            </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <button
              ...
              disabled={!managerName.trim() || !managerContact.trim() || submitting}
      ...
      {step === "done" ? (
        <div className="px-4 py-4">
          <div className="flex items-start gap-3 rounded-xl border border-border bg-surface p-4">
            <Check className="mt-0.5 size-5 shrink-0 text-[var(--color-success)]" />
            <div className="min-w-0 flex-1">
              <p className="text-[13.5px] font-medium">Verification request submitted</p>
              <p className="mt-1 text-[11.5px] text-muted-foreground">
                Kurukoo has received the request for{" "}
              ...
              <Rows>
                <li className="flex items-center justify-between gap-4 px-4 py-2 text-left">
                  <span className="text-[10.5px] text-muted-foreground">Booking manager</span>
                  <span className="text-[12px]">{managerName}</span>
                </li>
                <li className="flex items-center justify-between gap-4 px-4 py-2 text-left">
                  <span className="text-[10.5px] text-muted-foreground">Contact</span>
                  <span className="max-w-[160px] truncate text-[12px]">{managerContact}</span>
                </li>
                <li className="flex items-center justify-between gap-4 px-4 py-2 text-left">
                  <span className="text-[10.5px] text-muted-foreground">Status</span>
                  <span className="text-[12px] text-accent">Pending verification</span>
                </li>
              </Rows>
            </div>
          </div>
