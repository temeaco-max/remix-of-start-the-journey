...
async function readProfileName() {
  try {
    ...
    if (response.ok) {
      const payload = (await response.json()) as { profile?: { name?: string; displayName?: string; firstName?: string } };
      const name = payload.profile?.name || payload.profile?.displayName || payload.profile?.firstName;
...
async function readWeather(): Promise<WeatherState> {
  ...
  try {
    const position = await new Promise<GeolocationPosition>((resolve, reject) => navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: false, timeout: 6000, maximumAge: 900000 }));
    const { latitude, longitude } = position.coords;
    const response = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(latitude)}&longitude=${encodeURIComponent(longitude)}&current=temperature_2m,weather_code&timezone=auto`, { headers: { Accept: "application/json" } });
...
export function FieldDashboard() {
  ...
  return <div className="kurukoo-field min-w-0 pb-10">
    ...
    <section className="field-main-grid" aria-label="Your day">
      ...
      <div className="field-side-stack"><FieldCard className="field-agent-card"><CardHeading eyebrow="With a little help" title="Agent-assisted next steps" action={<span className="field-count">2</span>} /><Link to="/work" className="field-agent-row"><span className="field-agent-icon"><Sparkles className="size-4" /></span><span><strong>Prepare for client call</strong><small>Continue this work</small></span><ArrowRight className="size-3.5" /></Link><Link to="/work" className="field-agent-row"><span className="field-agent-icon field-agent-icon-green"><CheckCircle2 className="size-4" /></span><span><strong>Grocery top-up</strong><small>Continue this work</small></span><ArrowRight className="size-3.5" /></Link><Link to="/agents" className="field-see-all">See all suggestions <ArrowRight className="size-3.5" /></Link></FieldCard><FieldCard className="field-people-card"><CardHeading title="Trusted people" action={<Link to="/connect" className="field-card-action">Manage <ArrowRight className="size-3.5" /></Link>} /><div className="field-people-list">{trustedPeople.map((person) => <Link key={person.id} to="/profile/$entityId" params={{ entityId: person.id }} className="field-person-row"><span className="field-person field-person-a" aria-hidden="true">{person.name.slice(0, 1)}</span><span><strong>{person.name}</strong><small>{person.tagline}</small></span></Link>)}</div></FieldCard></div>
    </section>

    <section className="field-topics-card"><CardHeading eyebrow="Your Topics" title="What is useful around you" action={<Link to="/topics" className="field-card-action">Open Topics <ArrowRight className="size-3.5" /></Link>} /><div className="field-tabs" role="tablist" aria-label="Topic view"><button type="button" role="tab" aria-selected={topicTab === "trending"} onClick={() => setTopicTab("trending")}>Trending</button><button type="button" role="tab" aria-selected={topicTab === "new"} onClick={() => setTopicTab("new")}>New</button></div><div className="field-topic-list">{visibleTopics.map((topic) => <Link key={topic.slug} to="/topics/$slug" params={{ slug: topic.slug }} className="field-topic-row"><span><strong>{topic.name}</strong><small>{topic.blurb}</small></span><span className="field-topic-meta">{topic.followers.toLocaleString()} following <ArrowRight className="size-3.5" /></span></Link>)}</div></section>

    <section className="field-creators-card"><CardHeading eyebrow="From creators" title="Useful things to watch" action={<Link to="/videos" className="field-card-action">See all videos <ArrowRight className="size-3.5" /></Link>} /><div className="field-video-scroller">{videos.map((video) => { const creator = entityById(video.creatorId); return <Link key={video.id} to="/videos/$videoId" params={{ videoId: video.id }} className="field-video-card"><span className="field-video-thumb"><Play className="size-5" /><small>{video.duration}</small></span><span className="field-video-copy"><strong>{video.title}</strong><small>{creator?.name ?? "Kurukoo creator"} · {video.views} views</small></span></Link>; })}</div></section>
