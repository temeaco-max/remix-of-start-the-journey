...
export async function listFcmDevices(phone: string): Promise<FcmDevice[]> {
  ...
  while (stmt.step()) {
    ...
    rows.push({
      ...
      lastAcceptedAt: row.last_accepted_at ? String(row.last_accepted_at) : undefined,
...
export async function deactivateFcmDevice(phone: string, deviceId: string, error = 'device_revoked'): Promise<boolean> {
  const db = await ensureTable();
  const normalizedPhone = String(phone || '').trim();
  const normalizedDevice = String(deviceId || '').trim();
  if (!normalizedPhone || !normalizedDevice) return false;
  db.run(`UPDATE fcm_devices SET active=0, last_error=?, updated_at=CURRENT_TIMESTAMP WHERE phone=? AND device_id=? AND active=1`, [String(error).slice(0, 240), normalizedPhone, normalizedDevice]);
  const changed = db.getRowsModified() > 0;
  if (changed) {
    await syncLegacyProfileToken(db, normalizedPhone);
    saveDb();
  }
  return changed;
}
