...
export interface ObjectiveSimulationOptions {
  budget?: AgentExecutionBudget;
}

export function simulateCompoundObjective(
  options: ObjectiveSimulationOptions = {},
): ObjectiveSimulationResult {
  ...
  const parent: SimulatedGoal = {
    id: 'sim-parent',
    objective: 'Fix my laptop and sell it when it is ready',
    status: 'ready',
  };
  const repair: SimulatedGoal = {
    id: 'sim-repair',
    objective: 'Repair the laptop',
    status: 'ready',
  };
  const sale: SimulatedGoal = {
    id: 'sim-sale',
    objective: 'Sell the laptop after repair completion',
    dependsOn: repair.id,
