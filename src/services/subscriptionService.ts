...
export async function upgradeSubscriptionAfterPayment(phone:string,plan:string,country:string,options?:{payment_ref?:string;force_sandbox_grant?:boolean}):Promise<SubscriptionResult>{
  if(!phone||!plan||!country)return{success:false,message:'phone, plan, and country are required',error:'missing_params'};
  const planObj=await getPlan(country,plan);if(!planObj||!(planObj as any).active)return{success:false,message:'Pricing plan not found or inactive',error:'plan_not_found'};
  const amountMinor=Number((planObj as any).monthly_price_minor)||0;const currency=String((planObj as any).currency||'NGN').toUpperCase();const provider=process.env.KURUKOO_PAY_PROVIDER||'sandbox';let paid=false;
  if(amountMinor<=0)paid=true;else if(provider==='sandbox'&&process.env.FORCE_SANDBOX_SUBSCRIPTION==='true')paid=true;else paid=await processDirectPayment(phone,'SYSTEM',amountMinor/100);
  if(!paid)return{success:false,message:'Payment not confirmed. Complete checkout with a verified provider before retrying.',error:'payment_required',payment_required:true,plan:planObj};
  const tier=normalizeTierName(plan);const db=await getDb();db.run(`UPDATE memory_profiles SET subscription_tier=?,updated_at=CURRENT_TIMESTAMP WHERE phone=?`,[tier,phone]);saveDb();
  await recordCommercialEvent({eventType:'subscription_charge',direction:'inbound',status:'settled',currency,grossMinor:amountMinor,platformFeeMinor:amountMinor,providerAmountMinor:0,payer:phone,payee:'KURUKOO',representedParty:phone,externalReference:options?.payment_ref,idempotencyKey:`subscription:${phone}:${country}:${plan}:${options?.payment_ref||new Date().toISOString().slice(0,7)}`,metadata:{plan,billingPeriod:'monthly',paymentProvider:provider}});
  const nextBillingDate=new Date();nextBillingDate.setMonth(nextBillingDate.getMonth()+1);await upsertSubscriptionBilling({ownerPhone:phone,productCode:`consumer:${country}:${plan}`,tier,priceMinor:amountMinor,currency,nextBillingDate});
