...
export function chooseInferenceProvider(input:{task:InferenceTask;prompt:string;preferred?:AIProvider}):InferenceDecision{
  ...
  if(lowSignals.some(signalText=>text===signalText||text.startsWith(`${signalText} `))&&input.task!=='high_stakes')return{provider:'smollm2',reason:'low-complexity conversational/support turn',maxComplexity:'low'};
