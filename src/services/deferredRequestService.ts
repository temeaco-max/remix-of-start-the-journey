...
export async function createOpenIntention(
    ...
    db.run(`
        INSERT INTO open_intentions
          (phone, intent, context, skill, location, status, ttl_days, max_attempts, next_check_at, expires_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 'requested', ?, ?, ?, ?, ?, ?)
    `, [phone, intent, context, options.skill || null, options.location || null, ttlDays, maxAttempts, nextCheck.toISOString(), expires.toISOString(), now.toISOString(), now.toISOString()]);
...
export async function incrementAttempt(phone: string, intentionId: string | number) {
    await initDeferredTables();
    const db = await getDb();
    db.run(`UPDATE open_intentions SET attempts = attempts + 1, deferred_attempts = deferred_attempts + 1, next_check_at = ?, updated_at = ? WHERE phone = ? AND id = ?`, [new Date(Date.now() + DEFAULT_RECHECK_HOURS * 60 * 60 * 1000).toISOString(), new Date().toISOString(), phone, Number(intentionId)]);
    saveDb();
    return getIntentionById(phone, intentionId);
}
