...
import { interpretConversationSemantics, type SemanticConversationInterpretation } from './semanticConversationInterpreter.js';

export type ConversationGenerationMode = 'generate' | 'present' | 'deterministic';
export interface ConversationalGenerationInput {
  owner?: 'conversationalGenerationService'; prompt: string; phone?: string; agentId?: string; threadId?: string; provider?: AIProvider; systemPrompt?: string; contextHint?: ConversationalContextHint; activeContextIds?: string[]; activeGoals?: string[]; knownFacts?: string[]; pendingFields?: string[]; pausedGoals?: string[]; currentGoal?: string; priorAssistantReplies?: string[]; canonicalAction?: string; cardType?: string; generationMode?: ConversationGenerationMode; seedResponse?: AIResponse; canonicalOutcomeGuidance?: CapabilityConversationGuidance; canonicalOutcome?: CanonicalOutcomeLike;
}
export interface ConversationalGenerationResult extends AIResponse { contract: ConversationTurnContract; quality: ConversationQualityAssessment; escalated: boolean; attemptCount: number; contextTurns: number; generationMode: ConversationGenerationMode; semanticInterpretation: SemanticConversationInterpretation; }
...
function needsCanonicalRelevanceFallback(prompt: string, responseText: string): boolean {
  const user = String(prompt || '').toLowerCase();
  const reply = String(responseText || '').toLowerCase();
  return /\bwhat should i know before using kurukoo\b/.test(user)
...
export async function generateConversationalResponse(input: ConversationalGenerationInput): Promise<ConversationalGenerationResult> {
  ...
  const semanticInterpretation = await interpretConversationSemantics({ message: input.prompt, phone: input.phone, threadId: input.threadId, provider: input.provider || 'auto', contextHint: input.contextHint, activeGoals: input.activeGoals, pausedGoals: input.pausedGoals, knownFacts: input.knownFacts, pendingFields: input.pendingFields });
  const contract = buildConversationTurnContract({ latestUserMessage: input.prompt, assistantReply: generationMode === 'present' || generationMode === 'deterministic' ? input.seedResponse?.text || '' : '', userMessage: input.prompt, activeContextIds: input.activeContextIds, activeGoals: input.activeGoals, knownFacts: input.knownFacts, pendingFields: input.pendingFields, pausedGoals: input.pausedGoals, currentGoal: input.currentGoal, priorAssistantReplies: input.priorAssistantReplies, semanticInterpretation });
  ...
  const skillPack = resolveRuntimeSkillBehaviour(input.prompt, [...(input.knownFacts || []), ...(input.pendingFields || []), input.currentGoal || '', semanticInterpretation.intent]);
  const skillInstruction = skillPack ? await buildRuntimeSkillInstruction(input.phone, input.prompt, [...(input.knownFacts || []), ...(input.pendingFields || []), input.currentGoal || '', semanticInterpretation.intent]) : '';
  const semanticGuidance = [`--- LLM semantic interpretation (primary conversational understanding; never reveal) ---`,`mode=${semanticInterpretation.mode}`,`speech_act=${semanticInterpretation.speechAct}`,`intent=${semanticInterpretation.intent}`,semanticInterpretation.skillHint ? `skill_hint=${semanticInterpretation.skillHint}` : '',semanticInterpretation.capabilityHint ? `capability_hint=${semanticInterpretation.capabilityHint}` : '',`explicit_authorization=${semanticInterpretation.explicitAuthorization}`,`requires_clarification=${semanticInterpretation.requiresClarification}`,semanticInterpretation.references.length ? `references=${JSON.stringify(semanticInterpretation.references)}` : '',semanticInterpretation.missingInformation.length ? `missing_information=${semanticInterpretation.missingInformation.join(', ')}` : '',semanticInterpretation.goalStatements.length ? `goal_statements=${semanticInterpretation.goalStatements.join(' | ')}` : '',semanticInterpretation.topicShift ? 'topic_shift=true' : '',semanticInterpretation.resumption ? 'resumption=true' : ''].filter(Boolean).join('\n');
  ...
  return { ...response, contract, quality: assessment, escalated, attemptCount: attempts, contextTurns: contextPack.turns, generationMode, semanticInterpretation };
}
