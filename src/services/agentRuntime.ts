...
function goalCapabilityPlan(skill: string): string[] { return resolveSkillCapabilityPlan(skill); }
function supportsAgentGoal(skill: string): boolean { return skill === 'reminder' || goalCapabilityPlan(skill).length > 0; }
function mapCapabilityRisk(skill: string): AgentRiskLevel {
  ...
  const candidates = composition.ordered.filter(item => item.descriptor.capability !== `skill.${String(skill || '').trim().toLowerCase()}`);
  const risk = registration?.descriptor.risk || candidates.find(item => item.descriptor.risk)?.descriptor.risk || 'low_risk';
  if (risk === 'read_only') return 'read_only';
  if (risk === 'high_risk') return 'high_risk';
  if (risk === 'confirmation_required') return 'user_confirmation_required';
  return 'reversible';
}

function capabilityPlanDependencies(skill: string): string[] { return resolveSkillCapabilityComposition(skill).ordered.map(item => item.descriptor.capability); }
...
export async function notifyGoalIfNeeded(goal: AgentGoal): Promise<void> { if (!['needs_user', 'blocked', 'completed'].includes(goal.status)) return; const message = goal.summary || 'Kurukoo has an update on your agent goal.'; if (!(await autonomousNotificationsAllowed(goal.phone))) return; const key = `goal:${goal.id}:notify:${goal.status}:${goal.updatedAt}`; if (await notificationAlreadyQueued(goal, key)) return; try { await sendFcmPush(goal.phone, 'Kurukoo update', message, goal.conversationId ? `/chat?conversationId=${encodeURIComponent(goal.conversationId)}&contextId=${encodeURIComponent(`goal:${goal.id}`)}&action=resume` : undefined, { contextId: `goal:${goal.id}`, conversationId: goal.conversationId, availableAction: goal.status === 'completed' ? 'review' : 'resume', canonicalAction: goal.status === 'completed' ? 'agent.goal.review' : 'agent.goal.resume', objectType: 'agent_goal', objectId: String(goal.id), ownerScope: goal.phone, idempotencyKey: key, surface: 'chat' }); } catch {} await recordEvent(goal, 'notification_queued', 'success', message, { idempotencyKey: key }); }
...
function buildGoalPlan(objective: string, skill: string, requestId?: string): AgentPlan {
  const composition = resolveSkillCapabilityComposition(skill);
  const dependencies = capabilityPlanDependencies(skill);
  const riskLevel = mapCapabilityRisk(skill);
  const confirmationRequired = Boolean(requestId) || riskLevel === 'user_confirmation_required' || riskLevel === 'high_risk';
  const capabilityStep: AgentPlanStep = {
    id: 'inspect_capability_plan',
    action: `Inspect the canonical capability plan for ${skill}`,
    tool: 'inspect_capability_plan',
    risk: 'read_only',
    status: composition.unresolved.length || composition.cycle?.length ? 'blocked' : 'pending',
    authorization: 'canonical_capability_read',
    idempotencyKey: `inspect:capability:${skill}`,
  };
  const steps: AgentPlanStep[] = [capabilityStep];
  if (requestId) {
    steps.push({ id: 'inspect_request', action: 'Check the current request state', tool: 'get_request_state', risk: 'read_only', status: 'pending', authorization: 'owned_request_read', idempotencyKey: `inspect:${requestId}` });
    steps.push({ id: 'await_confirmation', action: 'Wait for your approval before any consequential action', tool: 'get_request_state', risk: 'user_confirmation_required', status: 'pending', authorization: 'existing_request_confirmation', idempotencyKey: `confirm:${requestId}` });
  } else if (riskLevel === 'read_only') {
    steps.push({ id: 'observe_context', action: `Observe the current ${skill} context using the registered capability tools`, tool: 'list_capabilities', risk: 'read_only', status: 'pending', authorization: 'canonical_capability_read', idempotencyKey: `observe:${skill}:${objective.slice(0,80)}` });
  } else {
    steps.push({ id: 'prepare_action', action: `Prepare the next ${skill} action through the canonical executor`, tool: 'execute_capability', risk: riskLevel, status: 'pending', authorization: 'canonical_executor_policy', idempotencyKey: `prepare:${skill}:${objective.slice(0,80)}` });
  }
  return {
    objective: objective.slice(0, 1000),
    currentStep: 0,
    status: composition.unresolved.length || composition.cycle?.length ? 'blocked' : requestId || riskLevel === 'read_only' ? 'active' : 'needs_user',
    requiredInputs: requestId ? [] : riskLevel === 'read_only' ? [] : ['action details or explicit confirmation'],
    dependencies,
...
export async function createConversationGoal(input: { phone: string; conversationId?: string; skill: string; objective: string; economicRequestId?: string; source?: AgentGoalSource; persistWhenDisabled?: boolean }): Promise<AgentGoal | null> { if ((!enabled() && !input.persistWhenDisabled) || !input.phone || input.phone.startsWith('anon_') || !supportsAgentGoal(input.skill)) return null; await ensureAgentRuntimeSchema();   const db = await getDb(); const existing = db.prepare(`SELECT * FROM agent_goals WHERE phone=? AND conversation_id IS ? AND goal_type=? AND status IN ('active','waiting','needs_user','blocked') ORDER BY updated_at DESC LIMIT 1`); existing.bind([input.phone, input.conversationId || null, input.skill]); const prior = existing.step() ? rowToGoal(existing.getAsObject()) : null; existing.free(); if (prior) return prior; const countStmt = db.prepare(`SELECT COUNT(*) AS count FROM agent_goals WHERE phone=? AND status IN ('active','waiting','needs_user','blocked')`); countStmt.bind([input.phone]); const activeCount = countStmt.step() ? Number(countStmt.getAsObject().count || 0) : 0; countStmt.free(); if (activeCount >= maxConcurrent()) return null; const autonomy: AgentAutonomyLevel = input.economicRequestId ? 'act_with_confirmation' : 'assist'; const plan = buildGoalPlan(input.objective, input.skill, input.economicRequestId); const initialStatus = plan.status; const goal: AgentGoal = { id: crypto.randomUUID(), phone: input.phone, conversationId: input.conversationId, economicRequestId: input.economicRequestId, source: input.source || 'conversation', goalType: input.skill, objective: input.objective.slice(0, 1000), status: initialStatus, priority: 50, autonomy, plan, nextActionAt: initialStatus === 'active' ? nextTime() : undefined, summary: input.economicRequestId ? 'Kurukoo is checking the existing request and its canonical capability plan.' : initialStatus === 'blocked' ? 'Kurukoo cannot continue this goal because its capability composition is invalid.' : initialStatus === 'active' ? `Kurukoo has a canonical plan for ${input.skill} and will continue from the exact goal context.` : 'Kurukoo needs explicit action details or permission before it can continue.', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; db.run(`INSERT INTO agent_goals (id, phone, conversation_id, economic_request_id, source, goal_type, objective, status, priority, autonomy, next_action_at, summary, plan_json, risk_level, confirmation_required, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [goal.id, goal.phone, goal.conversationId || null, goal.economicRequestId || null, goal.source, goal.goalType, goal.objective, goal.status, goal.priority, goal.autonomy, goal.nextActionAt || null, goal.summary || null, JSON.stringify(plan), plan.riskLevel, plan.confirmationRequired ? 1 : 0, plan.expiresAt]); saveDb(); await recordEvent(goal, 'goal_created', goal.status === 'needs_user' ? 'needs_user' : goal.status === 'blocked' ? 'blocked' : 'success', goal.summary || 'Goal created.', { idempotencyKey: `goal:create:${goal.phone}:${goal.conversationId || 'none'}:${goal.goalType}` }); return goal; }
...
async function evaluateGoal(goal: AgentGoal): Promise<AgentGoal> {
  if (goal.economicRequestId) {
    const result = await internalCoordinator.handle(coordinatorEventForAgentGoal({ ownerPhone: goal.phone, agentGoalId: goal.id, economicRequestId: goal.economicRequestId, conversationId: goal.conversationId }));
    if (!result.ok) { const updated = await updateGoal(goal, { status: 'blocked', summary: result.message || 'The request could not be verified.', failureReason: result.message }); await recordEvent(updated, 'inspect_request', 'blocked', updated.summary || '', { tool: result.tool, evidence: result.evidence?.source, idempotencyKey: `goal:${goal.id}:inspect:${goal.updatedAt}` }); return updated; }
    ...
    await recordEvent(updated, 'inspect_request', transition.event, transition.summary, { tool: result.tool || 'get_request_state', evidence: result.evidence?.source, idempotencyKey: `goal:${goal.id}:request:${String(request.status)}` });
    return updated;
  }
  const planResult = await executeAgentTool('inspect_capability_plan', { skill: goal.goalType }, { phone: goal.phone, conversationId: goal.conversationId, goalId: goal.id });
  if (!planResult.ok) { const updated = await updateGoal(goal, { status: 'blocked', summary: planResult.message || 'The capability plan could not be resolved.', failureReason: planResult.message }); await recordEvent(updated, 'inspect_capability_plan', 'blocked', updated.summary || '', { tool: planResult.tool, evidence: planResult.evidence, idempotencyKey: `goal:${goal.id}:capability_plan:${goal.updatedAt}` }); return updated; }
  const plan = Array.isArray(planResult.data?.capabilities) ? planResult.data?.capabilities as string[] : [];
  if (goal.plan.riskLevel === 'read_only') {
    const updated = await updateGoal(goal, { status: 'waiting', summary: `The ${goal.goalType} goal is active. Kurukoo resolved ${plan.length} canonical capability steps and will continue from this exact context.`, nextActionAt: nextTime() });
    await recordEvent(updated, 'inspect_capability_plan', 'waiting', updated.summary || '', { tool: planResult.tool, evidence: planResult.evidence, idempotencyKey: `goal:${goal.id}:capability_plan:${plan.join(',')}` });
    return updated;
  }
  const updated = await updateGoal(goal, { status: 'needs_user', summary: `Kurukoo resolved the ${goal.goalType} capability path, but the next consequential action requires explicit details or confirmation.` });
  await recordEvent(updated, 'inspect_capability_plan', 'needs_user', updated.summary || '', { tool: planResult.tool, evidence: planResult.evidence, idempotencyKey: `goal:${goal.id}:capability_plan:${plan.join(',')}` });
  return updated;
}
...
export async function resumeAgentGoal(phone: string, goalId: string): Promise<AgentGoal | null> { const goal = await getAgentGoal(phone, goalId); if (!goal || ['completed', 'cancelled', 'failed', 'expired'].includes(goal.status)) return goal; const updated = await updateGoal(goal, { status: goal.economicRequestId ? 'active' : goal.plan.riskLevel === 'read_only' ? 'active' : 'needs_user', summary: goal.economicRequestId ? 'Resumed. Kurukoo will re-check the existing request through the bounded runtime.' : goal.plan.riskLevel === 'read_only' ? 'Resumed. Kurukoo will continue observing the registered capability path.' : 'Resumed, but the next consequential action still requires explicit permission.', nextActionAt: goal.economicRequestId || goal.plan.riskLevel === 'read_only' ? nextTime() : undefined }); await recordEvent(updated, 'resumed_by_user', 'success', updated.summary || '', { idempotencyKey: `goal:${goalId}:resume:${updated.updatedAt}` }); return updated; }
