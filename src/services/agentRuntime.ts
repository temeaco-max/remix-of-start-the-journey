...
async function evaluateGoal(goal: AgentGoal): Promise<AgentGoal> { if (!goal.economicRequestId) return updateGoal(goal, { status: 'needs_user', summary: 'Kurukoo needs a few details before it can continue.' }); const result = await executeAgentTool('get_request_state', { requestId: goal.economicRequestId }, { phone: goal.phone, conversationId: goal.conversationId, goalId: goal.id }); if (!result.ok) { const updated = await updateGoal(goal, { status: 'blocked', summary: result.message || 'The request could not be verified.', failureReason: result.message }); await recordEvent(updated, 'inspect_request', 'blocked', updated.summary || '', { tool: result.tool, evidence: result.evidence, idempotencyKey: `goal:${goal.id}:inspect:${goal.updatedAt}` }); return updated; } const request = result.data || {}; const transition = messageForRequest(request); const updated = await updateGoal(goal, { status: transition.status, summary: transition.summary, nextActionAt: transition.nextActionAt, completedAt: transition.status === 'completed' ? new Date().toISOString() : undefined }); await recordEvent(updated, 'inspect_request', transition.event, transition.summary, { tool: result.tool, evidence: result.evidence, idempotencyKey: `goal:${goal.id}:request:${String(request.status)}` }); return updated; }
...
export async function notifyGoalIfNeeded(goal: AgentGoal): Promise<void> { if (!['needs_user', 'blocked', 'completed'].includes(goal.status)) return; const message = goal.summary || 'Kurukoo has an update on your agent goal.'; if (!(await autonomousNotificationsAllowed(goal.phone))) return; const key = `goal:${goal.id}:notify:${goal.status}:${goal.updatedAt}`; if (await notificationAlreadyQueued(goal, key)) return; try { await sendFcmPush(goal.phone, { title: 'Kurukoo update', body: message, data: { type: 'agent_goal', goalId: goal.id, conversationId: goal.conversationId || '', economicRequestId: goal.economicRequestId || '' } }); } catch {} await recordEvent(goal, 'notification_queued', 'success', message, { idempotencyKey: key }); }
