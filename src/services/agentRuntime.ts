...
import { resolveSkillCapabilityPlan } from './capabilityFoundationIntegration.js';
...
export async function notifyGoalIfNeeded(goal: AgentGoal): Promise<void> { if (!['needs_user', 'blocked', 'completed'].includes(goal.status)) return; const message = goal.summary || 'Kurukoo has an update on your agent goal.'; if (!(await autonomousNotificationsAllowed(goal.phone))) return; const key = `goal:${goal.id}:notify:${goal.status}:${goal.updatedAt}`; if (await notificationAlreadyQueued(goal, key)) return; try {
    await sendFcmPush(goal.phone, 'Kurukoo update', message, goal.conversationId ? `/chat?conversationId=${encodeURIComponent(goal.conversationId)}&contextId=${encodeURIComponent(`goal:${goal.id}`)}&action=resume` : undefined, {
