...
export const MCP_SCOPES = ['kurukoo.read', 'kurukoo.act', 'kurukoo.write', 'kurukoo.execute'] as const;
export type McpScope = typeof MCP_SCOPES[number];

const AUTH_CODE_TTL_MS = 5 * 60 * 1000;
const ACCESS_TTL_MS = Math.max(300, Number(process.env.KURUKOO_MCP_ACCESS_TOKEN_TTL_SECONDS || 3600)) * 1000;
const REFRESH_TTL_MS = Math.max(3600, Number(process.env.KURUKOO_MCP_REFRESH_TOKEN_TTL_SECONDS || 7_776_000)) * 1000;

function issuer(): string {
  const configured = String(process.env.KURUKOO_MCP_ISSUER || '').trim().replace(/\/$/, '');
  if (!configured) throw new Error('KURUKOO_MCP_ISSUER must be configured for the remote MCP integration');
