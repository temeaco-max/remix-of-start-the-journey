...
async function inspectConnectedResourceForDeviceSupport(phone: string | undefined, query: string, threadId?: string): Promise<IntentRoutingResult | null> {
  ...
  const exactLabelMatches = active.filter(resource => resource.label.trim() && normalizedQuery.includes(resource.label.trim().toLowerCase()));
  const matches = exactLabelMatches.length ? exactLabelMatches : active.filter(resource => deviceResourceMatches(query, resource));
  if (matches.length !== 1) {
...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  if (phone && /\b(?:walk|guide) me through\b.*\b(?:safe checks?|checks?|phone|device|laptop|computer|wi-?fi|internet|network)\b|\b(?:safe checks?|what can i check)\b.*\b(?:phone|device|laptop|computer|wi-?fi|internet|network)\b/i.test(q)) {
    ...
    return { skill: 'device_support', reply: `I cannot inspect your ${subject} directly from this client, but I can walk you through a few safe checks. Try the steps below and tell me what you find; I will help interpret the results before suggesting repair.`, cardData: { type: 'device_support', status: 'needs_user', title: 'Guided device checks', inspection: { status: 'unavailable', reason: 'no_authorized_connected_resource' }, guidedChecks, nextActions: [{ id: 'report_guided_results', label: 'Tell me what you found', prompt: `I completed the safe checks for my ${subject}; here is what I found:` }, { id: 'find_inspector', label: 'Find someone to inspect it', prompt: `Find someone who can inspect my ${subject} for me.` }], liveObservation: false, noInspectionPerformed: true, ownerScoped: true }, canonicalAction: 'device_support.guided_checks', progressStage: 'information', extractionSource: 'deterministic' };
  }
