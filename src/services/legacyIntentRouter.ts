...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  const outcomeSkill = explicitFoodOutcome ? 'order_food' : explicitTransportOutcome ? 'ride_request' : explicitRepairOutcome ? 'phone_repair' : explicitProductOutcome ? 'product_sourcing' : explicitProviderOutcome ? 'find_worker' : null;
