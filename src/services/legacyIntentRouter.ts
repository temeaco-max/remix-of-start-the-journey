...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  const outcomeSkill = explicitFoodOutcome ? 'order_food' : explicitAutomotiveServiceOutcome ? 'find_worker' : explicitTransportOutcome ? 'ride_request' : explicitAccommodationOutcome ? 'hotel_deals' : explicitPropertyOutcome ? 'rental_tracker' : explicitJobOutcome ? 'job_tracker' : explicitTutorOutcome ? 'find_worker' : explicitRepairOutcome ? 'phone_repairer' : explicitProductOutcome ? 'product_sourcing' : explicitProviderOutcome ? 'find_worker' : null;
