...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  const explicitTransportOutcome = /\b(?:get|take|drive|bring)\s+me\s+(?:to|from)\b|\b(?:need|want)\s+to\s+(?:get|be)\s+(?:to|in)\b|\b(?:find|book|hire)\s+(?:me\s+)?(?:a\s+)?(?:bus|taxi|ride|driver|train|ferry|car)\b/i.test(q) || (/\b(?:airport|station|transport|ride|travel|bus|taxi|train|ferry|journey|trip)\b/i.test(q) && /\b(?:tomorrow|today|tonight|from|to|by\s+\d|arrive|arrival|leave|leaving|book|find|need)\b/i.test(q));
  ...
  const explicitPropertyOutcome = /\b(?:apartment|flat|house|rental|rent|two[ -]?bed(?:room)?|property)\b/i.test(q) && /\b(?:find|need|want|rent|looking|search)\b/i.test(q);
  const explicitJobOutcome = /\b(?:job|jobs|work|vacancy|vacancies|employment|role)\b/i.test(q) && /\b(?:find|need|want|looking|apply|search)\b/i.test(q);
  ...
  const explicitProviderOutcome = /\b(?:find|hire|book|get|need)\b/i.test(q) && /\b(?:someone|person|plumber|teacher|mechanic|electrician|cleaner|technician|guitar)\b/i.test(q);
  ...
  const outcomeSkill = explicitFoodOutcome ? 'order_food' : explicitTransportOutcome ? 'ride_request' : explicitAccommodationOutcome ? 'hotel_deals' : explicitPropertyOutcome ? 'rental_tracker' : explicitJobOutcome ? 'job_tracker' : explicitRepairOutcome ? 'phone_repairer' : explicitProductOutcome ? 'product_sourcing' : explicitProviderOutcome ? 'find_worker' : null;
  ...
  if (directSkill && phone) {
    try {
      ...
      const workerMatch = q.match(/\b(plumber|plumb|electrician|electrical|mechanic|carpenter|tailor|cleaner|clean|cleaning|housekeeping|technician|teacher|guitar|painter|paint|painting|decorator|decorating|tiler|tiling|roofer|roofing|mason|welder)\b/i)?.[1]; const worker = workerMatch ? (/^plumb/i.test(workerMatch) ? 'plumber' : /^electri/i.test(workerMatch) ? 'electrician' : /^clean|^housekeep/i.test(workerMatch) ? 'house_cleaner' : /^teach/i.test(workerMatch) ? 'teacher' : /^guitar/i.test(workerMatch) ? 'guitar_teacher' : /^paint/i.test(workerMatch) ? 'painter' : /^decorat/i.test(workerMatch) ? 'decorator' : /^til/i.test(workerMatch) ? 'tiler' : /^roof/i.test(workerMatch) ? 'roofer' : workerMatch.toLowerCase()) : undefined;
