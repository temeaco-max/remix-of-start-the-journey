...
export async function routeIntent(query: string, phone?: string, provider?: AIProvider, contextHint?: ConversationalContextHint, threadId?: string): Promise<IntentRoutingResult> {
  ...
  const explicitTransportOutcome = /\b(?:get me|take me)\b[\s\S]*\b(?:airport|station|home|office)\b/i.test(q) || /\b(?:airport|station|transport|ride|travel)\b/i.test(q) && /\b(?:tomorrow|today|from|to|book|find|need)\b/i.test(q);
  const explicitProductOutcome = /\b(?:buy|order|get|find|replace|replacement)\b/i.test(q) && /\b(?:charger|cable|phone|laptop|product|item|groceries)\b/i.test(q);
  ...
  const outcomeSkill = explicitFoodOutcome ? 'order_food' : explicitTransportOutcome ? 'ride_request' : explicitProductOutcome ? 'product_sourcing' : explicitProviderOutcome ? 'find_worker' : null;
