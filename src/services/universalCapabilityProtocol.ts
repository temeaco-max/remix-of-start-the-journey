...
export interface CapabilityValidationResult {
  valid: boolean;
  code?: 'missing_capability' | 'unsupported_action' | 'invalid_confidence' | 'foreign_context' | 'stale_context' | 'missing_confirmation' | 'invalid_arguments';
  message?: string;
}
...
const ACTIONS_BY_MODE: Record<CapabilityMode, string[]> = {
  read_only: ['read', 'open'],
  conversation: ['answer', 'clarify', 'continue'],
  structured_action: ['start', 'review', 'select', 'update', 'cancel', 'resume'],
  state_change: ['create', 'update', 'pause', 'resume', 'cancel', 'confirm'],
  external_execution: ['discover', 'quote', 'select', 'confirm', 'pay', 'execute', 'track', 'dispute', 'recover'],
};

export const CANONICAL_CONTEXT_ACTIONS = new Set([
  'agent_goal:agent.goal.resume',
  'agent_goal:agent.goal.review',
  'agent_goal:agent.goal.pause',
  'agent_goal:agent.goal.cancel',
  'economic_request:economic_request.review_match',
  'economic_request:economic_request.select_provider',
  'economic_request:economic_request.update',
  'economic_request:economic_request.cancel',
  'notification:notification.open',
  'notification:notification.dismiss',
  'discovery_entity:discovery.context.open',
  'memory_context:memory.context.forget',
  'reminder:reminder.open',
  'points:points.balance.open',
]);

export function isSupportedCanonicalContextAction(objectType: string | undefined, canonicalAction: string | undefined): boolean {
  return Boolean(objectType && canonicalAction && CANONICAL_CONTEXT_ACTIONS.has(`${objectType}:${canonicalAction}`));
}
...
const LIFECYCLE_FOR_MODE: Record<CapabilityMode, string[]> = {
  read_only: ['requested', 'answered', 'completed', 'failed'],
  conversation: ['requested', 'clarifying', 'answered', 'follow_up', 'completed', 'failed'],
  structured_action: ['requested', 'clarifying', 'ready', 'completed', 'cancelled', 'failed'],
  state_change: ['requested', 'awaiting_confirmation', 'accepted', 'completed', 'cancelled', 'failed'],
  external_execution: ['requested', 'awaiting_match', 'quoted', 'awaiting_confirmation', 'payment_pending', 'executing', 'evidence_pending', 'completed', 'disputed', 'failed'],
};

function modeFor(flow: SkillFlow | null, capabilities: EconomicCapability[]): CapabilityMode {
  if (flow?.mode === 'information') return 'read_only';
  if (flow?.mode === 'safety') return 'conversation';
  if (flow?.mode === 'coordination') return 'state_change';
  ...
  if (flow?.mode === 'economic' || capabilities.length) return 'structured_action';
  return 'conversation';
}

function riskFor(mode: CapabilityMode, capabilities: EconomicCapability[]): CapabilityRisk {
  if (mode === 'read_only' || mode === 'conversation') return 'read_only';
  if (capabilities.some(capability => ['payment', 'escrow', 'fulfillment', 'completion', 'dispute'].includes(capability))) return 'confirmation_required';
  if (mode === 'external_execution') return 'confirmation_required';
  return 'low_risk';
}

function activationFor(capabilities: EconomicCapability[], mode: CapabilityMode): CapabilityActivationState {
  if (mode === 'read_only' || mode === 'conversation') return 'locally_available';
  if (capabilities.some(capability => ['discovery', 'payment', 'escrow', 'fulfillment', 'tracking', 'evidence'].includes(capability))) return 'repository_ready_external_activation';
  return 'locally_available';
}

function ownersFor(flow: SkillFlow | null, capabilities: EconomicCapability[]): string[] {
  const owners = ['canonicalChatTurnService', 'intentRouter', 'skillFlows'];
  if (flow?.mode === 'economic' || capabilities.length) owners.push('economicRequestService', 'agenticStorefront');
  if (capabilities.includes('discovery')) owners.push('discoveryNetwork', 'providerDiscovery');
  if (capabilities.includes('payment') || capabilities.includes('escrow')) owners.push('paymentRoutes', 'escrow');
  if (capabilities.includes('fulfillment') || capabilities.includes('tracking')) owners.push('executionConnector', 'notificationQueue');
  if (flow?.mode === 'safety') owners.push('safetyService');
  if (flow?.mode === 'coordination') owners.push('agentRuntime', 'notificationQueue');
...
function descriptorForSkill(skill: string, flow: SkillFlow | null): UniversalCapabilityDescriptor {
  ...
  const declaredCapabilities = getSkillCapabilities(skill);
  const mode = modeFor(flow, declaredCapabilities);
  const capabilities = flow?.mode === 'information' ? [] : declaredCapabilities;
  const risk = riskFor(mode, capabilities);
  ...
  const capabilityActions: Record<string, string> = {
    discovery: 'discover', availability: 'check_availability', quote: 'quote', verification: 'verify', reservation: 'reserve',
    payment: 'pay', escrow: 'authorize_escrow', contract: 'review_contract', fulfillment: 'fulfill', tracking: 'track',
    evidence: 'submit_evidence', cancellation: 'cancel', dispute: 'dispute', completion: 'confirm_completion',
  };
  const actions = [...new Set([...ACTIONS_BY_MODE[mode], ...capabilities.map(capability => capabilityActions[capability]).filter(Boolean)])];
  const external = activationFor(capabilities, mode) === 'repository_ready_external_activation'
...
const CANONICAL_OPERATION_DEFINITIONS: Array<{ capability: string; family: string; actions: string[]; owner: string[]; risk: CapabilityRisk; activationState: CapabilityActivationState }> = [
  ...
  { capability: 'payment', family: 'economic-execution', actions: ['authorize', 'confirm', 'refund'], owner: ['stripePayment', 'paymentRoutes', 'escrow'], risk: 'confirmation_required', activationState: 'repository_ready_external_activation' },
  ...
  { capability: 'subscription', family: 'account', actions: ['inspect', 'change'], owner: ['subscriptionService', 'pricingService'], risk: 'confirmation_required', activationState: 'repository_ready_external_activation' },
  ...
  { capability: 'points', family: 'economy', actions: ['balance', 'claim', 'use'], owner: ['pointsEngine'], risk: 'confirmation_required', activationState: 'locally_available' },
  ...
  { capability: 'order', family: 'commerce', actions: ['create'], owner: ['orderRoutes', 'orderFinalizer', 'economicRequestService'], risk: 'confirmation_required', activationState: 'repository_ready_external_activation' },
...
export function validateCapabilityProposal(proposal: CapabilityActionProposal, descriptor: UniversalCapabilityDescriptor | undefined, options: { ownerVerified: boolean; objectVerified: boolean; stale: boolean; confirmationGranted: boolean }): CapabilityValidationResult {
  if (!descriptor) return { valid: false, code: 'missing_capability', message: 'The requested capability is not registered.' };
  if (!proposal.action || !descriptor.actions.includes(proposal.action)) return { valid: false, code: 'unsupported_action', message: 'That action is not supported by the canonical capability.' };
  if (proposal.confidence !== undefined && (!Number.isFinite(proposal.confidence) || proposal.confidence < 0 || proposal.confidence > 1)) return { valid: false, code: 'invalid_confidence', message: 'The action proposal confidence is invalid.' };
  if (proposal.contextId && !options.ownerVerified) return { valid: false, code: 'foreign_context', message: 'The referenced context is not owned by the current user.' };
  if (proposal.canonicalObjectId && !options.objectVerified) return { valid: false, code: 'foreign_context', message: 'The referenced canonical object is not owned by the current user.' };
  if (options.stale) return { valid: false, code: 'stale_context', message: 'The referenced context is no longer active.' };
  if (proposal.confirmationRequired && !options.confirmationGranted) return { valid: false, code: 'missing_confirmation', message: 'This action requires explicit user confirmation.' };
  if (proposal.arguments && Object.values(proposal.arguments).some(value => typeof value === 'function' || typeof value === 'symbol')) return { valid: false, code: 'invalid_arguments', message: 'The action arguments are not valid.' };
  return { valid: true };
}

export function projectCapabilityResult(input: { capability?: string; action?: string; contextId?: string; canonicalObjectId?: string; cardData?: any; message: string; activationState?: CapabilityActivationState }): UniversalCapabilityResult {
  const card = input.cardData || {};
  const raw = String(card.status || card.stage || card.state || '').toLowerCase();
  const status: CapabilityResultStatus = input.activationState === 'unavailable_external_dependency' ? 'unavailable_external_dependency'
    : raw.includes('complete') || raw === 'completed' || raw === 'fulfilled' ? 'completed'
      : raw.includes('confirm') || raw === 'awaiting_confirmation' ? 'confirmation_required'
        : raw.includes('fail') || raw.includes('error') || raw === 'cancelled' ? 'failed'
          : raw.includes('wait') || raw.includes('pending') || raw.includes('match') ? 'waiting'
