...
export async function queryUnifiedAI(prompt: string, options: UnifiedAIOptions = {}): Promise<AIResponse> {
  ...
  if (preferred === 'gemini') { try { const result = cleanThinking(await queryGemini(prompt, { systemInstruction: options.systemPrompt })); return { provider: 'Gemini', model: 'configured-gemini', text: result.text, thought: result.thought, latencyMs: Date.now() - started, cost: 'configured', intent: classification?.intent, confidence: classification?.confidence }; } catch (err) { console.warn('[AI] Gemini failed:', err); return fallback(classification); } }
  if (preferred === 'groq') { try { const result = cleanThinking(await queryGroq(prompt, { systemPrompt: options.systemPrompt })); return { provider: 'Groq', model: 'llama-3.1-8b-instant', text: result.text, thought: result.thought, latencyMs: Date.now() - started, cost: 'rate-limited', intent: classification?.intent, confidence: classification?.confidence }; } catch (err) { console.warn('[AI] Groq failed:', err); return fallback(classification); } }
  if (preferred === 'smollm2') { try { const result = cleanThinking(await querySmolLM2(prompt, options.systemPrompt)); return { provider: 'SmolLM2', model: 'SmolLM2-1.7B-Instruct', text: result.text, thought: result.thought, latencyMs: Date.now() - started, cost: 'low', intent: classification?.intent, confidence: classification?.confidence }; } catch (err) { console.warn('[AI] SmolLM2 failed:', err); return fallback(classification); } }
  ...
  if (!classification || SIMPLE_INTENTS.has(classification.intent)) {
    try { const result = cleanThinking(await querySmolLM2(prompt, options.systemPrompt)); return { provider: 'SmolLM2', model: 'SmolLM2-1.7B-Instruct', text: result.text, thought: result.thought, latencyMs: Date.now() - started, cost: 'low', intent: classification?.intent, confidence: classification?.confidence }; }
    catch (err) { console.warn('[AI] Simple-query SmolLM2 failed:', err); }
  }
  if (process.env.GROQ_API_KEY) {
    try { const result = cleanThinking(await queryGroq(prompt, { systemPrompt: options.systemPrompt })); return { provider: 'Groq', model: 'llama-3.1-8b-instant', text: result.text, thought: result.thought, latencyMs: Date.now() - started, cost: 'rate-limited', intent: classification?.intent, confidence: classification?.confidence }; }
    catch (err) { console.warn('[AI] Complex-query Groq failed:', err); }
  }
...
export async function* streamUnifiedAI(prompt: string, options: UnifiedAIOptions = {}): AsyncGenerator<AIStreamChunk> {
  ...
  if (result.thought) yield { type: 'thought', thought: 'Reasoning completed.' };
  for (let i = 0; i < (result.text || '').length; i += 8) { yield { type: 'text', content: result.text.slice(i, i + 8) }; await new Promise(resolve => setTimeout(resolve, 8)); }
}
