...
export interface ChatMessageInput { phone: string; sender: 'user' | 'assistant' | 'system'; content: string; channel?: string; conversationId?: string; cardData?: unknown; metadata?: unknown; }
...
export async function listChatConversations(phone: string, limit = 50): Promise<any[]> {
    const db = await dbReady(); const stmt = db.prepare(`SELECT id, phone, title, channel, created_at, updated_at FROM chat_conversations WHERE phone = ? ORDER BY updated_at DESC LIMIT ?`);
    stmt.bind([phone, Math.min(Math.max(limit, 1), 100)]); const rows: any[] = []; while (stmt.step()) rows.push(stmt.getAsObject()); stmt.free(); return rows;
}

export async function listChatMessages(phone: string, options: ChatHistoryOptions = {}): Promise<any[]> {
    const db = await dbReady(); const limit = Math.min(Math.max(options.limit || 50, 1), 100); const params: any[] = [phone];
    ...
    const stmt = db.prepare(sql); stmt.bind(params); const rows: any[] = []; while (stmt.step()) rows.push(stmt.getAsObject()); stmt.free(); return rows.reverse();
}

export async function deleteChatMessage(phone: string, messageId: number): Promise<boolean> {
    const db = await dbReady(); const stmt = db.prepare(`SELECT id FROM messages WHERE id = ? AND phone = ?`); stmt.bind([messageId, phone]); const exists = stmt.step(); stmt.free();
    if (!exists) return false; db.run(`DELETE FROM chat_message_meta WHERE message_id = ?`, [messageId]); db.run(`DELETE FROM messages WHERE id = ? AND phone = ?`, [messageId, phone]); saveDb(); return true;
}

export async function clearChatConversation(phone: string, conversationId: string): Promise<number> {
    const db = await dbReady(); const idsStmt = db.prepare(`SELECT message_id FROM chat_message_meta WHERE conversation_id = ?`); idsStmt.bind([conversationId]);
    const ids: number[] = []; while (idsStmt.step()) ids.push(Number(idsStmt.getAsObject().message_id)); idsStmt.free();
    for (const id of ids) db.run(`DELETE FROM messages WHERE id = ? AND phone = ?`, [id, phone]);
    db.run(`DELETE FROM chat_message_meta WHERE conversation_id = ?`, [conversationId]); db.run(`DELETE FROM chat_conversations WHERE id = ? AND phone = ?`, [conversationId, phone]); saveDb(); return ids.length;
}
