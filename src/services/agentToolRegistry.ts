...
export type AgentToolName = 'get_request_state' | 'get_memory_context' | 'get_reminders' | 'get_connected_resources' | 'recheck_economic_request';
...
function autonomousLowRiskEnabled(): boolean {
  return process.env.KURUKOO_AGENT_AUTONOMOUS_LOW_RISK === 'true';
}
...
export async function executeAgentTool(
  ...
  if (!definition || !context.phone || context.phone.startsWith('anon_')) {
    return { ok: false, tool: name, permission: definition?.permission || 'high_risk', message: 'This action is not available for the current identity.' };
  }
  ...
  if (name === 'get_connected_resources') {
    ...
    return {
      ...
      permission: 'read',
      data: {
        resources: active.slice(0, 24).map(resource => ({
          id: resource.id,
          kind: resource.kind,
          ...
          hasView: Boolean(resource.viewUrl || resource.streamUrl),
          lastSeenAt: resource.lastSeenAt,
        })),
      },
      evidence: `connected_resources:${active.length}`,
    };
  }
