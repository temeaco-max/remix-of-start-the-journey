...
async function executeAgentToolInternal(name: AgentToolName, args: Record<string, unknown>, context: AgentToolContext): Promise<AgentToolResult> {
  ...
  if (name === 'get_memory_context') { const query = typeof args.query === 'string' ? args.query.slice(0, 500) : ''; const [memory, identity] = await Promise.all([buildWorkingContext(context.phone, query || 'active goal', { threadId: context.conversationId, route: 'compact' }), getCanonicalIdentityContext(context.phone, 'agentToolRegistry')]); return { ok: true, tool: name, permission: 'read', data: { context: memory.context.slice(0, 2000), items: memory.selected.map(item => ({ tier: item.tier, source: item.source, text: item.text.slice(0, 220) })), identity: { hasProfile: identity.hasProfile, hasName: identity.hasName, name: identity.name, nameProvenance: identity.nameProvenance, location: identity.location, country: identity.country, preferences: identity.preferences }, memoryFacts: identity.stableFacts }, evidence: `memory:${memory.selected.length}:facts:${identity.stableFacts.length}` }; }
