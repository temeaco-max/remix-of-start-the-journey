...
export async function createEconomicRequest(input:{id:string;phone:string;skill:string;requirements:Record<string,unknown>;amount?:number}):Promise<EconomicRequest>{const category=getEconomicCategory(input.skill)||'classifieds-marketplace';const capabilities=getDefaultCapabilities(category);const db=await ensureEconomicRequestsTable();const s=db.prepare(`INSERT INTO economic_requests(id,phone,skill,category,status,requirements_json,capabilities_json) VALUES(?,?,?,?,'requested',?,?)`);s.bind([input.id,input.phone,input.skill,category,JSON.stringify({...input.requirements,amount_minor:input.amount??null}),JSON.stringify(capabilities)]);s.step();s.free();saveDb();return (await getEconomicRequest(input.id))!;}
