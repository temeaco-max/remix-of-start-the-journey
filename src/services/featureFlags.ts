...
export const FEATURE_REGISTRY: Record<string, FeatureDefinition> = {
    ...
    hosted_poolside: { description: 'Poolside Laguna XS 2.1 hosted AI execution', lifecycle: 'pilot', defaultEnabled: false, requires: ['POOLSIDE_API_KEY'], providerPrerequisites: ['approved model, quota, privacy and retention review'], risk: 'medium', killSwitch: true, adminVisible: true },
    ...
    points_engine: { description: 'Kurukoo Points/currency economy (network units, ledger, rewards, deductions)', lifecycle: 'production-capable', defaultEnabled: true, markets: ['ng'], risk: 'medium', killSwitch: true, adminVisible: true },
    uk_life_admin: { description: 'UK life-admin skills (bin_day, mot_reminder, council_tax, doctor_appointment, train_check)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb'], requires: ['UK_EXTERNAL_API_CONFIG'], providerPrerequisites: ['approved UK government and service external API credentials'], risk: 'medium', killSwitch: true, adminVisible: true },
    diaspora_payments: { description: 'Cross-border diaspora payment rail (Stripe UK → OPay/Moniepoint NG)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb', 'ng'], requires: ['STRIPE_SECRET_KEY', 'OPAY_OR_MONIEEPOINT_CONFIG'], providerPrerequisites: ['approved Stripe and OPay/Moniepoint credentials, FX conversion setup'], risk: 'high', killSwitch: true, adminVisible: true },
    // ── muse.ai gap-fill capabilities (UK-gated, paid tier) ───────────────
    secure_credentials: { description: 'User-scoped encrypted credential store the agent cannot see (AES-256-GCM, PBKDF2)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb'], risk: 'high', killSwitch: true, adminVisible: true },
    one_time_cards: { description: 'One-time virtual cards + purchase protection claims (provider-agnostic)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb'], requires: ['ONE_TIME_CARD_PROVIDER'], providerPrerequisites: ['Stripe Issuing or equivalent card tokenization provider'], risk: 'high', killSwitch: true, adminVisible: true },
    secure_execution: { description: 'Isolated browser/VM execution surface for agent tasks (sandboxed, provider-pluggable)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb'], requires: ['SECURE_EXECUTION_PROVIDER'], providerPrerequisites: ['Browserless, Playwright-as-a-service, or equivalent sandboxed runtime'], risk: 'high', killSwitch: true, adminVisible: true },
    execution_audit: { description: 'User-facing audit trail timeline (reads existing coordinator events)', lifecycle: 'pilot', defaultEnabled: false, markets: ['gb'], risk: 'medium', killSwitch: true, adminVisible: true },
};
