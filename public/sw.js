  ...
  event.waitUntil(
    Promise.all([
      ...
      caches
        ...
        .then((cache) =>
          cache
            .addAll(PAGES_TO_CACHE.map((path) => new Request(path, { cache: "reload" })))
            .catch(() => {}),
        ),
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys.map((key) => (ALLOWED_CACHES.includes(key) ? undefined : caches.delete(key))),
        ),
      )
  ...
  if (url.pathname === "/chat/" || url.pathname === "/chat") {
    event.respondWith(
      fetch(request)
        .then((response) => {
          if (response.ok)
            caches.open(PWA_SHELL_CACHE).then((cache) => cache.put("/chat/", response.clone()));
          return response;
        })
        .catch(() =>
          caches.match("/chat/").then((cached) => cached || caches.match("/offline.html")),
        ),
    );
    return;
  }
