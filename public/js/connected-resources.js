(() => {
  const state = { activeView: null, loaded: false };
  ...
  function renderResource(resource) {
    ...
    if (resource.viewUrl || resource.streamUrl) {
      ...
      view.type = 'button'; view.addEventListener('click', () => loadView(card, resource.id)); actions.appendChild(view);
    }
    for (const command of capabilities.filter(item => item && !['control', 'view', 'stream', 'inspect'].includes(String(item))).slice(0, 8)) {
      ...
      button.type = 'button'; button.addEventListener('click', () => control(card, resource.id, String(command))); actions.appendChild(button);
    }
    card.appendChild(actions);
    card.appendChild(make('p', 'workspace-note', 'Commands remain owner-scoped and pass through Kurukoo’s canonical action boundary. External device delivery is only claimed when the connected adapter provides evidence.')).className = 'workspace-note connected-resource-note';
    card.appendChild(make('div', 'connected-resource-view', undefined)).dataset.connectedView = '1';
    card.appendChild(make('p', 'workspace-note', 'Ready')); card.lastChild.dataset.connectedStatus = '1';
    return card;
  }
  ...
  async function render() {
    ...
    if (!body) return;
    if (body.dataset.connectedResourcesLoaded === '1') return;
    body.dataset.connectedResourcesLoaded = '1';
    try {
      const response = await fetch('/api/connect/resources', { credentials: 'same-origin' });
      const data = await response.json().catch(() => ({}));
      if (response.status === 401) {
        body.replaceChildren(make('p', 'empty-state', 'Sign in to connect devices and access their views and controls in Chat.'));
        return;
      }
      ...
      intro.append(make('div', 'panel-heading', undefined), make('p', 'workspace-note', 'Anything you connect to Kurukoo can expose its authorised capabilities to the same conversation, subject to its permissions and adapter.'));
      intro.querySelector('.panel-heading').append(make('div', undefined, 'Connected to Kurukoo'));
      body.replaceChildren(intro);
      if (!resources.length) {
        body.appendChild(make('section', 'workspace-panel', 'No connected resources yet.'));
        body.appendChild(make('p', 'empty-state', 'Connect a device, channel, camera or other authorised resource, then ask Kurukoo to view or control what it exposes.'));
        return;
      }
      for (const resource of resources) body.appendChild(renderResource(resource));
    } catch (error) {
      body.replaceChildren(make('p', 'empty-state', String(error?.message || 'Connected resources unavailable.')));
    }
  }
  ...
  function scheduleRender() {
    if (scheduled) return;
    scheduled = true;
    setTimeout(() => { scheduled = false; render(); }, 50);
  }
