(() => {
  ...
  const bootstrap = async () => {
    const isAuthenticatedSurface =
      document.body?.classList.contains("k-app-page") ||
      document.body?.classList.contains("workspace-page");
    if (!isAuthenticatedSurface || document.querySelector("[data-kurukoo-auth-left-rail-ad]"))
      return;

    const rail =
      document.querySelector(".k-app-sidebar") || document.querySelector(".workspace-sidebar");
    if (!rail) return;

    try {
      const response = await fetch("/api/advertising/left-rail", {
      ...
      const account =
        rail.querySelector(".workspace-account") || rail.querySelector(".k-app-account");
