(() => {
  ...
  function readPendingMessages() {
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.filter((item) => item && typeof item.text === "string").slice(-10)
        : [];
    } catch {
      return [];
    }
  ...
  function retryPendingMessage(id) {
    if (!navigator.onLine) {
      showNotice("You’re still offline. Kurukoo has kept the message for you.", {
        tone: "offline",
      });
      return;
    }
  ...
  function captureOfflineIntent(event) {
    ...
    const quickAction = target?.closest(
      ".composer-quick-actions [data-prompt], .quick-actions [data-prompt]",
    );
    const isEnter =
    ...
    renderPendingNotice();
    showNotice(
      "You’re offline. Your message has been saved and will only send when you choose Retry.",
      { tone: "offline" },
    );
  }
  ...
  function updateConnectivityState() {
    ...
    if (!isAppSurface()) return;
    if (online) {
      showNotice("Back online. Kurukoo can continue your conversation.", {
        tone: "online",
        timeout: 4200,
      });
      renderPendingNotice();
      ...
      showNotice(
        "You’re offline. Cached Kurukoo pages remain available; new messages can be saved for manual retry.",
        { tone: "offline" },
      );
      renderPendingNotice();
    }
  ...
  async function registerWorker() {
    ensureClientVisualAuthority();
    if (isAppSurface() && navigator.onLine)
      showNotice("Checking Kurukoo connection…", { tone: "loading" });
    ...
    try {
      ...
      if (navigator.onLine && isAppSurface())
        showNotice("Kurukoo is ready to continue your conversation.", {
          tone: "online",
          timeout: 2600,
        });
  ...
  window.addEventListener("appinstalled", () => {
    ...
    deferredInstallPrompt = null;
    showNotice("Kurukoo is installed and ready from your home screen.", {
      tone: "installed",
      timeout: 4200,
    });
  });
