(() => {
  ...
  if (!feed) return;
  const state = { view: "all", radius: 10000, home: null };
  const sectionMeta = {
  ...
  const iconFor = (type) =>
    ({ discovery: "discover", topic: "chat", capability: "sparkles", promotion: "badge" })[type] ||
    "discover";
  ...
  function chatHref(item) {
    const params = new URLSearchParams({
      prompt: item.chatAction?.prompt || `Help me explore ${item.title}`,
    });
  ...
  async function trackPromotion(id, kind) {
    try {
      await fetch(`/api/discover/promotions/${encodeURIComponent(id)}/${kind}`, {
        method: "POST",
        keepalive: true,
        credentials: "include",
      });
    } catch {
      /* analytics must never block discovery actions */
    }
  }
  function actionButton(item, action, label, primary = false) {
    const button = make(
      "button",
      `discover-action${primary ? " discover-action--primary" : ""}`,
      label,
    );
    button.type = "button";
    if (action === "open_chat" || action === "act") {
      button.addEventListener("click", () => {
        if (action === "act") {
          void trackPromotion(String(item.id).replace(/^promotion:/, ""), "click");
          if (item.destination) {
            window.location.href = item.destination;
            return;
          }
        }
        window.location.href = chatHref(item);
      });
      return button;
    }
    button.addEventListener("click", async () => {
      ...
      try {
        await persistAction(item, action);
        button.textContent =
          action === "watch" ? "Watching" : action === "follow" ? "Following" : "Saved";
      } catch {
  ...
  function itemCard(item) {
    ...
    const body = make("div");
    const title = make("h4", "discover-discovery-card__title", item.title);
    if (item.sponsored) {
      const disclosure = make(
        "span",
        "discover-item-badge discover-item-badge--sponsored",
        item.disclosure || "Sponsored",
      );
      title.append(" ", disclosure);
    }
    ...
    if (item.verified)
      meta.appendChild(
        make("span", "discover-item-badge discover-item-badge--verified", "Verified"),
      );
    if (item.available)
      meta.appendChild(
        make("span", "discover-item-badge discover-item-badge--live", "Available now"),
      );
    if (item.distanceMetres != null)
      meta.appendChild(
        make("span", "discover-item-badge", `${Math.round(Number(item.distanceMetres))} m`),
      );
    if (item.lifecycle)
      meta.appendChild(
        make("span", "discover-item-badge", String(item.lifecycle).replaceAll("_", " ")),
      );
    ...
    actions.appendChild(
      actionButton(
        item,
        item.type === "promotion" ? "act" : "open_chat",
        item.type === "promotion"
    ...
    if (item.actions?.includes("follow"))
      actions.appendChild(actionButton(item, "follow", "Follow"));
    if (item.actions?.includes("save")) actions.appendChild(actionButton(item, "save", "Save"));
    const wrap = make("div");
    wrap.append(body, actions);
    card.append(iconMarkup(iconFor(item.type)), wrap);
    card.style.gridTemplateColumns = "28px minmax(0,1fr)";
    card.firstElementChild.style.cssText =
      "width:28px;height:28px;display:grid;place-items:center;color:var(--terracotta);background:rgba(217,122,92,.1);border-radius:7px;align-self:start";
    if (item.sponsored) {
      void trackPromotion(String(item.id).replace(/^promotion:/, ""), "impression");
    }
    return card;
  }
  function renderSection(key, items) {
    if (
      state.view !== "all" &&
      key !== state.view &&
      !(state.view === "nearby" && ["for_you", "nearby", "today"].includes(key))
    )
      return null;
    const section = make("section", "discover-section-card");
    section.dataset.section = key;
    const heading = make("div", "discover-section-card__heading");
    const copy = make("div");
    copy.append(
      make("span", "discover-section-eyebrow", sectionMeta[key][1]),
      make("h3", "", sectionMeta[key][0]),
    );
    heading.append(copy, make("span", "discover-section-count", String(items.length)));
    section.appendChild(heading);
    const list = make("div", "discover-section-card__items");
    if (!items.length) list.append(make("div", "discover-empty", "Nothing attributed here yet."));
    else items.slice(0, 5).forEach((item) => list.appendChild(itemCard(item)));
  ...
  function render() {
    ...
    Object.entries(sections).forEach(([key, items]) => {
      const section = renderSection(key, Array.isArray(items) ? items : []);
      if (section) feed.appendChild(section);
    });
    const density = document.querySelector("[data-density]");
    if (density)
      density.textContent =
        state.home?.density === "rich"
          ? "Rich local view"
          : state.home?.density === "sparse"
            ? "Sparse but useful"
            : "Explore-first view";
    const sparse = document.querySelector("[data-sparse]");
    if (sparse) sparse.hidden = !state.home?.sparse;
    const sparseCopy = document.querySelector("[data-sparse-copy]");
    if (sparseCopy && state.home?.explanation) sparseCopy.textContent = state.home.explanation;
  }
  async function loadHome() {
    ...
    try {
      const response = await fetch(`/api/discover/home?${params.toString()}`, {
        credentials: "include",
      });
      ...
      if (last)
        last.textContent = where.approximate
      ...
      state.home = {
        ...
        density: "empty",
        explanation:
          "Discover is available even when the local network is quiet. Ask Kurukoo directly or explore its capabilities.",
      };
      render();
    }
  }
  ...
  document.getElementById("discover-search-form")?.addEventListener("submit", (event) => {
    ...
    if (!q) return;
    window.location.href = `/chat?prompt=${encodeURIComponent(`Help me discover ${q}`)}`;
  });
