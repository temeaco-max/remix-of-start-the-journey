(() => {
  ...
  const createSecondaryNav=()=>{
    if(!document.body.classList.contains('k-app-page')||document.querySelector('.k-app-more'))return;
    const nav=Array.from(document.querySelectorAll('.k-app-nav')).find(node=>node.getAttribute('aria-label')==='Workspace');
    const account=Array.from(document.querySelectorAll('.k-app-nav')).find(node=>node.getAttribute('aria-label')==='Account');
    const sidebar=document.querySelector('.k-app-sidebar');
    if(!nav||!sidebar)return;
    const hidden=Array.from(nav.querySelectorAll('a')).filter(a=>{try{return !['/agents','/topics','/points'].includes(new URL(a.href,location.href).pathname)}catch{return false}});
    const accountLinks=account?[...account.querySelectorAll('a')]:[];
    const details=document.createElement('details'); details.className='k-app-more';
    if(hidden.some(a=>a.classList.contains('active'))||accountLinks.some(a=>a.classList.contains('active'))) details.open=true;
  ...
  const boot=()=>{loadStylesheet('kurukoo-client-foundation','/css/kurukoo-client-foundation.css');loadStylesheet('kurukoo-web-completion','/css/kurukoo-web-completion.css');loadStylesheet('kurukoo-feature-compass','/css/kurukoo-feature-compass.css');loadStylesheet('kurukoo-visual-completion','/css/kurukoo-visual-completion.css');loadStylesheet('kurukoo-platform-state-visual','/css/kurukoo-platform-state-visual.css');loadStylesheet('kurukoo-os-final','/css/kurukoo-os-final.css');loadStylesheet('kurukoo-os-workspace-final','/css/kurukoo-os-workspace-final.css');loadStylesheet('kurukoo-webapp-pixel-refinement','/css/kurukoo-webapp-pixel-refinement.css?v=1');loadStylesheet('kurukoo-webapp-screen-refinement','/css/kurukoo-webapp-screen-refinement.css?v=1');loadStylesheet('kurukoo-desktop-final','/css/kurukoo-desktop-final.css?v=1');loadStylesheet('kurukoo-page-architecture','/css/kurukoo-page-architecture.css?v=1');loadStylesheet('kurukoo-facelift','/css/kurukoo-facelift.css?v=1');loadStylesheet('kurukoo-facelift-controls','/css/kurukoo-facelift-controls.css?v=1');if(path==='/chat')loadStylesheet('kurukoo-webapp-agent-refinement','/css/kurukoo-webapp-agent-refinement.css?v=1');if(path==='/call'){loadStylesheet('kurukoo-provider-communication','/css/provider-communication.css');loadScript('kurukoo-provider-communication','/js/provider-communication-client.js');}if(path==='/top-up'||path==='/points'){loadStylesheet('kurukoo-agent-commerce','/css/agent-commerce.css');loadScript('kurukoo-agent-commerce','/js/agent-commerce-client.js');}if(document.body.classList.contains('workspace-page')){createSecondaryNav();createCollapseControl();wireMobileNav();createTabBar();void createFeatureCompass();loadScript('kurukoo-page-architecture','/js/kurukoo-page-architecture.js');}wireExploreSearch();if(document.body.classList.contains('k-app-page'))loadScript('kurukoo-view-state','/js/kurukoo-view-state.js?v=1');};
