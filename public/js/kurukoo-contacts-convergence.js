(() => {
  ...
  const target = String(value) => encodeURIComponent(value);
  ...
  const communicationAction = (person, action) => {
    if (action === 'message' && person.communication?.message) {
      const prompt = `Message ${person.displayName}`;
      return `<a class="workspace-text-action" href="/chat?prompt=${path(prompt)}" aria-label="Message ${esc(person.displayName)}">Message</a>`;
    }
    if (action === 'call' && person.communication?.call) return `<a class="workspace-text-action" href="/call?person=${path(person.identityId)}" aria-label="Call ${esc(person.displayName)}">Call</a>`;
    return '';
  };
  const render = (contacts) => {
    ...
    section.innerHTML = `<div class="k-app-card"><span class="k-app-card-label">People & relationships</span><h2 id="k57-contacts-title">Your contacts</h2><p>Contacts use the canonical identity and relationship owners. Messaging and calling stay on Kurukoo’s existing communication boundaries.</p><form class="k57-contact-add" data-k57-add><label>Contact phone or identity<input name="personPhone" autocomplete="tel" inputmode="tel" required placeholder="Enter a known Kurukoo contact"></label><button class="k-app-primary" type="submit">Add contact</button></form><div class="k57-contact-state" data-k57-add-state></div></div><section class="k-app-card"><div class="k57-contact-grid" data-k57-list></div><div class="k57-unavailable" data-k57-empty hidden><div><h3>No contacts yet</h3><p>Add a known Kurukoo identity. Public discovery is not inferred where the canonical identity owner does not expose it.</p></div></div><div class="k57-unavailable" data-k57-error hidden role="alert"><div><h3>Contacts are unavailable</h3><p>Kurukoo could not read the canonical identity/contact authority. No relationship or communication state is inferred locally.</p><button class="k-app-primary" type="button" data-k57-retry>Retry Contacts</button></div></div></section></section>`;
    ...
    section.querySelector('[data-k57-add]').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const phone = form.elements.personPhone.value.trim(); const state = section.querySelector('[data-k57-add-state]'); const button = form.querySelector('button'); button.disabled = true; state.textContent = 'Checking canonical identity…'; try { await api('/api/contacts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ personPhone: phone }) }); form.reset(); state.textContent = 'Contact added.'; await load(); } catch (error) { state.textContent = error instanceof Error ? error.message : 'Unable to add contact'; } finally { button.disabled = false; } });
    return section;
  };
  const loading = () => { const section = document.createElement('section'); section.className = 'k57-contacts'; section.innerHTML = '<section class="k-app-card" aria-live="polite" aria-busy="true"><div class="k-app-list-loading">Loading canonical contacts…</div></section>'; return section; };
  const load = async () => {
    const prior = container.querySelector('.k57-contacts'); prior?.remove();
    const place = loading(); container.appendChild(place);
    try { const payload = await api('/api/contacts'); const section = render(Array.isArray(payload.contacts) ? payload.contacts : []); place.replaceWith(section); section.querySelector('[data-k57-empty]').hidden = Boolean(payload.contacts?.length); section.querySelector('[data-k57-retry]')?.addEventListener('click', load); }
    catch { const failed = loading(); failed.innerHTML = '<section class="k-app-card k57-unavailable" role="alert"><div><h3>Contacts are unavailable</h3><p>Kurukoo could not read the canonical identity/contact authority. No relationship or communication state is inferred locally.</p><button class="k-app-primary" type="button" data-k57-retry>Retry Contacts</button></div></section>'; container.appendChild(failed); failed.querySelector('[data-k57-retry]').addEventListener('click', load); }
  };
  load();
})();
