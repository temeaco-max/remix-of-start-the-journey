(() => {
  ...
  const communicationAction = (person) => {
    ...
    const call = person.communication?.call ? `<a class="workspace-text-action" href="/call" aria-label="Open canonical Call for ${esc(person.displayName)}">Call</a>` : '';
    const unavailable = !person.communication?.call ? `<span class="k57-contact-meta">Call unavailable: ${esc(person.communication?.callUnavailableReason || 'Realtime call transport is unavailable.')}</span>` : '';
