(() => {
  ...
  const pathToSection = (pathname) => {
    ...
    return (
      match?.[0] || (normalized.startsWith("/app/") ? normalized.split("/")[2] || "desk" : "desk")
    );
  };
  ...
  const parseLocation = (url) => {
    ...
    return Object.freeze({
      section: pathToSection(url.pathname),
  ...
  const buildUrl = (next = {}) => {
    ...
    ["conversationId", "contextId", "objectId", "action"].forEach((key) => {
      ...
      if (next[key]) url.searchParams.set(key, String(next[key]));
