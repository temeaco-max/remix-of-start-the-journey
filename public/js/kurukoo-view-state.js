(() => {
  ...
  const ROUTES = Object.freeze({
    desk: '/desk', discover: '/discover', requests: '/requests', tasks: '/tasks', connect: '/connect',
  ...
  const setActiveNav = (nextState) => {
    document.querySelectorAll('.k-app-nav a[href]').forEach((anchor) => {
      let pathname = '';
  ...
  window.KurukooViewState = Object.freeze({ ROUTES, store, parse: () => state });
  ...
  document.addEventListener('click', (event) => {
    ...
    try {
      ...
      try { sessionStorage.setItem('kurukoo.last.view', JSON.stringify({ section: state.section, contextId: state.contextId })); } catch { /* storage unavailable */ }
