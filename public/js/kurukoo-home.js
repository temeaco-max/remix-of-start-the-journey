(() => {
  ...
  const illustrativeSend = document.querySelector(
    '.home-chat-card__composer button[aria-label="Illustrative send"]',
  );
  if (illustrativeSend) {
  ...
  const scenarios = {
    food: {
      ...
      assistant:
        "I’ll capture what you need, then show an option only when availability and pricing are confirmed.",
      category: "Food & drink",
    ...
    repair: {
      ...
      assistant:
        "I’ll collect the fault and location before Kurukoo considers eligible repair options.",
      category: "Repairs & maintenance",
      title: "Describe the repair",
      detail:
        "A diagnosis, location and urgency are needed before any provider or quote is presented.",
    ...
    ride: {
      ...
      assistant:
        "I’ll collect the route and timing. Availability is checked before a ride option is shown.",
  ...
  const applyScenario = (name) => {
    ...
    window.setTimeout(() => {
      ...
      demo
        .querySelectorAll("[data-storefront-scenario]")
        .forEach((button) =>
          button.classList.toggle("is-active", button.dataset.storefrontScenario === name),
        );
  ...
  demo
    .querySelectorAll("[data-storefront-scenario]")
    .forEach((button) =>
      button.addEventListener("click", () => applyScenario(button.dataset.storefrontScenario)),
    );
  demo.querySelector("[data-demo-action]")?.addEventListener("click", () => {
    const prompt =
      demo.querySelector("[data-demo-action]")?.dataset.prompt || scenarios.food.prompt;
