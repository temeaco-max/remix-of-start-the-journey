...
(() => {
  ...
  function chooseVoice(preferredLanguage) {
    ...
    const language = String(
      preferredLanguage || document.documentElement.lang || navigator.language || "en",
    ).toLowerCase();
    return (
      voices.find((v) => String(v.lang || "").toLowerCase() === language) ||
      voices.find((v) =>
        String(v.lang || "")
          .toLowerCase()
  ...
  function emit(status) {
    window.dispatchEvent(
      new CustomEvent("kurukoo:voice-presence", {
        detail: { status, speaking: state.speaking, provider: "browser" },
      }),
    );
  }
  function speak(text, options = {}) {
    ...
    utterance.rate = Number.isFinite(options.rate)
      ? Math.max(0.75, Math.min(1.25, options.rate))
      : 1;
    utterance.pitch = Number.isFinite(options.pitch)
      ? Math.max(0.75, Math.min(1.25, options.pitch))
      : 1;
  ...
  window.speechSynthesis?.addEventListener?.("voiceschanged", () =>
    emit(state.speaking ? "speaking" : "idle"),
  );
})();
