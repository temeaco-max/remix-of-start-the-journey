(() => {
  const run = () => {
    ...
    if (!document.querySelector("link[data-kurukoo-os-header]")) {
      ...
      link.rel = "stylesheet";
      link.href = "/css/kurukoo-os-header.css?v=1";
      link.dataset.kurukooOsHeader = "";
    ...
    if (actions && identity && !actions.querySelector("[data-os-profile]")) {
      ...
      notify.className = "os-header-notify";
      notify.href = "/notifications";
      notify.setAttribute("aria-label", "Notifications");
      notify.innerHTML =
        '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg>';
      const profile = document.createElement("a");
      profile.className = "os-profile";
      profile.href = "/settings";
      profile.dataset.osProfile = "";
      profile.setAttribute("aria-label", "Open account settings");
      profile.innerHTML = `<span class="os-profile-avatar">${initials}</span><span class="os-profile-copy"><strong>${escapeHtml(name)}</strong><small>Profile</small></span><span class="os-profile-chevron">⌄</span>`;
    ...
    const sponsored = [...document.querySelectorAll(".os-dashboard .os-section-title")].find(
      (el) => el.textContent?.trim() === "Sponsored provider match",
    );
    if (sponsored) {
      sponsored.textContent = "Promotion slot";
      sponsored
        .closest("section")
        ?.querySelector(".os-opportunity strong")
        ?.replaceChildren(document.createTextNode("Paid visibility appears here when eligible"));
    }
    const safety = document.querySelector(".os-safety");
    if (safety) {
      const heading = safety.querySelector("h3");
      if (heading) heading.textContent = "Ready";
      const copy = safety.querySelector("p");
      if (copy)
        copy.textContent =
          "Your safety tools, check-ins and trusted-contact controls stay available here when you choose to use them.";
    }
    ...
    if (activity && activity.dataset.staticPlaceholder !== "1") {
      ...
      activity.parentElement
        ?.querySelector(".os-feature-action span:first-child")
        ?.replaceChildren(document.createTextNode("View activity"));
    }
    if (!document.querySelector("script[data-kurukoo-os-live]")) {
      const script = document.createElement("script");
      script.src = "/js/kurukoo-os-live-hydration.js?v=1";
      script.defer = true;
      script.dataset.kurukooOsLive = "";
