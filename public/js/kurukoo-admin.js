(() => {
  ...
  const render = (text) => {
    if (queue)
      queue.innerHTML = `<div class="k-row"><span>${esc(text)}</span><span class="k-muted">Authenticated admin API</span></div>`;
  };
  ...
  const readinessLabel = (state) =>
    ({
      ready: "Ready",
      activation_required: "Needs activation",
      device_required: "Needs device verification",
    })[state] || "Unavailable";
  const moduleLabel = (state) =>
    ({
      connected: "Connected",
      readiness: "Readiness",
      "legacy-surface": "Existing admin surface",
    })[state] || "Review";
  const pilotLabel = (state) =>
    ({
      READY: "Ready",
    ...
    state ||
    "Unknown";
  const scaleLabel = (state) =>
    ({
      READY: "Ready",
      PENDING: "Pending validation",
      REQUIRED: "Required before scale",
      NOT_CONFIGURED: "Not configured",
    })[state] ||
    state ||
    "Unknown";
  ...
  const renderSurfaces = (groups) => {
    if (!surfaces) return;
    surfaces.innerHTML = (groups || [])
      .map((group) => {
        const ready = group.surfaces.filter((surface) => surface.readiness === "ready").length;
        const activation = group.surfaces.filter(
          (surface) => surface.readiness === "activation_required",
        ).length;
        const device = group.surfaces.filter(
          (surface) => surface.readiness === "device_required",
        ).length;
  ...
  const renderList = (items) =>
    `<div class="k-list">${items.map(([label, value]) => `<div class="k-row"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join("")}</div>`;
  const renderOperationalQueue = (items) => {
    ...
    queue.innerHTML = rows.length
      ? rows
          .map(
            (item) =>
              `<a class="k-row admin-queue-row admin-queue-${esc(item.state)}" href="${esc(item.href)}"><span><strong>${esc(item.label)}</strong><small>${esc(item.description)}</small></span><span class="k-status">${esc(item.count)} · ${esc(item.state === "clear" ? "Clear" : item.state === "blocked" ? "Activation boundary" : item.state === "pending" ? "Pending" : "Needs review")}</span></a>`,
          )
          .join("")
      : '<div class="k-row"><span>No operational items are currently projected.</span><span class="k-muted">Canonical admin API</span></div>';
  };

  const renderPilotReadiness = (readiness) => {
    if (!readiness || typeof readiness !== "object")
      return '<p class="admin-muted">Canonical dependency readiness is unavailable.</p>';
    const categories = Object.entries(readiness);
    return `<div class="admin-readiness-grid">${categories
      .map(([category, entries]) => {
        const rows = Object.entries(entries || {});
        return `<section class="admin-module"><div class="admin-module-head"><div><span class="k-muted">${esc(category)}</span><h3>${esc(category.replaceAll("_", " "))}</h3></div><span class="k-status">${rows.length} checks</span></div><div class="admin-module-list">${rows
          .map(([name, value]) => {
  ...
  const loadPlatform = async () => {
    try {
      ...
      setStat(
        "stat-integrations",
        `${data.integrations?.implemented ?? 0}/${data.integrations?.total ?? 0}`,
      );
      setStat(
        "stat-active-integrations",
        `External active ${data.integrations?.externallyActive ?? 0}`,
      );
      ...
      if (contract)
        contract.textContent = `${data.contractVersion || "admin-platform"} · ${new Date(data.generatedAt).toLocaleTimeString()}`;
      if (summary && data.readinessSummary)
        summary.textContent = `${data.readinessSummary.ready} ready · ${data.readinessSummary.activation_required} activation · ${data.readinessSummary.device_required} device`;
      ...
      if (surfaces)
        surfaces.innerHTML =
          '<p class="admin-error">Client surface state could not be loaded. Admin authentication and canonical API health are required.</p>';
      if (modules)
        modules.innerHTML = '<p class="admin-error">Admin module registry could not be loaded.</p>';
      throw error;
    }
  };
  ...
  const renderReadiness = async (title, description, rows) => {
    ...
    try {
      ...
      document.getElementById("section-data").innerHTML =
        `<p class="admin-error">Readiness unavailable: ${esc(error.message)}</p>`;
    }
  };
  ...
  const renderSection = async () => {
    ...
    if (section === "connectors") {
      try {
        const data = await loadJson("/api/admin/platform/overview");
        panel.innerHTML = `<div class="card"><h2>Channels & integrations</h2><p class="muted">Implementation, activation and device readiness are displayed separately.</p>${renderList(
          [
            ["Integrations represented", data.integrations?.total ?? 0],
            ["Implemented", data.integrations?.implemented ?? 0],
            ["Externally active", data.integrations?.externallyActive ?? 0],
            ["PWA", data.surfaces?.find((x) => x.family === "pwa")?.surfaces?.length ?? 0],
            [
              "Native surfaces",
              data.surfaces?.find((x) => x.family === "native")?.surfaces?.length ?? 0,
            ],
          ],
        )}</div>`;
      } catch (error) {
        panel.innerHTML = `<div class="card"><h2>Channels & integrations</h2><p class="admin-error">${esc(error.message)}</p></div>`;
      }
      return;
    }
    if (section === "providers") {
      await renderReadiness(
        "Provider readiness",
        "Canonical provider entities, trust evidence and channel state. External fulfilment is never inferred from implementation alone.",
        (data) => {
          const evidence = Object.entries(data.channelEvidence || {}).reduce(
            (sum, [, statuses]) =>
              sum + Object.values(statuses || {}).reduce((n, value) => n + Number(value || 0), 0),
            0,
          );
          return [
    ...
    if (section === "compliance" || section === "settings") {
      await renderReadiness(
        section === "compliance" ? "Compliance & safety" : "Platform settings",
        section === "compliance"
          ? "Canonical safety, trust, privacy and activation gates."
          : "Non-secret feature and activation state. Credential values are never exposed.",
        (data) =>
          section === "compliance"
            ? [
                ...
                [
                  "Private number masking",
                  data.featureFlags?.privateNumberMasking ? "Enabled" : "Disabled",
                ],
            ...
            : [
                ...
                [
                  "Trusted devices",
                  `${data.trustedDevices?.active ?? 0} active / ${data.trustedDevices?.revoked ?? 0} revoked`,
                ],
                ["Trust challenges pending", data.trustChallenges?.pending ?? 0],
                [
                  "Local model state",
                  data.brain?.localModel?.mode ??
                    data.brain?.localModel?.executionMode ??
                    "Unknown",
                ],
                [
                  "Context arbitration telemetry",
                  data.brain?.contextArbitration ? "Available" : "Unavailable",
                ],
    ...
    if (section === "notifications") {
      try {
        const data = await loadJson("/api/admin/platform/overview");
        panel.innerHTML = `<div class="card"><h2>Notifications & delivery</h2><p class="muted">Queue state is owned by the canonical notification service; delivery is not marked complete without evidence.</p>${renderList(
          [
            ["Pending", data.counts?.notificationsPending ?? 0],
            [
              "Queue mode",
              data.notifications?.mode ?? data.notifications?.executionMode ?? "internal",
            ],
            ["Push approval", data.notifications?.pushApproval ?? "deployment-dependent"],
            ["Delivery truth", "Evidence-gated"],
          ],
        )}</div>`;
      } catch (error) {
        panel.innerHTML = `<div class="card"><h2>Notifications & delivery</h2><p class="admin-error">${esc(error.message)}</p></div>`;
      }
      return;
    }
    if (section === "economic") {
      try {
        const data = await loadJson("/api/admin/platform/overview");
        panel.innerHTML = `<div class="card"><h2>Economic requests</h2><p class="muted">Requests, orders, disputes and payment state remain owned by their canonical services.</p>${renderList(
          [
            ["Open requests", data.counts?.openRequests ?? 0],
            ["Orders", data.counts?.orders ?? 0],
            ["Open disputes", data.counts?.disputes ?? 0],
            ["Payment state", "Canonical payment service"],
            ["Escrow / payout", "External activation required"],
            ["Audit", "Required"],
          ],
        )}</div>`;
      } catch (error) {
        panel.innerHTML = `<div class="card"><h2>Economic requests</h2><p class="admin-error">${esc(error.message)}</p></div>`;
      }
      return;
    }
    if (section === "moderation" || section === "conversations" || section === "seo") {
      const titles = {
        moderation: [
          "Moderation & safety",
          "Safety gates, curation and evidence remain canonical.",
        ],
        conversations: [
          "Conversations",
          "The canonical conversation service owns user/agent history and state.",
        ],
        seo: ["SEO", "SEO remains an operator-managed surface backed by the existing SEO service."],
      };
