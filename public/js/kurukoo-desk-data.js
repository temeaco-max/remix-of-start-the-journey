(() => {
  "use strict";
  if (
    document.body?.dataset.workspaceSection !== "desk" &&
    document.body?.dataset.appSection !== "desk"
  )
    return;
  ...
  const humanize = (value) =>
    String(value || "")
      .replaceAll("_", " ")
      .replace(/\b\w/g, (l) => l.toUpperCase()) || "Not available";
  ...
  const exactRequestHref = (request) => {
    ...
    return id
      ? exactChatHref({
          prompt: "Open this request.",
          objectType: "economic_request",
          objectId: id,
          canonicalAction: "economic_request.open",
          conversationId: request?.conversationId || request?.conversation_id,
        })
      : "/chat?prompt=Continue%20this%20request.";
  };
  ...
  const exactReminderHref = (reminder) => {
    ...
    return id
      ? exactChatHref({
          ...
          conversationId: reminder?.conversationId || reminder?.conversation_id,
        })
      : "/reminders";
  };

  const actionNeededStatuses = new Set([
  ...
  const setModuleList = (module, rows, emptyTitle, emptyCopy, emptyHref) => {
    ...
    if (!rows.length) {
      setModuleState(
        ...
        emptyTitle,
        emptyCopy,
        emptyHref ? { label: "Open →", href: emptyHref } : null,
      );
      return;
    }
    const list = document.createElement("div");
    list.className = "k-desk-live-list";
    rows.forEach(({ label, href, detail, status }) =>
      list.appendChild(flowRow(label, href, detail, status)),
    );
  ...
  const notificationHref = (notification) => {
    ...
    const conversationId = String(
      notification?.conversationId || notification?.conversation_id || "",
    ).trim();
    ...
    if (mapped && id)
      return exactChatHref({
        prompt: "Open this update.",
        objectType: mapped[0],
        objectId: id,
        canonicalAction: mapped[1],
        conversationId,
      });
    return conversationId
      ? exactChatHref({ prompt: "Open this update.", conversationId })
      : "/notifications";
  };
  ...
  const goalActivityLabel = (event) => {
    ...
    if (kind === "tool_call" && status === "started")
      return "Kurukoo is carrying out the next step";
    if (kind === "tool_call" && status === "completed")
      return "Kurukoo completed a step and checked the result";
    if (kind === "authorization")
      return status === "satisfied"
        ? "Permission checks passed"
        : "Permission is required before continuing";
    if (kind === "evidence") return "Supporting evidence was recorded";
    if (kind === "outcome")
      return status === "completed"
        ? "Kurukoo verified the recorded outcome"
        : `Kurukoo updated this objective: ${humanize(status)}`;
    if (kind === "quality_evaluated")
      return status === "pass"
        ? "Kurukoo checked the recorded outcome"
        : "Kurukoo paused safely while it checks the outcome";
  ...
  const safeBlockedByLabel = (blockedBy) => {
    const safe = blockedBy
      ...
      .filter(
        (value) =>
          !/(?:^goal:|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|token|secret|prompt|argument|payload)/i.test(
            value,
          ),
      );
    return safe.length
      ? `Waiting on: ${safe.slice(0, 2).join(", ")}`
      : blockedBy.length
        ? "Waiting on a prerequisite objective"
        : "";
  };
  ...
  const safeNextActionLabel = (nextAction) => {
    ...
    if (!value) return "";
    if (
      /(?:goal:|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|token|secret|prompt|argument|payload|canonical|capability|runtime|executor|worker|trace)/i.test(
        value,
      )
    )
      return "Kurukoo will continue this objective when it is ready.";
    return value;
  };
  ...
  const goalRow = (goal, continuation, trace, order = 0) => {
    ...
    const blockers = Array.isArray(continuation?.blockedBy)
      ? continuation.blockedBy.filter(Boolean).slice(0, 2)
      : [];
    const detail = [
      goalPresenceLabel(goalStatus),
      safeBlockedByLabel(blockers) ||
        (safeNextActionLabel(continuation?.nextAction)
          ? `Next: ${safeNextActionLabel(continuation?.nextAction)}`
          : ""),
      latest ? `Latest: ${goalActivityLabel(latest)}` : "",
      latest?.createdAt
        ? formatDate(latest.createdAt)
        : formatDate(goal?.updatedAt || goal?.updated_at),
    ]
      .filter(Boolean)
      .join(" · ");
    return {
      ...
      detail,
      href: exactChatHref({
        prompt: "Open this objective.",
        objectType: "agent_goal",
        objectId: String(goal?.id || ""),
        canonicalAction: "agent.goal.review",
        conversationId:
          goal?.conversationId || goal?.conversation_id || continuation?.conversationId,
      }),
    };
  };
  ...
  const load = async () => {
    ...
    const requests = requestsR.ok
      ? Array.isArray(requestsR.value?.requests)
        ? requestsR.value.requests
        : Array.isArray(requestsR.value)
          ? requestsR.value
          : []
      : [];
    const tasks = tasksR.ok
      ? Array.isArray(tasksR.value)
        ? tasksR.value
        : Array.isArray(tasksR.value?.tasks)
          ? tasksR.value.tasks
          : []
      : [];
    const notifications = notificationsR.ok
      ? Array.isArray(notificationsR.value?.notifications)
        ? notificationsR.value.notifications
        : Array.isArray(notificationsR.value)
          ? notificationsR.value
          : []
      : [];
    const reminders = remindersR.ok
      ? Array.isArray(remindersR.value?.reminders)
        ? remindersR.value.reminders
        : Array.isArray(remindersR.value)
          ? remindersR.value
          : []
      : [];
    const goals = goalsR.ok
      ? Array.isArray(goalsR.value?.goals)
        ? goalsR.value.goals
        : Array.isArray(goalsR.value)
          ? goalsR.value
          : []
      : [];
    ...
    const visibleGoalStatuses = new Set([
      "active",
      "waiting",
    ...
    const goalDetails = goals
      ...
      .sort(
        (a, b) =>
          (goalStatusPriority[a.status] ?? 99) - (goalStatusPriority[b.status] ?? 99) ||
          a.order - b.order,
      )
      .slice(0, 4);
    if (goalsR.ok) {
      for (const { goal } of goalDetails) {
        ...
        if (!goalId) continue;
        const continuationR = await settle(
          api(`/api/agent/goals/${encodeURIComponent(goalId)}/continuation`),
        );
        const traceR = await settle(
          api(`/api/agent/goals/${encodeURIComponent(goalId)}/trace?limit=3`),
        );
        if (continuationR.ok && continuationR.value?.continuation)
          goalContinuations.set(goalId, continuationR.value.continuation);
        if (traceR.ok && Array.isArray(traceR.value?.trace))
          goalTraces.set(goalId, traceR.value.trace);
      }
    }
    const goalRows = goals.map((goal, order) =>
      goalRow(
        goal,
        goalContinuations.get(String(goal?.id || "")),
        goalTraces.get(String(goal?.id || "")),
        order,
      ),
    );
    ...
    notifications
      ...
      .forEach((n) => {
        ...
        attention.push({
          ...
          detail: [
            humanize(n.surface || n.object_type || "Kurukoo"),
    ...
    requests.forEach((req) => {
      ...
      const detail = [
        ...
        req.updated_at || req.updatedAt ? formatDate(req.updated_at || req.updatedAt) : "",
      ]
        .filter(Boolean)
        .join(" · ");
      if (actionNeededStatuses.has(status)) {
        ...
        cont.push({
          label,
          detail,
          href: id ? `/requests/${encodeURIComponent(id)}` : "/requests",
        });
      }
    });
    ...
    reminders.forEach((rem) => {
      ...
      const detail = [overdue ? "Overdue" : humanize(status), formatDate(due)]
        .filter(Boolean)
        .join(" · ");
    ...
    goalRows.forEach((row) => {
      ...
      else if (["active", "waiting", "waiting_on_dependency"].includes(row.status))
        progress.push(row);
      else if (["paused", "completed", "cancelled", "failed", "expired"].includes(row.status))
        cont.push(row);
    });
    ...
    if (todayList) {
      ...
      const hierarchy = [
        ...attention
          .slice(0, 4)
          .map((row) => ({ ...row, label: `Needs attention · ${row.label}` })),
      ...
      if (hierarchy.length) {
        hierarchy.forEach((row) =>
          todayList.appendChild(flowRow(row.label, row.href, row.detail, row.status)),
        );
      } else {
    ...
    if (!goalsR.ok) {
      setModuleState(
        agentObjectives,
        "unavailable",
        "Agent objective state unavailable",
        "Kurukoo could not read your canonical objective activity.",
      ...
      setModuleList(
        agentObjectives,
        visibleGoalRows.slice(0, 6),
        "No active Agent objectives",
        "No Agent objective is currently active, waiting, paused, blocked or awaiting your input.",
        "/agents",
      );
    }
    ...
    if (!requestsR.ok) {
      setModuleState(
        ...
        "Canonical Economic Request authority could not be read.",
        { label: "Retry Requests", href: "/requests" },
      );
    } else {
      ...
      setModuleList(
        requestCard,
        [...actionRows, ...progressRows],
        "No active requests",
        "Start in Chat when you are ready to source, compare or continue a request.",
        "/chat",
      );
    }
    ...
    if (!tasksR.ok && !remindersR.ok) {
      setModuleState(
        ...
        "Task authority could not be read.",
        { label: "Retry Tasks", href: "/tasks" },
      );
    } else {
        ...
        detail:
          {
            ...
            cancelled: "Cancelled",
            expired: "Expired",
          }[String(task.status || "available").toLowerCase()] ||
          humanize(task.status || "available"),
        href: exactTaskHref(task),
      }));
      reminders
        .filter(
          (r) => !["cancelled", "completed", "done"].includes(String(r.status || "").toLowerCase()),
        )
        .slice(0, 4)
        .forEach((rem) => {
          rows.push({
      ...
      setModuleList(
        taskModule,
        rows,
        "No task work waiting",
        "No available or in-progress tasks for this identity.",
        "/tasks",
      );
    }
    ...
    if (!notificationsR.ok) {
      setModuleState(
        ...
        "Notification authority could not be read.",
      ...
      setModuleList(
        ...
        rows,
        "No live pulse",
        "Current notifications remain available from their canonical source.",
        "/notifications",
      );
    }
    ...
    if (activity) {
      ...
      if (metrics) {
        const openRequests = requests.filter(
          (r) =>
            !["completed", "cancelled", "failed", "abandoned"].includes(
              String(r.status || "").toLowerCase(),
            ),
        ).length;
        const openTasks = tasks.filter(
          (t) =>
            !["completed", "approved", "cancelled", "expired"].includes(
        ...
        if (cells[2])
          cells[2].innerHTML = `<span>Agent goals</span><strong>${activeGoals}</strong>`;
      }
    }
