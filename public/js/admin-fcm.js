(() => {
  const boot = async () => {
    const main = document.querySelector(
      ".admin-main, .k-main, main, .control-room, .admin-content",
    );
    if (!main || document.getElementById("admin-fcm-readiness")) return;
    try {
      const response = await fetch("/api/admin/fcm/readiness", {
        ...
        try {
          ...
          if (status)
            status.textContent = payload.accepted
              ? "Firebase accepted the message for delivery."
              : "Message queued/not accepted; inspect delivery state.";
        } catch (error) {
          if (status)
            status.textContent = `Test failed: ${error instanceof Error ? error.message : "unknown error"}`;
        }
      });
    } catch (error) {
