(() => {
  const boot = async () => {
    ...
    if (!section) return;
    try {
      const response = await fetch("/data/kurukoo-page-architecture.json", {
      ...
      const container =
        document.querySelector(".k-app-container") || document.querySelector(".k-app-main");
      ...
      const chipList = (items, className) =>
        (Array.isArray(items) ? items : [])
          .map(
            (item) => `<span class="k-page-architecture-chip ${className}">${escape(item)}</span>`,
          )
          .join("");
      const sectionRows = (Array.isArray(model.sections) ? model.sections : [])
        .map(
          (item, index) =>
            `<div class="k-page-architecture-row"><span class="k-page-architecture-index">${String(index + 1).padStart(2, "0")}</span><span>${escape(item)}</span></div>`,
        )
        .join("");
