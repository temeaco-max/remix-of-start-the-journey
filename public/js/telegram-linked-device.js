(() => {
    ...
    showError(
      status?.lastError ||
        (state === "not_configured"
          ? "Configure TELEGRAM_API_ID, TELEGRAM_API_HASH, the owner phone, and both Telegram linked-device gates before pairing."
          : ""),
    );
  }
  ...
  async function load() {
    try {
      const response = await fetch("/api/telegram-linked-device/status", {
        credentials: "same-origin",
        cache: "no-store",
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.error || "Telegram linked-device status is unavailable.");
      apply(data);
    } catch (error) {
  ...
  async function action(path) {
    showError("");
    try {
      const response = await fetch(`/api/telegram-linked-device/${path}`, {
        method: "POST",
        credentials: "same-origin",
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok)
        throw new Error(data.error || "Telegram linked-device action was not accepted.");
