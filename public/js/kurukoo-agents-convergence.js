(() => {
  ...
  const live = (message) => {
    let region = document.querySelector("[data-agents-live]");
    if (!region) {
      region = document.createElement("div");
      region.className = "k-sr-only";
      region.dataset.agentsLive = "";
  ...
  const goalPresenceLabel = (status) =>
    ...
    })[String(status || "").toLowerCase()] || "Updating";
  const activityLabel = (event) => {
    ...
    if (kind === "outcome")
      return status === "completed"
        ? "Kurukoo verified the outcome."
        : `Kurukoo updated the objective: ${humanize(status)}.`;
  ...
  const render = (goals, continuations = new Map(), traces = new Map()) => {
    ...
    list.innerHTML = goals
      ...
      .map((goal) => {
        ...
        const next = esc(
          status === "paused"
            ? "Work is paused. Resume when you want Kurukoo to continue."
            : status === "needs_user"
              ? "Review the decision Kurukoo needs before work can continue."
              : status === "blocked"
                ? "Review the recorded update before work can continue."
                : safeNextAction(continuation?.nextAction),
        );
        ...
        const href = conversationId
          ...
          : `/chat?prompt=${encodeURIComponent("Continue this objective.")}`;
        const canPause = [
          "active",
          "running",
          "working",
  ...
  list.addEventListener("click", async (event) => {
    ...
    if (
      action === "cancel" &&
      !window.confirm("Cancel this work item? This stops further automatic progress.")
    )
      return;
  ...
  const load = async () => {
    try {
      ...
      await Promise.all(
        goals.slice(0, 25).map(async (goal) => {
          const goalId = String(goal.id || "");
          try {
            const result = await api(`/api/agent/goals/${encodeURIComponent(goalId)}/continuation`);
            if (result?.continuation) continuations.set(goalId, result.continuation);
          } catch {
            /* truthful fallback */
          }
          try {
            const result = await api(
              `/api/agent/goals/${encodeURIComponent(goalId)}/trace?limit=3`,
            );
            if (Array.isArray(result?.trace)) traces.set(goalId, result.trace);
          } catch {
            /* truthful fallback */
          }
        }),
      ...
      if (empty) {
        ...
        if (title) title.textContent = "Objectives are unavailable";
      }
      list.setAttribute("aria-busy", "false");
      list.innerHTML = "";
    }
