(() => {
  ...
  const setStatus = (value) => {
    ...
    dot.dataset.state = value.toLowerCase().replace(/\s+/g, "-");
  };
  ...
  const render = (data) => {
    ...
    start.hidden =
      data.state === "connected" || data.state === "pairing" || data.state === "connecting";
    stop.hidden = !(
      data.state === "pairing" ||
  ...
  const poll = async () => {
    try {
      const response = await fetch("/api/whatsapp-linked-device/status", {
        credentials: "same-origin",
        headers: { Accept: "application/json" },
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to read linked-device status");
      render(data);
    } catch (err) {
  ...
  start.addEventListener("click", async () => {
    ...
    start.disabled = true;
    try {
      const response = await fetch("/api/whatsapp-linked-device/start", {
        method: "POST",
        credentials: "same-origin",
  ...
  stop.addEventListener("click", async () => {
    await fetch("/api/whatsapp-linked-device/stop", { method: "POST", credentials: "same-origin" });
    await poll();
  });
  logout.addEventListener("click", async () => {
    if (!window.confirm("Log out this linked WhatsApp device?")) return;
    await fetch("/api/whatsapp-linked-device/logout", {
      method: "POST",
      credentials: "same-origin",
    });
    await poll();
