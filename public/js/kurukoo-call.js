(() => {
  ...
  const json = async (url, options = {}) => {
    ...
    if (!response.ok)
      throw new Error(
        payload.error || payload.readiness?.activationRequirement || "Call request failed.",
      );
    return payload;
  };
  function readRoomId() {
    ...
    return (
      params.get("room") ||
      params.get("roomId") ||
      (sessionId ? `provider-session:${sessionId}` : `call-${crypto.randomUUID()}`)
    );
  }
  async function statusCheck() {
    ...
    if (!payload?.webrtc?.available)
      throw new Error(
        payload?.webrtc?.activationRequirement ||
          "WebRTC calling is not enabled for this deployment.",
      );
  }
  async function loadIceConfig() {
    ...
    if (!iceServers.length)
      setStatus(
        "Calling is configured for direct browser candidates only; a relay may still be required for some networks.",
      );
  }
  async function createRoom() {
    const payload = await json("/api/webrtc/create", {
      method: "POST",
      body: JSON.stringify({ roomId: readRoomId() }),
    });
    roomId = payload.roomId;
    peerId = payload.phone;
    signalCursor = 0;
    remotePeerId = (payload.peers || []).find((id) => id !== peerId) || null;
  }
  function buildPeerConnection() {
    ...
    pc.onicecandidate = async (event) => {
      ...
      try {
        await sendSignal("ice", event.candidate, remotePeerId);
      } catch (error) {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (state === "connected") setStatus("Call connected.");
        ...
        setStatus(
          "The call connection failed. Try again or use audio-only if your network blocks peer media.",
        );
      else if (state === "disconnected") setStatus("The other participant may have disconnected.");
    };
    return pc;
  }
  ...
  async function sendSignal(kind, payload, to) {
    await json("/api/webrtc/signal", {
      method: "POST",
      body: JSON.stringify({ roomId, kind, payload, ...(to ? { to } : {}) }),
    });
  }
  async function pollSignals() {
    ...
    try {
      const payload = await json(
        `/api/webrtc/signals?roomId=${encodeURIComponent(roomId)}${signalCursor ? `&after=${signalCursor}` : ""}`,
      );
  ...
  async function discoverPeers() {
    const peers = await json(`/api/webrtc/peers?roomId=${encodeURIComponent(roomId)}`);
    remotePeerId = (peers.peers || []).find((id) => id !== peerId) || remotePeerId;
    return Boolean(remotePeerId);
  }
  ...
  async function maybeOffer() {
    ...
    } else if (connection?.textContent !== "connected") setStatus("Connecting…");
  }
  ...
  async function cleanup(sendHangup = false) {
    if (sendHangup && roomId) {
      try {
        await sendSignal("hangup", { reason: "client_disconnect" }, remotePeerId);
      } catch (_) {}
    }
    clearInterval(pollTimer);
    pollTimer = null;
    if (roomId && peerId) {
      try {
        await json("/api/webrtc/leave", { method: "POST", body: JSON.stringify({ roomId }) });
      } catch (_) {}
    }
