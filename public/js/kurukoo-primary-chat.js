(() => {
  ...
  function renderCard(card, messageEl) {
    ...
    else if (card.type === 'artist_booking') holder.innerHTML = '<strong>Verified creator booking</strong><div class="deferred">Representation must be verified before Kurukoo presents a booking for confirmation.</div><span class="escrow-badge">🔒 Escrow Protected</span>';
  ...
  async function loadPoints() { try { const res = await fetch('/api/points/balance', { credentials: 'same-origin' }); if (!res.ok) return; const data = await res.json(); const points = Number(data.points || 0); $('points-balance').querySelector('span').textContent = points; $('inspector-points').textContent = points; } catch {} }
