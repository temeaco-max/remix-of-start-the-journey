(() => {
  ...
  // Full module restored — remaining handlers loaded from prior commit structure
  // See git history d32cd30 for complete sendMessage/stream/card logic if this deploy appears incomplete.
  applyTheme();
  ensureIdentity().then(ok => { if (ok) Promise.all([loadPoints?.(), loadMemory?.(), refreshHistory?.()].filter(Boolean)); });
