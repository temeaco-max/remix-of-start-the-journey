(() => {
  const state = {
    ...
    controller: null,
    theme:
      localStorage.getItem("kurukoo_theme") ||
    ...
    pinnedMessages: [],
    surfaceView: null,
    canonicalContextAction: null,
    resumeCanonicalContextOnLoad: false,
    notifiedNotificationIds: new Set(),
    notifiedTrustChallengeIds: new Set(),
    radarActive: localStorage.getItem("kurukoo_radar_enabled") !== "0",
    radarLive: false,
    lastAgentBriefId: null,
  ...
  function savePinnedMessages() {
    try {
      localStorage.setItem(pinStorageKey(), JSON.stringify(state.pinnedMessages));
    } catch {}
  }
  function loadPinnedMessages() {
    try {
      const parsed = JSON.parse(localStorage.getItem(pinStorageKey()) || "[]");
      state.pinnedMessages = Array.isArray(parsed) ? parsed.slice(0, 12) : [];
    } catch {
  ...
  function renderPinnedMessages() {
    ...
    if (!state.pinnedMessages.length) {
      list.appendChild(
        makeElement("p", "empty-state", "Long-press or right-click a message to pin it here."),
      );
      return;
    }
    state.pinnedMessages.forEach((pin) => {
      const row = makeElement("div", "pinned-message");
      const reference = makeElement("button", "pinned-message-reference", pin.text);
      reference.type = "button";
      reference.title = "Jump to pinned message";
      reference.addEventListener("click", () => {
        const target = chatContent.querySelector(`[data-pin-key="${CSS.escape(pin.key)}"]`);
        if (target) {
          target.scrollIntoView({ behavior: "smooth", block: "center" });
          target.classList.add("message-pinned-focus");
          setTimeout(() => target.classList.remove("message-pinned-focus"), 1200);
        }
      });
      const remove = makeElement("button", "text-btn", "Unpin");
      remove.type = "button";
      remove.addEventListener("click", () => togglePinnedMessage(pin.key));
      row.append(reference, remove);
      list.appendChild(row);
    });
  }
  function togglePinnedMessage(key, message = null) {
    const index = state.pinnedMessages.findIndex((pin) => pin.key === key);
    if (index >= 0) state.pinnedMessages.splice(index, 1);
    else if (message)
  ...
  function wirePinGestures(wrap, role, text) {
    const key =
      wrap.dataset.pinKey ||
      (wrap.dataset.messageId
        ? `message-${wrap.dataset.messageId}`
        : `local-${crypto.randomUUID()}`);
    wrap.dataset.pinKey = key;
    const toggle = () => togglePinnedMessage(key, { role, text });
    let timer = null;
    ...
    wrap.addEventListener("pointerdown", (event) => {
      if (event.pointerType !== "touch" || event.target.closest("button,a,input,textarea")) return;
      timer = setTimeout(() => {
        timer = null;
        toggle();
      }, 560);
  ...
  function updateSurfaceHeader(view = null) {
    const title = $("header-context-title");
    const back = $("surface-header-back");
    if (title) title.textContent = view ? surfaceTitles[view] || "Workspace" : "Agent";
    if (back) {
      back.hidden = !view;
      back.setAttribute(
        "aria-label",
        view ? `Back from ${surfaceTitles[view] || "workspace"}` : "Back to conversation",
      );
    }
  }
  function updateSurfaceContext(view = null) {
    ...
    if (title) title.textContent = view ? surfaceTitles[view] || "Context" : "Context";
    inspector.setAttribute(
      "aria-label",
      view ? `${surfaceTitles[view] || "Workspace"} context` : "Conversation context",
    );
    ...
    cards.forEach((card) => {
      const tokens = String(card.dataset.contextCard || "")
        .split(/\s+/)
        .filter(Boolean);
      const relevant =
        card.id === "agent-goal-card"
          ? !view ||
            (["tasks", "requests", "reminders"].includes(view) &&
              card.dataset.goalAvailable === "true")
  ...
  function leaveWorkspaceSurface() {
    ...
    updateSurfaceHeader(null);
    updateSurfaceContext(null);
    chatContent.replaceChildren();
    if (state.messages.length) renderMessages(state.messages);
    else renderWelcome();
  }
  const guestSurfaceCopy = {
    requests: [
      "Your requests stay connected here.",
      "Start with the conversation and Kurukoo will keep the request, clarification and next step together.",
    ],
    reminders: [
      "Keep important follow-ups connected.",
      "Ask Kurukoo to remind you about something and it will appear here after the reminder is created.",
    ],
    saved: [
      "Save useful options for later.",
      "Offers, discoveries and request context you choose to keep will appear here.",
    ],
    cart: [
      "Review sourced items before checkout.",
      "Choose a seller offer in Chat first. Nothing is purchased or reserved until the canonical checkout boundary confirms it.",
    ],
    points: [
      "Your Points balance",
      "Points and eligible activity are shown here only when the account and deployment state make them available.",
    ],
    tasks: [
      "Work that needs your attention.",
      "Tasks and follow-ups will appear here as Kurukoo coordinates them. Guest conversations can continue in Chat without losing their context.",
    ],
    "daily-picks": [
      "Useful suggestions, clearly labelled.",
      "Daily Picks combines relevant activity and approved sponsored content. It never presents an advertisement as a personal recommendation without disclosure.",
    ],
    discover: [
      "Find useful things around you.",
      "Explore services, opportunities, events and places through source-attributed discovery. A discovered entity is not treated as a Kurukoo provider until it is claimed and verified.",
    ],
    connect: [
      "Connect your access points.",
      "Set up WhatsApp, Telegram and other channels for the same Kurukoo relationship. A channel is not described as connected until provider evidence confirms it.",
    ],
    topics: [
      "Useful community context.",
      "Read and contribute local questions, reports and experiences. Topics provide context; they do not confirm provider identity, price, stock or availability.",
    ],
    memory: [
      "Your retained context stays under your control.",
      "Memory becomes available after you establish your account. You can review and remove active facts without changing conversation history.",
    ],
    safety: [
      "Safety and check-ins.",
      "Manage safety context and choose the next action. Kurukoo is not an emergency service and will not imply that a contact was notified without delivery evidence.",
    ],
    settings: [
      "Your Kurukoo settings.",
      "Account, privacy, notifications, memory, security and channel preferences are managed here.",
    ],
    topup: [
      "Top up without surprises.",
      "Review available balance actions. A payment is never described as complete without provider evidence.",
    ],
    subscription: [
      "Review your subscription path.",
      "Plans, eligibility and payment boundaries are shown here without claiming a completed subscription.",
    ],
  };
  function renderGuestSurface(view) {
    const copy = guestSurfaceCopy[view] || [
      "Continue in Chat.",
      "Tell Kurukoo what you need and it will keep the relevant context connected.",
    ];
    const panel = makeElement("section", "workspace-panel surface-guest-panel");
    const heading = makeElement("div", "panel-heading");
    heading.append(
      makeChildren("div", "", [
        makeElement("span", "workspace-eyebrow", surfaceTitles[view] || "Workspace"),
        makeElement("h2", "", copy[0]),
      ]),
    );
    ...
    const ask = makeElement("button", "workspace-button secondary", "Ask Kurukoo");
    ask.type = "button";
    ask.addEventListener("click", () => input?.focus());
    const back = makeElement("button", "workspace-button secondary", "Back to conversation");
    back.type = "button";
    back.addEventListener("click", leaveWorkspaceSurface);
    actions.append(ask, back);
  ...
  async function renderWorkspaceSurface(view) {
    ...
    updateSurfaceHeader(view);
    updateSurfaceContext(view);
    loadNearbyInspector(view);
    const surface = makeElement("section", "workspace-surface");
    surface.dataset.surfaceView = view;
    const heading = makeElement("div", "surface-heading");
    heading.append(makeElement("h1", "", surfaceTitles[view] || "Workspace"));
    {
      const ask = makeElement("button", "workspace-button secondary ask-cta");
      ask.type = "button";
      const mark = makeElement("span", "ask-mark");
      const image = document.createElement("img");
      image.src = "/assets/brand/logo-icon.png";
      image.alt = "";
      image.setAttribute("aria-hidden", "true");
      mark.append(image);
      ask.append(mark, makeElement("span", "", "Ask"));
      ask.addEventListener("click", () => input?.focus());
      heading.append(ask);
    }
    surface.append(heading);
    const body = makeElement("div", "surface-body");
    body.append(makeElement("div", "surface-loading", "Loading…"));
    surface.append(body);
    chatContent.replaceChildren(surface);
    scroll.scrollTop = 0;
    try {
      if (view === "points") {
        const res = await fetch("/api/points/balance", { credentials: "same-origin" });
        const data = await res.json().catch(() => ({}));
        body.replaceChildren(
          makeElement("div", "surface-stat-card", `${Number(data.points || 0)} Points`),
          makeElement(
            "p",
            "",
            "Points balance is shown here without leaving the conversation workspace.",
          ),
        );
        return;
      }
      if (view === "cart") {
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data.error || "Cart is unavailable");
        const items = Array.isArray(data.items) ? data.items : [];
        const panel = makeElement("section", "workspace-panel");
        panel.appendChild(
          makeChildren("div", "panel-heading", [
            makeChildren("div", "", [
              makeElement("span", "workspace-eyebrow", "Review cart"),
              makeElement(
                "h2",
                "",
                items.length ? "Offers you chose to review" : "Your review cart is empty",
              ),
            ]),
          ]),
        );
        if (!items.length)
          panel.appendChild(
            makeElement(
              "p",
              "empty-state",
              "Choose a sourced seller offer in Chat before starting checkout.",
            ),
          );
        else {
          const list = makeElement("div", "storefront-review-list");
          items.forEach((item) => {
            const row = makeElement("article", "storefront-review-item");
            const copy = makeElement("div");
            copy.append(
              makeElement("strong", "", String(item.title || "Seller offer")),
              makeElement(
                "small",
                "",
                `${String(item.quantity || 1)} × ${item.price_minor === null ? "Price pending confirmation" : `${item.price_minor} ${item.currency || "local currency"}`}`,
              ),
            );
            const remove = makeElement("button", "sf-btn sf-secondary", "Remove");
            remove.type = "button";
            remove.addEventListener("click", async () => {
              await fetch(`/api/cart/items/${encodeURIComponent(item.id)}`, {
                method: "DELETE",
                credentials: "same-origin",
              });
              await renderWorkspaceSurface("cart");
            });
            row.append(copy, remove);
            list.appendChild(row);
          });
          panel.appendChild(list);
          const checkout = makeElement(
            "button",
            "sf-btn sf-primary",
            "Continue to checkout review",
          );
          checkout.type = "button";
          const status = makeElement("p", "empty-state");
          checkout.addEventListener("click", async () => {
            ...
            status.textContent = resultData.message || resultData.error || "Checkout needs review.";
      ...
      if (view === "memory") {
        ...
        heading.append(
          makeChildren("div", "", [
            makeElement("span", "workspace-eyebrow", "Your retained context"),
            makeElement(
              "h2",
              "",
              facts.length ? "Review what Kurukoo may use" : "Nothing is retained yet",
            ),
          ]),
        );
        ...
        panel.appendChild(
          makeElement(
            "p",
            "workspace-note",
            "These are active fact-level memories connected to your account. Removing one stops it from active retrieval; it does not change your conversation history.",
          ),
        );
        if (!facts.length)
          panel.appendChild(
            makeElement(
              "p",
              "empty-state",
              "Tell Kurukoo what you would like it to remember, or continue your conversation without saving a preference.",
            ),
          );
        else {
          ...
          facts.forEach((fact) => {
            ...
            const memorySource =
              {
                user_declared: "You asked Kurukoo to remember this.",
                verified: "Recorded as confirmed information.",
                observed: "Recorded from an observed signal.",
                inferred: "Learned from context; remove it if it is not right.",
                system_generated: "Saved by Kurukoo for your review.",
              }[String(fact.provenance || "").toLowerCase()] ||
              "Saved in your Memory Profile for review.";
            copy.append(
              makeElement(
                "strong",
                "",
                `${String(fact.field || "Context").replace(/_/g, " ")}: ${String(fact.value || "")}`,
              ),
              makeElement("small", "", memorySource),
            );
            ...
            remove.addEventListener("click", async () => {
              ...
              try {
                const result = await fetch(`/api/memory/facts/${encodeURIComponent(fact.id)}`, {
                  method: "DELETE",
                  credentials: "same-origin",
                });
                const resultData = await result.json().catch(() => ({}));
                if (!result.ok)
                  throw new Error(resultData.error || "Memory fact could not be removed.");
                await renderWorkspaceSurface("memory");
              } catch (error) {
                remove.disabled = false;
                setInspectorFeedback(error.message || "Memory fact could not be removed.", "error");
              }
            });
            row.append(copy, remove);
            list.appendChild(row);
      ...
      const source = doc.querySelector(".workspace-content, main, .workspace-main");
      if (!source) throw new Error("Workspace content unavailable");
      body.replaceChildren(...Array.from(source.childNodes).map((node) => node.cloneNode(true)));
      body.querySelector(".workspace-header")?.remove();
      body.querySelector("[data-tasks-activity]") && loadTasksActivity();
      body.querySelectorAll("script").forEach((script) => script.remove());
      body.querySelectorAll("a[href]").forEach((link) => {
        const href = link.getAttribute("href") || "";
        const mapped = Object.entries(surfacePaths).find(
          ([, path]) => href === path || href.startsWith(`${path}?`),
        );
        if (mapped) {
          link.dataset.surfaceView = mapped[0];
          link.removeAttribute("href");
        }
      });
      body.querySelectorAll("a.ask-cta").forEach((link) => {
        link.addEventListener("click", (event) => {
          event.preventDefault();
          const prompt = new URL(
            link.href || link.getAttribute("data-href") || "/chat",
            location.origin,
      ...
      wireSurfaceActions(body);
      document.dispatchEvent(
        new CustomEvent("kurukoo:workspace-surface", { detail: { section: view } }),
      );
    } catch (error) {
      body.replaceChildren(renderGuestSurface(view));
  ...
  async function loadConversation(conversationId) {
    ...
    localStorage.setItem("kurukoo_conversation_id", conversationId);
    restoreComposerDraft();
    try {
      const url = new URL("/api/chat/history", location.origin);
  ...
  function wireSurfaceActions(root = document) {
    root.querySelectorAll("[data-surface-view]").forEach((action) => {
      if (action.dataset.surfaceWired === "1") return;
      action.dataset.surfaceWired = "1";
      action.addEventListener("click", (event) => {
        event.preventDefault();
        renderWorkspaceSurface(action.dataset.surfaceView);
      });
    });
  }
  ...
  const setConnection = (ok, text = ok ? "Connected" : "Offline") => {
    ...
    if (el) {
      const pulse = makeElement(
        "span",
        `presence-pulse${state.radarActive && ok ? " is-active" : ""}`,
      );
      pulse.setAttribute("aria-hidden", "true");
      el.replaceChildren(
        pulse,
        makeElement("span", "status-dot"),
        makeElement("span", "", String(text).toLowerCase()),
      );
      el.classList.toggle("offline", !ok);
    }
    const pulse = $("header-presence-pulse");
    if (pulse) pulse.classList.toggle("is-active", state.radarActive && ok);
    const label = $("connection-label");
    if (label) label.textContent = String(text).toLowerCase();
  };
  function setRadarActive(active) {
    state.radarActive = Boolean(active);
    localStorage.setItem("kurukoo_radar_enabled", state.radarActive ? "1" : "0");
    const toggle = $("radar-toggle");
    if (toggle) {
      toggle.setAttribute("aria-pressed", String(state.radarActive));
      toggle.classList.toggle("is-active", state.radarActive);
      toggle.setAttribute(
        "aria-label",
        state.radarActive ? "Nearby Radar ready" : "Turn on Nearby Radar",
      );
    }
    const status = $("radar-status");
    if (status)
      status.textContent = state.radarActive ? (state.radarLive ? "Live" : "Ready") : "Off";
    const pulse = $("header-presence-pulse");
    if (pulse)
      pulse.classList.toggle(
        "is-active",
        state.radarActive && !$("connection-status")?.classList.contains("offline"),
      );
    const header = $("connection-status");
    if (header) header.classList.toggle("radar-active", state.radarActive);
  }
  async function loadPulseReadiness() {
    try {
      ...
      if (data.nudge && !sessionStorage.getItem(nudgeKey)) {
        ...
        pushAgentSurfaceToast(
          data.active ? "Nearby Pulse is live" : "Nearby Radar is ready",
          data.nudge,
          false,
        );
      }
  ...
  let sponsoredAds = Array.isArray(window.KURUKOO_SPONSORED_ADS)
    ? window.KURUKOO_SPONSORED_ADS
    : [];
  let sponsoredIndex = -1;
  function renderSponsoredAd() {
    const card = $("sidebar-promo");
    const image = card?.querySelector("img");
    const tag = card?.querySelector(".sidebar-sponsored-tag");
    const dailyCard = $("daily-picks-promo-card");
    const dailyImage = $("daily-picks-promo-image");
    ...
    image.src = String(ad.image || "");
    image.alt = String(ad.alt || ad.title || "Sponsored local service");
    ...
    if (dailyCard && dailyImage) {
      ...
      dailyImage.src = String(dailyAd.image || "");
      dailyImage.alt = String(dailyAd.alt || dailyAd.title || "Daily Picks community discovery");
  ...
  async function loadSponsoredAds() {
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (res.ok && Array.isArray(data.campaigns) && data.campaigns.length)
        sponsoredAds = data.campaigns;
    } catch {}
    renderSponsoredAd();
    window.setInterval(() => {
      if (document.visibilityState === "visible") renderSponsoredAd();
    }, 6500);
  }
  loadSponsoredAds();
  ...
  function setTypingStatus(status = "complete", label = "") {
    ...
    if (!indicator) {
      ...
      const dots = makeElement("span", "typing-indicator-dots");
      dots.setAttribute("aria-hidden", "true");
      dots.append(makeElement("i"), makeElement("i"), makeElement("i"));
      bubble.append(text, activity, dots);
    ...
    if (text)
      text.textContent =
        label ||
        (status === "thinking"
          ? "Kurukoo is considering the best next step…"
          : "Kurukoo is typing…");
  ...
  function updateNativeAssistanceStatus() {
    const banner = $("native-assistance-status");
    if (!banner) return;
    const now = Date.now();
    const reminders = Array.isArray(state.nativeAssistance?.reminders)
      ? state.nativeAssistance.reminders
      : [];
    const checkIns = Array.isArray(state.nativeAssistance?.checkIns)
      ? state.nativeAssistance.checkIns
      : [];

    const activeCheckIns = checkIns
      .filter((item) => item?.status === "active" && item?.expires_at)
      .map((item) => ({ ...item, expiresAt: new Date(item.expires_at).getTime() }))
      .filter((item) => Number.isFinite(item.expiresAt))
      .sort((a, b) => a.expiresAt - b.expiresAt);
    const expiringCheckIn = activeCheckIns.find(
      (item) => item.expiresAt <= now + 2 * 60 * 60 * 1000,
    );
    if (expiringCheckIn) {
      const when =
        expiringCheckIn.expiresAt <= now
          ? "has reached its scheduled end"
          : `ends at ${new Date(expiringCheckIn.expiresAt).toLocaleString()}`;
      banner.textContent = `Personal safety check-in ${when}. Review it in the context inspector; no contact is notified automatically.`;
      banner.dataset.kind = "safety";
      banner.hidden = false;
      return;
    }

    const upcomingReminder = reminders
      .filter((item) => item?.status === "active" && item?.due_at)
      .map((item) => ({ ...item, dueAt: new Date(item.due_at).getTime() }))
      .filter((item) => Number.isFinite(item.dueAt) && item.dueAt <= now + 24 * 60 * 60 * 1000)
      .sort((a, b) => a.dueAt - b.dueAt)[0];
    if (upcomingReminder) {
      const title = String(upcomingReminder.title || "Reminder");
      const due =
        upcomingReminder.dueAt <= now
          ? "needs your attention now"
          : `is scheduled for ${new Date(upcomingReminder.dueAt).toLocaleString()}`;
      banner.textContent = `Reminder: ${title} ${due}. Review it in the context inspector.`;
      banner.dataset.kind = "reminder";
      banner.hidden = false;
      return;
    }
    banner.hidden = true;
  ...
  function showEmbedSignInGate() {
    ...
    const avatar = makeElement("div", "avatar", "K");
    avatar.setAttribute("aria-hidden", "true");
    const body = makeElement("div", "message-body");
    const bubble = makeElement("div", "bubble");
    const md = makeElement("div", "markdown-body");
    const p1 = document.createElement("p");
    p1.appendChild(makeElement("strong", "", "Continue chatting with Kurukoo"));
    const p2 = document.createElement("p");
    p2.textContent =
      "Your Memory Profile and conversation history stay private until you complete the name, phone and verification conversation in full chat.";
    const p3 = document.createElement("p");
    const a1 = makeElement("a", "primary-btn", "Open full chat");
    a1.href = "/chat";
    a1.target = "_top";
    a1.rel = "noopener";
    p3.append(a1);
  ...
  async function ensureIdentity() {
    const sessionCheck = await fetch("/api/auth/me", { credentials: "same-origin" }).catch(
      () => null,
    );
    if (sessionCheck?.ok) {
      ...
      state.isGuest = false;
      state.displayName =
        sessionData.name || sessionData.user?.name || sessionData.profile?.name || "";
  ...
  async function startGuestAuth() {
    ...
    try {
      const response = await fetch("/api/chat/auth/start", {
        method: "POST",
        credentials: "same-origin",
      });
      ...
      const steps = {
        ...
        awaiting_otp: "otp",
        awaiting_email_phone: "phone",
        awaiting_email_otp: "otp",
      };
  ...
  function setAuthComposerStep(step = "none") {
    ...
    const prompts = {
      ...
      otp: "Type the 6-digit code…",
    };
    input.placeholder =
      prompts[step] ||
      (state.displayName ? "Tell Kurukoo what you need…" : "Tell Kurukoo what you need…");
    input.setAttribute(
      "aria-label",
      step === "name"
        ? "Your name"
  ...
  function addHistoryItem(conversation, active = false) {
    ...
    if (!list || !conversation?.id) return;
    ...
    item.append(makeElement("span", "history-title", conversation.title || "New conversation"));
    try {
      const pins = JSON.parse(localStorage.getItem(`kurukoo_pins_${conversation.id}`) || "[]");
      if (Array.isArray(pins) && pins.length) {
        const pin = makeIcon("saved", "Pinned");
        pin.classList.add("history-pin-indicator");
        pin.setAttribute("aria-label", "Pinned references");
  ...
  function speakAssistantResponse(value, control) {
    ...
    const spoken = Boolean(
      text &&
      output?.isSupported?.() &&
      output.speak(text, { language: document.documentElement.lang || navigator.language }),
    );
    if (control) {
  ...
  function createMessage(role, text = "", id = null, cardData = null, animate = true) {
    ...
    wrap.dataset.messageState = animate ? "incoming" : "history";
    if (animate) wrap.classList.add("message-enter");
    if (id) wrap.dataset.messageId = id;

    const avatarDiv = makeElement("div", "avatar");
    ...
    const buttons =
      role === "assistant"
        ? [
            ["speak", "Speak response", "mic"],
            ["pin", "Pin", "saved"],
            ["copy", "Copy", "copy"],
            ["regenerate", "Retry", "retry"],
            ["delete", "Delete", "trash"],
          ]
        : [
            ["pin", "Pin", "saved"],
    ...
    actions.addEventListener("click", async (event) => {
      ...
      if (action === "copy")
        await navigator.clipboard?.writeText(wrap.querySelector(".bubble").innerText);
      if (action === "speak")
        speakAssistantResponse(wrap.querySelector(".bubble")?.innerText || text, button);
      if (action === "edit") {
        input.value = text;
        input.focus();
  ...
  function appendStreamBubble() {
    $("welcome")?.remove();
    const wrap = document.createElement("article");
    wrap.className = "message assistant message-enter message-streaming";
    wrap.dataset.messageState = "incoming";
    wrap.hidden = true;
    state.currentAssistantArticle = wrap;
    window.KurukooTurnState?.mount?.(wrap);
    window.KurukooTurnState?.setStatus?.(wrap, "working", activityStages[activityStageIndex]);
    ...
    const thinking = makeElement("div", "thinking");
    ...
    details.appendChild(
      makeElement(
        "div",
        "",
        "Kurukoo selected the appropriate response path. Private model reasoning is not exposed.",
      ),
    );
    ...
    actions.addEventListener("click", (event) => {
      ...
      if (button?.dataset.action === "speak")
        speakAssistantResponse(wrap.querySelector(".bubble")?.innerText || "", button);
    });
    chatContent.appendChild(wrap);
  ...
  function addUserMessage(text, id = null) {
    $("welcome")?.remove();
    state.messages.push({ role: "user", text, id });
    return createMessage("user", text, id);
  }
  ...
  function storefrontStageLabel(stage) {
    return storefrontStageLabels[String(stage || "").toLowerCase()] || "Current request update";
  }
  function outcomeSafeCopy(value) {
    return String(value || "")
      ...
      .replace(
        /Preserve the exact owner and conversation context/gi,
        "Keep this work and conversation connected",
      )
      .replace(
        /Find or coordinate an authorised provider through the existing Economic Request path\./gi,
        "Find or coordinate an authorised provider for this request.",
      )
      .replace(
        /Hand off to the existing provider, commerce, booking, or fulfilment owner when local preparation is complete\./gi,
        "Continue through the appropriate confirmed provider path after local preparation.",
      )
      .replace(
        /Prepare a canonical action; execute only when the capability, authorization, and confirmation allow it\./gi,
        "Prepare the next step, and act only after the required permission and confirmation are available.",
      )
  ...
  function setDeferredStatus(card) {
    ...
    if (card.type === "agentic_storefront") {
      if (card.stage === "deferred") {
        status.hidden = false;
        status.textContent =
          "Waiting for: Kurukoo is still looking for a verified option and will retain this request for the next supported update.";
      } else if (card.stage === "fulfillment") {
        status.hidden = false;
        status.textContent =
          "I need you: payment evidence is recorded. Review the result before confirming completion.";
      } else if (
        [
          "slot_fill",
          "quote_review",
        ...
        const prefix = [
          ...
          ? "I need you"
          : ["seller_handover", "delivery_in_progress"].includes(card.stage)
            ? "Waiting for"
            : "I’m working on it";
    ...
    if (!["worker_match", "service_search", "nearby_radar"].includes(card.type)) {
      status.hidden = true;
      return;
    }
    status.hidden = false;
    status.textContent =
      card.type === "nearby_radar"
  ...
  function markStorefrontStepSuperseded(
    messageEl,
    label = "Submitted — see the latest request state below.",
  ) {
    const holder = messageEl?.querySelector(".agentic-storefront");
    if (
      !holder ||
      !holder.querySelector("[data-storefront-field]") ||
  ...
  async function advanceStorefront(requestId, action, fields, messageEl) {
    ...
    try {
      const idempotencyKey =
        `chat:${state.conversationId || "draft"}:${requestId}:${action}:${Object.entries(
          fields || {},
        )
      ...
      const res = executorAction
        ...
        : await fetch(
            `/api/chat/economic-requests/storefront/${encodeURIComponent(requestId)}/advance`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              credentials: "same-origin",
      ...
      if (!res.ok || !data.success) {
        ...
        if (identity?.nextUpgrade === "name") {
          ...
          const message =
            "I have kept this exact request unchanged. Before you select a provider or send a contact request, please enter your name so Kurukoo can keep the action tied to your account.";
          setMarkdown(wrap.querySelector(".markdown-body"), message);
          state.messages.push({ role: "assistant", text: message, id: null });
          if (input) {
            input.placeholder = "Enter your name to continue safely…";
            input.focus();
          }
          return;
        }
        if (
          identity?.nextUpgrade?.kind === "connect_channel" ||
          identity?.nextUpgrade?.kind === "verify_phone"
        ) {
          setConnection(true);
          const wrap = appendStreamBubble();
      ...
      const message = executorAction
        ? data.result?.message || card?.message || "The exact request was updated."
        : card?.message || "Updated.";
      ...
      const wrap = appendStreamBubble();
      setMarkdown(
        wrap.querySelector(".markdown-body"),
        `Could not continue that request. **${escapeText(error.message)}**`,
      );
    } finally {
      state.busy = false;
  ...
  async function startKnownOffer(offerId) {
    ...
    try {
      const res = await fetch(
        `/api/chat/economic-requests/offers/${encodeURIComponent(offerId)}/start`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "same-origin",
          body: JSON.stringify({ conversationId: state.conversationId || undefined }),
        },
      );
      if (res.status === 401) {
        await ensureIdentity();
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success)
        throw new Error(data.error || "Could not start request from that offer");
      ...
      setMarkdown(
        wrap.querySelector(".markdown-body"),
        `Could not select that offer. **${escapeText(error.message)}**`,
      );
    } finally {
      state.busy = false;
      if (send) send.disabled = false;
  ...
  async function selectDeliveryCandidate(requestId, providerPhone) {
    ...
    try {
      const res = await fetch(
        `/api/chat/economic-requests/${encodeURIComponent(requestId)}/delivery-selection`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "same-origin",
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success)
        throw new Error(data.error || "Could not select delivery provider");
      const card = data.card;
      setDeferredStatus(card);
      const wrap = appendStreamBubble();
      setMarkdown(
        wrap.querySelector(".markdown-body"),
        card.message || "Delivery provider selected.",
      );
      renderCard(card, wrap);
      state.messages.push({ role: "assistant", text: card.message || "", id: null });
      scroll.scrollTop = scroll.scrollHeight;
      ...
      setMarkdown(
        wrap.querySelector(".markdown-body"),
        `Could not select that delivery provider. **${escapeText(error.message)}**`,
      );
    } finally {
      state.busy = false;
      if (send) send.disabled = false;
  ...
  function renderAgenticStorefront(card, messageEl) {
    ...
    holder.appendChild(
      makeElement(
        "p",
        "outcome-primitive outcome-situation",
        String(
          card.message ||
            card.description ||
            "Kurukoo is carrying this request through its canonical path.",
        ),
      ),
    );
    const resultEvidence = [];
    if (card.quote && typeof card.quote === "object")
      resultEvidence.push({
        label: "Quote",
        value: `${String(card.quote.amount_minor ?? card.quote.priceMinor ?? "Pending")} ${String(card.quote.currency || "local currency")}`,
      });
    if (card.offer && typeof card.offer === "object")
      resultEvidence.push({
        label: "Offer",
        value: String(card.offer.title || card.offer.description || "Recorded offer"),
      });
    if (card.execution && typeof card.execution === "object")
      resultEvidence.push({
        label: "Execution",
        value: String(card.execution.status || "recorded").replace(/_/g, " "),
      });
    if (card.provider && typeof card.provider === "object")
      resultEvidence.push({
        label: "Provider",
        value: String(card.provider.name || card.provider.label || "Recorded provider"),
      });
    appendOutcomeEvidence(holder, resultEvidence);
    const storefrontStage = String(card.stage || card.status || "working").toLowerCase();
    const storefrontTone = /complete|fulfilled|success/.test(storefrontStage)
      ? "success"
      : /quote|confirm|payment|select|approval/.test(storefrontStage)
        ? "attention"
        : /wait|pending|search|match|in_fulfillment/.test(storefrontStage)
          ? "waiting"
          : "neutral";
    const storefrontLabel =
      storefrontTone === "success"
        ? "Done"
        : storefrontTone === "attention"
          ? "I need you"
          : storefrontTone === "waiting"
            ? "Waiting for"
            : "I’m working on it";
    appendOutcomeStatus(
      holder,
      `${storefrontLabel}: ${storefrontStageLabel(card.stage || card.status)}.`,
      storefrontTone,
    );
    ...
    progressBar.appendChild(
      makeElement("i", `storefront-progress-meter progress-${progressClass}`),
    );
    ...
    if (Array.isArray(card.knownOffers) && card.knownOffers.length) {
      ...
      card.knownOffers.forEach((offer) => {
        ...
        const amount = Number.isInteger(price)
          ? `${price} ${String(offer.currency || "local currency")}`
        ...
        if (offer.availabilityNote)
          details.appendChild(makeElement("small", "", String(offer.availabilityNote)));
        if (offer.provenance || offer.externalSource)
          details.appendChild(
            makeElement(
              "small",
              "",
              `${String(offer.provenance || "source record").replace(/_/g, " ")}${offer.externalSource ? ` · ${String(offer.externalSource)}` : ""}`,
            ),
          );
        ...
        button.addEventListener("click", () => {
          void startKnownOffer(String(offer.id || ""));
        });
        ...
        cartButton.addEventListener("click", async () => {
          ...
          try {
            const response = await fetch("/api/cart/items", {
              ...
              credentials: "same-origin",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ offerId: String(offer.id || ""), quantity: 1 }),
            });
            const data = await response.json().catch(() => ({}));
            if (!response.ok || !data.success)
              throw new Error(data.error || "Could not add this offer to your review cart");
            cartButton.textContent = "Added to cart";
          } catch (error) {
            cartButton.disabled = false;
            cartButton.textContent = error.message || "Try again";
          }
        });
        ...
        if (offer.externalUrl) {
          const external = makeElement(
            "button",
            "sf-btn sf-secondary",
            `View on ${String(offer.externalSource || "partner")}`,
          );
          external.type = "button";
          external.addEventListener("click", async () => {
            await fetch("/api/cart/affiliate-click", {
              method: "POST",
              credentials: "same-origin",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ product: String(offer.id || offer.description || "") }),
            }).catch(() => {});
            window.open(String(offer.externalUrl), "_blank", "noopener");
          });
      ...
      card.deliveryCandidates.forEach((provider) => {
        ...
        details.append(
          ...
          makeElement(
            "span",
            "",
            `Verified option · listed rate ${String(provider.hourly_rate || 0)} ${String(provider.currency || "local currency")}`,
          ),
        );
        const button = makeElement("button", "sf-btn sf-primary", "Choose delivery");
        button.type = "button";
        button.addEventListener("click", () => {
          void selectDeliveryCandidate(card.requestId, String(provider.phone || ""));
        });
    ...
    if (Array.isArray(card.providers) && card.providers.length) {
      ...
      card.providers.forEach((provider, index) => {
        ...
        if (card.requestId && provider.phone) {
          const choose = makeElement(
            "button",
            "sf-btn sf-primary",
            `Choose ${String(provider.name || "provider")}`,
          );
          choose.type = "button";
    ...
    if (card.quote) {
      ...
      quote.appendChild(
        makeElement(
          "strong",
          "",
          `${String(card.quote.amount_minor)} ${String(card.quote.currency || "local currency")}`,
        ),
      );
    ...
    if (card.offer && typeof card.offer === "object") {
      ...
      offer.appendChild(
        makeElement("p", "", String(card.offer.description || "Offer details are unavailable.")),
      );
      const price = Number(card.offer.priceMinor);
      const amount = Number.isInteger(price)
        ? `${price} ${String(card.offer.currency || "NGN")}`
        : "Price pending confirmation";
      offer.appendChild(
        makeElement("span", "storefront-offer-price", `Listed item price: ${amount}`),
      );
      if (card.offer.availabilityNote)
        offer.appendChild(makeElement("small", "", String(card.offer.availabilityNote)));
    ...
    if (Array.isArray(card.participants) && card.participants.length) {
      ...
      card.participants.forEach((participant) => {
        ...
        const roleLabels = {
          ...
          external_platform: "Partner service",
          agent: "Kurukoo",
        };
        const participantStatusLabels = {
          ...
          in_progress: "Progress update recorded",
          delivered: "Delivery reported",
          declined: "Unavailable",
          withdrawn: "No longer available",
        };
        const role = roleLabels[String(participant.role || "")] || "Participant";
        const status =
          participantStatusLabels[String(participant.status || "")] || "Update recorded";
    ...
    if (card.execution && typeof card.execution === "object") {
      ...
      execution.appendChild(
        makeElement(
          "span",
          "storefront-execution-status",
          String(card.execution.status || "pending").replace(/_/g, " "),
        ),
      );
      execution.appendChild(
        makeElement(
          "small",
          "",
          card.execution.connectorId
            ? "Recorded through an authorized service."
            : "No external dispatch is claimed.",
        ),
      );
      if (card.execution.externalReference)
        execution.appendChild(
          makeElement("small", "", "An execution reference has been recorded."),
        );
      if (card.execution.failureReason)
        execution.appendChild(
          makeElement(
            "small",
            "storefront-execution-failure",
            `Dispatch failed: ${String(card.execution.failureReason)}. Manual confirmation is required.`,
          ),
        );
      if (Array.isArray(card.execution.evidence) && card.execution.evidence.length) {
        ...
        card.execution.evidence.forEach((item) => {
          ...
          evidence.appendChild(
            makeElement("li", "", `${String(item.type || "event")} · ${source} · ${state}`),
          );
        });
    ...
    const actions = (Array.isArray(card.actions) ? card.actions : []).filter(
      (action) =>
        !(
          action?.id === "select_provider" &&
          Array.isArray(card.providers) &&
    ...
    if (actions.length) {
      ...
      actions.forEach((action) => {
        ...
        const button = makeElement(
          "button",
          `sf-btn sf-${style}`,
          action.label || action.id || "Continue",
        );
        button.type = "button";
    ...
    if (card.escrowProtected !== false)
      holder.appendChild(makeElement("span", "escrow-badge", "Payment protection recorded"));
  ...
  function renderAssistanceOutcome(card, messageEl) {
    ...
    head.append(
      makeElement(
        "strong",
        "",
        card.mode === "support" ? "Guidance from Kurukoo" : "Useful guidance",
      ),
      ...
      sources.forEach((source) => {
        ...
        details.append(
          ...
          makeElement(
            "span",
            "",
            String(source.excerpt || "Open this attributed source for more context."),
          ),
          makeElement(
            "small",
            "",
            `${String(source.kind || "source").replace(/_/g, " ")} · ${String(source.provenance || "attributed")}`,
          ),
        );
        const params = new URLSearchParams({
          prompt: `Review this ${String(source.kind || "source")} with me`,
          ...(source.kind === "topic"
            ? { topicSlug: String(source.id || "") }
            : { resourceSlug: String(source.id || "") }),
        });
    ...
    const truthLabel =
      truth.sourceAttributed === true &&
      truth.noProviderClaim === true &&
      truth.noExecutionClaim === true
  ...
  function renderOSCollection(card, messageEl) {
    ...
    const title =
      card.type === "notifications"
        ...
        : card.type === "reminders"
          ? "Your reminders"
          : card.type === "reminder_action"
            ? "Reminder review"
            : card.type === "tasks"
              ? "Your tasks"
              : card.type === "task_action"
                ? "Task review"
                : card.type === "memory"
                  ? "Your saved context"
                  : card.type === "request_status"
                    ? "Request status"
                    : card.type === "os_status"
                      ? `${String(card.domain || "OS status").replace(/_/g, " ")} status`
                      : "Kurukoo result";
    ...
    if (card.count !== undefined)
      holder.appendChild(
        makeElement(
          "small",
          "storefront-execution-status",
          `${String(card.count)} stored item${Number(card.count) === 1 ? "" : "s"}`,
        ),
      );
    const list = makeElement("div", "inspector-list");
    const items =
      card.type === "notifications"
        ? Array.isArray(card.notifications)
          ? card.notifications
          : []
        : card.type === "reminders"
          ? Array.isArray(card.reminders)
            ? card.reminders
            : []
          : card.type === "reminder_action"
            ? card.reminder
        ...
        const meta =
          {
            ...
            scheduled: "Scheduled",
            sent: "Reminder time reached",
            cancelled: "Cancelled",
            saved: "Saved",
          }[rawMeta] || rawMeta.replace(/_/g, " ");
        details.append(makeElement("strong", "", String(label)), makeElement("span", "", meta));
        if (item.body || item.description || (card.type === "memory" && item.value))
          details.appendChild(
            makeElement("small", "", String(item.body || item.description || item.value)),
          );
        if (card.type === "memory" && item.provenance) {
          const memorySource =
            {
              user_declared: "You asked Kurukoo to remember this.",
              verified: "Recorded as confirmed information.",
              ...
              inferred: "Learned from context; remove it if it is not right.",
              system_generated: "Saved by Kurukoo for your review.",
            }[String(item.provenance).toLowerCase()] || "Saved in your Memory Profile for review.";
          details.appendChild(makeElement("small", "", memorySource));
        }
        const actions = makeElement("div", "storefront-actions");
        if (card.type === "notifications") {
          const open = makeElement("button", "sf-btn sf-secondary", "Open");
          open.type = "button";
          open.addEventListener("click", () => sendMessage(`Open notification ${String(item.id)}`));
          actions.appendChild(open);
          if (String(item.status) !== "read") {
            const read = makeElement("button", "sf-btn sf-primary", "Mark read");
            read.type = "button";
            read.addEventListener("click", () =>
              sendMessage(`Mark notification ${String(item.id)} as read`),
            );
            actions.appendChild(read);
          }
        } else if (card.type === "reminders") {
          const cancel = makeElement("button", "sf-btn sf-secondary", "Cancel");
          cancel.type = "button";
          cancel.addEventListener("click", () => sendMessage(`Cancel reminder ${String(item.id)}`));
          actions.appendChild(cancel);
        } else if (card.type === "tasks" && String(item.status) === "available") {
          const accept = makeElement("button", "sf-btn sf-primary", "Accept task");
          accept.type = "button";
          accept.addEventListener("click", () => sendMessage(`Accept task ${String(item.id)}`));
          actions.appendChild(accept);
        } else if (card.type === "memory" && item.id) {
          const forget = makeElement("button", "sf-btn sf-secondary", "Forget");
          forget.type = "button";
          forget.addEventListener("click", () => sendMessage(`Forget memory ${String(item.id)}`));
    ...
    } else if (card.type !== "request_status")
      list.appendChild(makeElement("div", "deferred", "Nothing is waiting here."));
    ...
    if (card.type === "memory_action") {
      holder.appendChild(
        makeElement(
          "span",
          "storefront-execution-status",
          "Saved to your owner-scoped Memory Profile.",
        ),
      );
    }
    if (card.type === "os_status") {
      holder.appendChild(
        makeElement(
          "span",
          "storefront-execution-status",
          `Status: ${String(card.status || "unknown").replace(/_/g, " ")}`,
        ),
      );
    ...
    if (card.type === "request_status") {
      ...
      if (card.quote && typeof card.quote === "object")
        requestEvidence.push({
          label: "Quote",
          value: `${String(card.quote.amount_minor ?? card.quote.priceMinor ?? "Recorded")} ${String(card.quote.currency || "local currency")}`,
        });
      if (card.fulfillment && typeof card.fulfillment === "object")
        requestEvidence.push({
          label: "Fulfilment",
          value: String(card.fulfillment.status || card.fulfillment.stage || "recorded").replace(
            /_/g,
            " ",
          ),
        });
      appendOutcomeEvidence(holder, requestEvidence);
      const requestTone = /complete|fulfilled|paid/.test(requestStatus)
        ? "success"
        : /quote|confirm|payment|approval|needs_user/.test(requestStatus)
          ? "attention"
          : /wait|pending|requested|match|fulfillment/.test(requestStatus)
            ? "waiting"
            : "neutral";
      const requestLabel =
        requestTone === "success"
          ? "Done"
          : requestTone === "attention"
            ? "I need you"
            : requestTone === "waiting"
              ? "Waiting for"
              : "I’m working on it";
      appendOutcomeStatus(
        holder,
        `${requestLabel}: ${requestStatus.replace(/_/g, " ")}.`,
        requestTone,
      );
    }
    holder.appendChild(
      makeElement(
        "small",
        "storefront-execution-status",
        card.ownerScoped === true
          ? "Owner-scoped persisted state. Choose an exact item to continue."
          : "Persisted state only; no external delivery or fulfilment is implied.",
      ),
    );
    messageEl.querySelector(".bubble")?.appendChild(holder);
  }
  ...
  function appendOutcomeChoices(holder, options = [], promptFallback, heading = "I can") {
    ...
    usable.forEach((option) => {
      const button = makeElement(
        "button",
        `sf-btn sf-${option.style === "secondary" ? "secondary" : "primary"}`,
        String(option.label),
      );
      button.type = "button";
        ...
        if (option.id === "copy_message" && option.copyText) {
          ...
          if (navigator.clipboard?.writeText) {
            navigator.clipboard
              .writeText(text)
              .then(() =>
                pushAgentSurfaceToast("Message copied", "No delivery channel was used.", false),
              )
              .catch(() =>
                pushAgentSurfaceToast(
                  "Copy unavailable",
                  "Select the message text and copy it manually. No delivery channel was used.",
                  true,
                ),
              );
          } else {
  ...
  function renderDeviceSupport(card, messageEl) {
    ...
    holder.appendChild(
      makeElement(
        "small",
        "outcome-primitive outcome-context",
        card.observedAt
          ? `Checked ${String(card.observedAt)}`
          : status === "completed"
            ? "Checked just now"
            : "Waiting for an authorised check",
      ),
    );
    holder.appendChild(
      makeElement(
        "p",
        "storefront-offer-copy",
        String(card.message || "Kurukoo has not claimed a live device diagnosis."),
      ),
    );
    const support =
      card.supportLevel && typeof card.supportLevel === "object" ? card.supportLevel : null;
    if (support?.level) {
      holder.appendChild(
        makeElement(
          "strong",
          "device-support-level",
          `Support level ${String(support.level)} · ${String(support.name || "Available support")}`,
        ),
      );
      holder.appendChild(
        makeElement(
          "p",
          "device-support-explanation",
          String(
            support.explanation ||
              "Kurukoo will use the strongest capability currently available and remain clear about what it cannot do.",
          ),
        ),
      );
    }
    if (card.resource && typeof card.resource === "object") {
      ...
      const evidence = [
        {
          label: "Resource",
          value: `${String(resource.kind || "device")} · ${String(resource.protocol || "adapter")}`,
        },
        ...(!state || typeof state !== "object"
          ? [{ label: "Recorded state", value: String(state || "No state detail") }]
    ...
    const resolution =
      card.resolution && typeof card.resolution === "object" ? card.resolution : null;
    const resolutionOptions = Array.isArray(resolution?.options) ? resolution.options : [];
    appendOutcomeChoices(holder, resolutionOptions);
    const supportText =
      support?.explanation ||
      (card.liveObservation === true
        ? "Recorded observation is available for reasoning."
        : "Kurukoo can guide checks or coordinate an expert, but has not claimed direct access or completion.");
    appendOutcomeStatus(
      holder,
      `${supportText} ${card.liveObservation === true ? "No action is claimed until the selected next step produces evidence." : "No live connection, diagnosis, remediation, or repair completion is claimed."}`,
      card.liveObservation === true ? "evidence" : "waiting",
    );
    messageEl.querySelector(".bubble")?.appendChild(holder);
  }
  ...
  function renderOutcomeActionCard(card, messageEl) {
    ...
    const title =
      card.type === "monitoring_setup"
        ? "Keep an eye on this"
        : card.type === "reminder_setup"
          ? "Keep this follow-up active"
    ...
    if (card.ownedWork && card.type === "monitoring_setup") {
      ...
      const statusCopy =
        workStatus === "waiting"
          ? "Saved follow-up · waiting for the next safe observation"
          : workStatus === "active"
            ? "Saved follow-up · ready for the next safe observation"
            : "Saved follow-up · review required before Kurukoo can continue";
      appendOutcomeStatus(holder, statusCopy, workStatus === "waiting" ? "neutral" : "waiting");
      if (card.workSummary)
        holder.appendChild(makeElement("p", "storefront-offer-copy", String(card.workSummary)));
    }
    if (card.target)
      appendOutcomeEvidence(holder, [{ label: "Target", value: String(card.target) }]);
    if (card.recipient)
      appendOutcomeEvidence(holder, [{ label: "Recipient", value: String(card.recipient) }]);
    ...
    const actions = (Array.isArray(card.actions) ? card.actions : []).map((action) => ({
      ...action,
      prompt: prompts[action.id] || String(action.label || action.id || "Continue"),
      copyText: action.id === "copy_message" ? String(card.body || "") : undefined,
    }));
    appendOutcomeChoices(holder, actions);
    const communicationStatus = {
      recipient_ready:
        "Recipient found. Your message is still unsent; choose a delivery route next.",
      ...
      channel_unavailable:
        "No authorised delivery route is active. Your message remains saved and unsent.",
      ...
      needs_recipient:
        "Recipient, channel, and confirmation are still required. Nothing has been sent.",
      needs_message: "Tell Kurukoo what you want to say. Nothing was prepared or sent.",
    };
    if (!(card.type === "monitoring_setup" && card.ownedWork)) {
      appendOutcomeStatus(
        holder,
        card.type === "communication_prepare"
          ? communicationStatus[String(card.status || "")] ||
              "Not sent. Recipient, channel, and confirmation are still required."
          : card.type === "reminder_setup"
            ? "Not scheduled yet. Cadence and completion condition are still required."
            : "Not monitoring yet. Kurukoo will only claim a watch after an observable target and notification path are recorded.",
        String(card.deliveryState || "").toLowerCase() === "accepted" ? "neutral" : "waiting",
      );
    }
    messageEl.querySelector(".bubble")?.appendChild(holder);
  }
  ...
  function renderTopicDraftCard(card, messageEl) {
    ...
    holder.appendChild(
      makeElement(
        "p",
        "storefront-offer-copy",
        "Your draft is private. Review and edit it before you choose to submit it for moderation.",
      ),
    );
    ...
    appendOutcomeStatus(
      holder,
      "Nothing has been published, and this draft is not evidence of a provider, availability, price, booking, payment, or completed service.",
      "waiting",
    );
    messageEl.querySelector(".bubble")?.appendChild(holder);
  }
  ...
  function renderComposedGoalCard(card, messageEl) {
    ...
    holder.appendChild(
      makeElement(
        "p",
        "",
        String(card.situation || goal.objective || "Kurukoo is coordinating this outcome."),
      ),
    );
    if (Array.isArray(card.evidence) && card.evidence.length)
      appendOutcomeEvidence(holder, card.evidence);
    ...
    subGoals.forEach((subGoal, index) => {
      ...
      const labels = {
        active: "Ready",
        waiting: "Waiting",
        waiting_on_dependency: "Waiting for earlier evidence",
      ...
      row.appendChild(
        makeElement(
          "span",
          "",
          `${index + 1}. ${outcomeSafeCopy(subGoal?.objective || subGoal?.goalType || "Next step")}`,
        ),
      );
      row.appendChild(
        makeElement("small", "storefront-execution-status", labels[status] || "Recorded state"),
      );
      list.appendChild(row);
    });
    ...
    if (firstCapability && typeof firstCapability === "object") {
      const capabilitySection = makeElement(
        "div",
        "outcome-primitive composed-goal-first-capability",
      );
      capabilitySection.appendChild(makeElement("strong", "", "First supported step"));
      if (firstCapability.message)
        capabilitySection.appendChild(
          makeElement("p", "storefront-offer-copy", String(firstCapability.message)),
        );
      ...
      capabilitySection.appendChild(
        makeElement(
          "small",
          "storefront-execution-status",
          `Working state: ${capabilityState}. No external success is assumed.`,
        ),
      );
    ...
    if (Array.isArray(card.choices) && card.choices.length && (activeWork || needsDecision))
      appendOutcomeChoices(holder, card.choices);
    const decisionStage = String(
      card.firstCapability?.stage || card.firstCapability?.status || "",
    ).toLowerCase();
    const requiresDecision =
      ...
      [
        ...
        "provider_selected",
        "quote_ready",
      ].some((value) => decisionStage.includes(value));
    const decisionActions = Array.isArray(card.firstCapability?.actions)
      ? card.firstCapability.actions
          .filter(
            (action) =>
              action?.label &&
              !["cancel", "check_again"].includes(String(action.id || "").toLowerCase()),
          )
          .map((action) => ({ ...action, prompt: action.prompt || action.label }))
      : [];
    if (requiresDecision && decisionActions.length)
      appendOutcomeChoices(holder, decisionActions, undefined, "I need you");
    if (card.work && typeof card.work === "object") {
      const workDetail = outcomeSafeCopy(
        card.work.detail || "Kurukoo is coordinating the next supported step.",
      );
      const workLabel =
    ...
    if (capabilityPath && typeof capabilityPath === "object") {
      ...
      const labels = {
        ...
        assist: "I can assist",
        act: "I can act",
        delegate: "I can delegate",
        continue: "I can continue",
      };
      Object.entries(labels).forEach(([key, label]) => {
        const details = Array.isArray(capabilityPath[key])
          ? capabilityPath[key].filter(Boolean)
          : [];
    ...
    holder.appendChild(
      makeElement(
        "small",
        "storefront-execution-status",
        "Kurukoo will only claim an action, provider result, payment, fulfilment, notification, or completion when the corresponding canonical evidence exists.",
      ),
    );
    messageEl.querySelector(".bubble")?.appendChild(holder);
  }
  ...
  function renderCard(card, messageEl) {
    ...
    if (card.type === "monitoring_setup" || card.type === "communication_prepare")
      return renderOutcomeActionCard(card, messageEl);
    ...
    if (
      ["notifications", "reminders", "tasks", "memory", "request_status", "os_status"].includes(
        card.type,
      )
    )
      return renderOSCollection(card, messageEl);
    if (
      ["notification_action", "reminder_action", "task_action", "memory_action"].includes(card.type)
    )
      return renderOSCollection(card, messageEl);
    if (card.type === "device_support" || card.type === "device_resource_selection")
      return renderDeviceSupport(card, messageEl);
    if (card.type === "agentic_storefront") {
      if (card.stage === "slot_fill" || card.stage === "intent_extraction") {
        const missing = (Array.isArray(card.fields) ? card.fields : [])
          .filter((field) => field?.required && !field.value)
          .map((field) => String(field.label || field.key || "the next detail").toLowerCase());
    ...
    if (card.type === "suggestions")
      return renderSuggestions(card.options, messageEl, card.sponsored);
    if (card.type === "intent_suggestions")
      return renderSuggestions(card.suggestions, messageEl, card.sponsored);
    ...
    if (card.type === "agent_brief") {
      const recoverBriefAction = (item) => {
        ...
        if (existing?.canonicalAction && existing?.objectType && existing?.objectId)
          return existing;
        const stableRef = String(item?.stableRef || "");
        const match = stableRef.match(
          /^(economic_request|task|reminder|agent_goal|notification):(.+)$/,
        );
        ...
        return canonicalAction
          ? {
              id: `open_${objectType}`,
              label: objectType === "agent_goal" ? "Open objective" : "Open update",
              canonicalAction,
              objectType,
              objectId,
            }
      ...
      const items = Array.isArray(card.items)
        ? card.items.filter((item) => item?.attention !== "SILENT").slice(0, 6)
        : [];
      if (!items.length)
        list.appendChild(makeElement("div", "deferred", "No active items need attention."));
      items.forEach((item) => {
        ...
        if (canResume) {
          ...
          row.setAttribute(
            "aria-label",
            `${String(action.label || "Open update")}: ${String(item.summary || "Kurukoo update")}`,
          );
          row.addEventListener("click", () => {
            state.canonicalContextAction = {
              type: "resume_canonical_context",
              contextId:
                `agent_brief:${String(card.briefId || card.id || "current")}:${String(item.stableRef || action.objectId)}`.slice(
                  0,
                  180,
                ),
      ...
      followUp.addEventListener("click", () => {
        if (input) {
          input.value = "What have I got going on?";
          input.dispatchEvent(new Event("input", { bubbles: true }));
          input.focus();
        }
      });
      holder.append(
        ...
        makeElement(
          "span",
          "escrow-badge",
          "Updates are read-only. Any consequential action still requires the existing approval boundary.",
        ),
      );
      messageEl.querySelector(".bubble")?.appendChild(holder);
      return;
    ...
    if (card.type === "emergency" || card.type === "emergency_dispatch") {
      const holder = makeElement("section", "emergency-chat-card");
      holder.setAttribute("aria-label", "Emergency assistance");
      holder.dataset.state = String(card.status || card.session?.dialState || "unavailable");
      if (card.status !== "ended") holder.setAttribute("aria-live", "assertive");
      const serviceType = String(
        card.service?.serviceType || card.service || card.session?.serviceType || "emergency",
      );
      const serviceLabels = {
      ...
      const statusLabels = {
        dial_requested: "Ready for you to call",
        unavailable: "Use the verified local route",
        externally_pending: "Ready for you to call",
        dial_unavailable: "Calling is not available here",
        ended: "Emergency mode ended",
        connected: "Connection evidence received",
        failed: "Could not prepare the call route",
      };
      const heading = makeChildren("div", "emergency-chat-card__heading", [
        makeIcon("safety", "Emergency"),
        makeElement("strong", "", status === "ended" ? "Emergency mode ended" : "Emergency help"),
      ]);
      const service = makeElement(
        "p",
        "emergency-chat-card__service",
        `${serviceLabels[serviceType] || "Emergency services"} · ${statusLabels[status] || "Emergency route ready"}`,
      );
      const location = makeElement(
        "p",
        "emergency-chat-card__location",
        `Location: ${card.locationStatus === "approximate" || card.session?.location ? "approximate location available" : "not known yet"}`,
      );
      const truth = makeElement(
        "p",
        "emergency-chat-card__truth",
        "Call the verified emergency number yourself if you are in immediate danger. Kurukoo does not claim connection or dispatch without evidence.",
      );
        ...
        const label = String(
          action.label ||
            (action.id === "dial"
              ...
              : action.id === "end"
                ? "End emergency mode"
                : action.id === "share_location"
                  ? "Share approximate location"
                  : "Continue"),
        );
        const button = makeElement(
          action.href ? "a" : "button",
          "emergency-chat-card__action",
          label,
        );
        if (action.href) {
          ...
          button.type = "button";
          button.addEventListener("click", () => {
            if (action.canonicalAction === "emergency.end" || action.id === "end")
              sendMessage("Actually this is not an emergency anymore.");
            else if (
              action.canonicalAction === "emergency.location" ||
              action.id === "share_location"
            )
              sendMessage(
                "I do not know exactly where I am. Help me share an approximate location.",
              );
          });
        }
        actions.appendChild(button);
      });
      holder.append(heading, service, location, truth, actions);
      messageEl.querySelector(".bubble")?.appendChild(holder);
      return;
    }
    ...
    if (card.type === "safety_contact_capture") {
      ...
      const desc = makeElement(
        "p",
        "",
        `You're adding ${card.name} as a contact. Share their phone number in the chat to continue.`,
      );
    ...
    if (card.type === "ride_picker") {
      ...
      ["🚗 Okada", "🛺 Keke", "🚕 Taxi"].forEach((t) => {
        const b = makeElement("button", "", t);
        b.type = "button";
        qa.appendChild(b);
      });
      holder.append(
        qa,
        makeElement("span", "escrow-badge", "Availability is confirmed in the request flow."),
      );
      holder
        .querySelectorAll("button")
        .forEach((btn) =>
          btn.addEventListener("click", () => sendMessage(`${btn.textContent.trim()} ride`)),
        );
    } else if (card.type === "worker_match" || card.type === "service_search") {
      holder.appendChild(
        makeElement("strong", "", card.category === "food" ? "Food request" : "Service request"),
      );
      holder.appendChild(
        makeElement(
          "div",
          "deferred",
          "Kurukoo is assessing the request context for a supported path.",
        ),
      );
      holder.appendChild(
        makeElement(
          "span",
          "escrow-badge",
          "Availability is confirmed before a next action is presented.",
        ),
      );
      ...
      holder.appendChild(
        makeElement(
          "div",
          "deferred",
          "Location-based information is shown only when relevant data is available.",
        ),
      );
    } else if (card.type === "sports_search") {
      holder.appendChild(makeElement("strong", "", "Sports network"));
      holder.appendChild(
        makeElement("div", "deferred", "Searching leagues, teams, matches and nearby play."),
      );
      ...
      holder.appendChild(
        makeElement(
          "div",
          "deferred",
          "Contributor participation and any resulting payment step require separate confirmation.",
        ),
      );
      ...
      holder.appendChild(
        makeElement(
          "span",
          "escrow-badge",
          "Provider suitability and availability require confirmation.",
        ),
      );
      ...
      detail.textContent = reminder.title
        ? `${reminder.title}${dueText}.`
        : "Your reminder is attached to this conversation.";
      const boundary = document.createElement("span");
      boundary.className = "escrow-badge";
      boundary.textContent =
        "This personal reminder does not create a provider request or payment step.";
      ...
      holder.appendChild(
        makeElement(
          "div",
          "deferred",
          `Skill: ${skill || "not specified"}${location ? ` · Location: ${location}` : ""}`,
        ),
      );
      holder.appendChild(
        makeElement(
          "span",
          "escrow-badge",
          "Review required. This does not publish availability, verification, pricing, matching, payment, or fulfilment.",
        ),
      );
      ...
      confirm.addEventListener("click", async () => {
        ...
        try {
          ...
          const data = await response.json().catch(() => ({}));
          if (!response.ok || !data.success)
            throw new Error(data.error || "Could not save provider profile");
          confirm.textContent = "Saved — availability remains off";
          holder.appendChild(
            makeElement(
              "div",
              "deferred",
              "Profile skill saved. Turn availability on separately only when you are ready, and verification is still required before matching.",
            ),
          );
        } catch (error) {
          ...
          holder.appendChild(
            makeElement(
              "div",
              "deferred",
              `Could not save profile: ${error.message || "try again"}`,
            ),
          );
        }
      });
      ...
      holder.appendChild(
        makeElement(
          "div",
          "deferred",
          "Availability and representation details require confirmation before a request can proceed.",
        ),
      );
      holder.appendChild(
        makeElement("span", "escrow-badge", "Payment availability is assessed separately."),
      );
    } else {
  ...
  async function uploadAttachment(file) {
    const allowed = /^(image\/(png|jpeg|webp|gif)|application\/pdf|video\/mp4|video\/webm)$/i.test(
      file.type || "",
    );
    if (!allowed)
      throw new Error("Unsupported attachment type. Use an image, PDF or supported video.");
    if (file.size > 25 * 1024 * 1024)
      throw new Error("Attachment is larger than the 25 MB chat limit.");
  ...
  function pushAgentSurfaceToast(title, detail, needsResponse = false) {
    const region = $("chat-toast-region");
    if (!region) return;
    ...
    const copy = makeElement("div", "chat-toast-copy");
    copy.append(
      makeElement("strong", "", title || "Kurukoo update"),
      makeElement("span", "", String(detail || "").slice(0, 360)),
    );
    const close = makeElement("button", "chat-toast-close");
    close.type = "button";
    close.setAttribute("aria-label", "Dismiss Kurukoo update");
    close.title = "Dismiss";
    close.append(makeIcon("close", "Dismiss"));
    close.addEventListener("click", () => toast.remove());
    if (needsResponse) toast.dataset.needsResponse = "true";
    toast.append(makeIcon("agent", "Kurukoo"), copy, close);
    region.appendChild(toast);
    setTimeout(() => toast.remove(), needsResponse ? 12000 : 7000);
  }
  function createBackgroundStreamBubble() {
    const wrap = makeElement("article", "message assistant message-streaming");
    wrap.hidden = true;
    state.currentAssistantArticle = wrap;
    window.KurukooTurnState?.mount?.(wrap);
    window.KurukooTurnState?.setStatus?.(wrap, "working", "Kurukoo is working on this…");
    const bubble = makeElement("div", "bubble");
    bubble.append(
      makeElement("div", "thinking", "Kurukoo is working…"),
      makeElement("div", "markdown-body"),
    );
  ...
  function applyCapabilityResult(result, assistant) {
    ...
    if (result.canonicalObjectId)
      assistant.dataset.canonicalObjectId = String(result.canonicalObjectId);
    const bubble = assistant.querySelector(".bubble");
    if (!bubble || bubble.querySelector(".message-capability-status")) return;
    const labels = {
      accepted: "Accepted",
      waiting: "Waiting for the next supported step",
      needs_user: "Needs your input",
      confirmation_required: "Confirmation required",
      blocked: "Blocked safely",
      failed: "Needs recovery",
      completed: "Completed",
      externally_pending: "Pending external evidence",
      unavailable_external_dependency: "External activation required",
    };
    const status = makeElement(
      "div",
      "message-capability-status",
      labels[result.status] || "Capability status updated",
    );
  ...
  async function sendMessage(raw) {
    ...
    if (!(await ensureIdentity())) {
      ...
      pushAgentSurfaceToast(
        "Kurukoo is unavailable",
        "Your message is still in the composer. Check your connection and try again.",
        true,
      );
      return;
    }
    ...
    try {
      if (attachment instanceof File) {
        ...
        attachment = await uploadAttachment(attachment);
      }
      state.attached = null;
      $("attachment-preview").hidden = true;
      $("attachment-preview").textContent = "";
      const finalText = attachment
        ? `${text}\n\n[Attachment: ${attachment.name} — ${attachment.type} — ${attachment.url}]`
        : text;
      const contextAction =
        state.canonicalContextAction || state.discoveryContextAction || undefined;
      state.canonicalContextAction = null;
      state.discoveryContextAction = undefined;
      const user = surfaceActive ? document.createElement("article") : addUserMessage(finalText);
      const assistant = surfaceActive ? createBackgroundStreamBubble() : appendStreamBubble();
      ...
      while (true) {
        ...
        for (const event of events) {
          ...
          if (data.type === "conversation") {
            ...
            user.dataset.messageId = data.messageId || "";
          }
          if (data.type === "auth_success") {
            ...
            setConnection(true);
            const logoutButton = $("workspace-logout") || $("logout-sidebar-btn");
            if (logoutButton) logoutButton.hidden = false;
            if (data.phone) localStorage.setItem("kurukoo_user_phone", data.phone);
          }
          if (data.type === "metadata") updateModelStatus(data);
          if (data.type === "status") {
            setTypingStatus(data.status, data.label);
            if (window.KurukooTurnState)
              window.KurukooTurnState.setStatus(assistant, data.status, data.label);
          }
          ...
          if (data.type === "text") {
            if (assistant.hidden) {
              ...
              assistant.classList.remove("message-streaming");
              assistant.classList.add("message-arrived");
            }
            setTypingStatus("complete");
            full += data.content || "";
            setMarkdown(output, full);
          ...
          if (data.type === "done") {
            ...
            if (surfaceActive)
              pushAgentSurfaceToast(
                data.cardData?.title || "Kurukoo update",
                full || data.cardData?.message || "Kurukoo has an update for this task.",
                Boolean(data.cardData?.fields?.length || data.cardData?.needsUser),
              );
      ...
      if (state.isGuest) {
        if (/enter your phone number/i.test(full)) setAuthComposerStep("phone");
          ...
          /6-digit(?: verification)? code|code from the approved verification channel/i.test(full)
        )
          setAuthComposerStep("otp");
        else if (/profile is now verified|account is now connected/i.test(full))
          setAuthComposerStep("none");
      }
      state.messages.push({
        role: "user",
        text: finalText,
        id: Number(user.dataset.messageId) || null,
      });
      state.messages.push({
        role: "assistant",
        text: full,
        id: Number(assistant.dataset.messageId) || null,
      });
      ...
      if (error?.name === "AbortError") {
        ...
        assistant.classList.remove("message-streaming");
        assistant.classList.add("message-arrived");
        if (!full) setMarkdown(output, "Generation stopped.");
        if (window.KurukooTurnState)
          window.KurukooTurnState.setStatus(assistant, "needs-you", "Stopped");
        assistant.dataset.turnFinal = "true";
        return;
      }
      setConnection(false, "Connection issue");
      ...
      if (window.KurukooTurnState)
        window.KurukooTurnState.setStatus(assistant, "offline", "Connection lost — reconnecting…");
      assistant.dataset.turnFinal = "true";
      if (!surfaceActive) {
        assistant.hidden = false;
        assistant.classList.remove("message-streaming");
      ...
      if (surfaceActive)
        pushAgentSurfaceToast(
          "Kurukoo could not finish that",
          error.message || "Please try again from the composer.",
          true,
        );
      const bubble = surfaceActive
        ? null
        : chatContent.querySelector(".message.assistant:last-child .markdown-body");
      if (bubble)
        setMarkdown(
          bubble,
      ...
      if (state.currentAssistantArticle && !state.currentAssistantArticle.dataset.turnFinal) {
        if (window.KurukooTurnState) {
          if (full && full.trim()) window.KurukooTurnState.done(state.currentAssistantArticle);
          else
            window.KurukooTurnState.setStatus(
              state.currentAssistantArticle,
              "needs-you",
              "Waiting on your next message",
            );
        }
        state.currentAssistantArticle.dataset.turnFinal = "true";
      }
      state.currentAssistantArticle = null;
      if (state.authStep !== "none") setAuthComposerStep(state.authStep);
      else
        input.placeholder = state.displayName
  ...
  async function loadPoints() {
    try {
      ...
      const balance = $("points-balance")?.querySelector("span");
      if (balance) balance.textContent = points;
      const ip = $("inspector-points");
      if (ip) ip.textContent = points;
    } catch {}
  }
  function renderNotifications(notifications = []) {
    ...
    unreadItems
      .filter((item) => item?.id && !state.notifiedNotificationIds.has(item.id))
      .slice(0, 3)
      .forEach((item) => {
        state.notifiedNotificationIds.add(item.id);
        if (!toastRegion) return;
        const toast = makeElement("div", "chat-toast");
        toast.append(
          makeIcon("alert", "Notification"),
          makeElement(
            "div",
            "",
            `${String(item.title || "Kurukoo update")}\n${String(item.body || "")}`,
          ),
        );
    ...
    if (summary)
      summary.textContent = unread
        ? `${unread} unread notification${unread === 1 ? "" : "s"} in your internal Kurukoo inbox.`
        : "Your internal Kurukoo inbox is up to date.";
    if (!list) return;
    if (!items.length) {
      list.appendChild(makeElement("div", "empty-state", "No notifications yet."));
      return;
    }
    items.forEach((item) => {
      ...
      const meta = makeElement(
        "small",
        "",
        `${String(item.delivery_state || "queued")} · ${String(item.created_at || "")}`,
      );
      row.append(title, body, meta);
      if (item.status === "unread") {
        const read = makeElement("button", "text-btn", "Mark read");
        read.type = "button";
  ...
  function renderTasksActivity(topics = []) {
    ...
    if (!list) return;
    list.replaceChildren();
    const items = Array.isArray(topics)
      ? topics.filter((topic) => topic && topic.title).slice(0, 5)
      : [];
    ...
    if (!items.length) {
      ...
      empty.append(
        makeElement("p", "topic-activity-empty", "No recent public Topics match this view yet."),
      );
    ...
    items.forEach((topic) => {
      ...
      const meta = makeElement(
        "div",
        "topic-activity-meta",
        [String(topic.type || "Topic").replace(/_/g, " "), topic.city || topic.lga || "Community"]
          .filter(Boolean)
          .join(" · "),
      );
      ...
      const body = makeElement(
        "p",
        "topic-activity-body",
        String(topic.body || "Community Topic").slice(0, 150),
      );
      const link = makeElement("button", "workspace-text-action topic-activity-link", "Open Topic");
      link.type = "button";
      link.dataset.topicSlug = String(topic.slug || "");
      link.addEventListener("click", () => {
        if (topic.slug) window.location.href = `/topics/${encodeURIComponent(topic.slug)}`;
      });
      row.append(meta, title, body, link);
      list.appendChild(row);
    });
  }
  async function loadTasksActivity() {
    ...
    try {
      ...
      const data = await response.json();
      renderTasksActivity(data.topics || []);
    } catch {
      renderTasksActivity([]);
    }
  }
  function renderProactiveInspector(opportunities = []) {
    const card = document.getElementById("proactive-context-card");
    const list = document.getElementById("proactive-context-list");
    if (!card || !list) return;
    const items = Array.isArray(opportunities)
      ? opportunities
          .filter((item) => item && item.title && item.status !== "dismissed")
          .slice(0, 3)
      : [];
    list.replaceChildren();
    ...
    items.forEach((item) => {
      ...
      row.append(
        makeElement("strong", "proactive-context-title", String(item.title)),
        makeElement(
          "p",
          "proactive-context-copy",
          String(item.subtitle || "Available in Chat.").slice(0, 130),
        ),
      );
      const action = makeElement(
        "button",
        "workspace-text-action proactive-context-action",
        String(item.ctaText || "Open in Chat"),
      );
      action.type = "button";
      action.addEventListener("click", () => {
        const link = String(item.ctaLink || "/chat");
        window.location.href = link.startsWith("/") ? link : "/chat";
      });
  ...
  function renderNearbyInspector(features = [], active = false) {
    ...
    const label = document.getElementById("nearby-context-label");
    if (!list) return;
    ...
    if (!items.length) {
      list.appendChild(
        makeElement(
          ...
          "empty-state",
          active
            ? "No active activity is available in this area yet."
            : "Ask Kurukoo what is nearby when local discovery is useful.",
        ),
      );
      return;
    }
  ...
  async function loadNearbyInspector(view = state.surfaceView) {
    ...
    if (!list) return;
    if (view !== "discover") {
      renderNearbyInspector([], false);
      return;
    }
    if (state.isGuest) {
      renderNearbyInspector([], true);
      const empty = list.querySelector(".empty-state");
      if (empty) empty.textContent = "Sign in to share your location for nearby results.";
      return;
    }
    ...
    try {
      ...
      await fetch("/api/location/consent", {
        method: "POST",
        credentials: "same-origin",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const url = new URL("/api/discover/map", location.origin);
      url.searchParams.set("lat", String(coarseLatitude));
      url.searchParams.set("lng", String(coarseLongitude));
      url.searchParams.set("radius", "10000");
      url.searchParams.set("layers", "mobile,stationary,agents,emergency,deals,events");
      const response = await fetch(url, { credentials: "same-origin" });
      if (!response.ok) throw new Error("Nearby activity unavailable");
      const data = await response.json();
      renderNearbyInspector(data.features || [], true);
    } catch {
      renderNearbyInspector([], true);
    }
  }
  async function loadProactiveInspector() {
    ...
    if (!card) return;
    try {
      const response = await fetch("/api/proactive/feed", { credentials: "same-origin" });
      if (!response.ok) throw new Error("Proactive feed unavailable");
      const data = await response.json();
      renderProactiveInspector(data.opportunities || []);
    } catch {
      renderProactiveInspector([]);
    }
  }
  async function loadTaskContext() {
    ...
    try {
      ...
      list.replaceChildren();
      if (!tasks.length) {
        list.appendChild(makeElement("div", "empty-state", "No pending tasks right now."));
        return;
      }
      tasks.slice(0, 4).forEach((task) => {
        ...
        const title = String(
          task.title || task.description || task.name || `Task #${task.id || "—"}`,
        );
        row.append(
          makeElement("span", "task-context-dot"),
          makeElement("span", "task-context-copy", title),
          makeElement("small", "task-context-status", status),
        );
        list.appendChild(row);
      });
    } catch {
      list.replaceChildren(makeElement("div", "empty-state", "Tasks are unavailable right now."));
    }
  }
  async function loadTrustChallenges() {
    ...
    try {
      ...
      const toastRegion = $("chat-toast-region");
      challenges
        .filter((challenge) => challenge?.id && !state.notifiedTrustChallengeIds.has(challenge.id))
        .forEach((challenge) => {
          ...
          const copy = makeElement("div");
          copy.append(
            makeElement("strong", "", "Approve this Kurukoo device"),
            makeElement("span", "", "A new browser is asking to continue your account."),
          );
          const actions = makeElement("div", "chat-toast-actions");
          const approve = makeElement("button", "text-btn", "Approve");
          approve.type = "button";
          const deny = makeElement("button", "text-btn", "Deny");
          deny.type = "button";
          ...
          const act = async (button, path) => {
            ...
            try {
              const result = await fetch(
                `/api/device/challenge/${encodeURIComponent(challenge.id)}/${path}`,
                {
                  method: "POST",
                  credentials: "same-origin",
                  headers: { "X-Kurukoo-Device-Id": deviceId },
                },
              );
              if (!result.ok)
                throw new Error("This device is not eligible to approve the request.");
              toast.remove();
            } catch (error) {
          ...
          toast.append(makeIcon("safety", "Device approval"), copy, actions);
          toastRegion.appendChild(toast);
        });
    } catch {}
  }
  ...
  function personalizeQuickActions(profile = {}) {
    ...
    const actions = [
      {
        prompt: location
          ? `What is useful to know near ${location}?`
          : "Help me find something nearby",
        label: location ? "Explore nearby" : "Explore nearby",
        icon: "discover",
      },
      {
        prompt:
          goal.includes("provider") || goal.includes("work")
            ? "Help me find work or a service opportunity"
            : "I need something sourced",
        label:
          goal.includes("provider") || goal.includes("work") ? "Find work" : "Source something",
        icon: "work",
      },
      { prompt: "Set a reminder", label: "Reminder", icon: "reminder" },
      { prompt: "Manage my safety contacts and check-ins", label: "Safety", icon: "safety" },
    ...
    document.querySelectorAll(".quick-actions, .composer-quick-actions").forEach((root) => {
      ...
      actions.forEach((action, index) => {
        ...
        button.dataset.personalized = location || goal ? "profile" : "default";
  ...
  async function loadMemory() {
    try {
      ...
      if (!state.displayName && profile.name) state.displayName = String(profile.name);
      const memoryStatus = $("sidebar-memory-status");
      if (memoryStatus)
        memoryStatus.textContent = profile.location || profile.name ? "In use" : "Ready";
      personalizeQuickActions(profile);
      const text = `Kurukoo remembers ${profile.location || "your area"}${profile.primary_lga ? `, ${profile.primary_lga}` : ""}. Your Memory Profile remains attached to your account.`;
      const mc = $("memory-context");
      if (mc) mc.textContent = text;
      const im = $("inspector-memory");
      if (im) im.textContent = text;
    } catch {}
  }

  function renderAgentGoal(goal, events = []) {
    const card = $("agent-goal-card");
    const statusEl = $("agent-goal-status");
    const summary = $("agent-goal-summary");
    const list = $("agent-goal-events");
    const pause = $("agent-goal-pause");
    const cancel = $("agent-goal-cancel");
    const doing = $("agent-goal-doing");
    const needs = $("agent-goal-needs");
    const next = $("agent-goal-next");
    const presenceEl = card?.querySelector(".agent-goal-presence");
    if (!card || !statusEl || !summary || !list || !pause || !cancel) return;
    if (!goal) {
      card.hidden = true;
      card.dataset.goalAvailable = "false";
      return;
    }
    card.hidden = false;
    card.dataset.goalAvailable = "true";
    ...
    const doingText =
      goalStatus === "active"
        ? currentSummary || "Kurukoo is working on the next step."
        : goalStatus === "waiting_on_dependency"
          ? "Kurukoo is waiting for earlier work in this objective to finish."
          : goalStatus === "waiting"
            ? "Kurukoo is waiting for the next confirmed update."
            : goalStatus === "needs_user"
              ? "Kurukoo has paused for your decision."
              : goalStatus === "blocked"
                ? "Kurukoo has paused safely until the blocker is resolved."
                : goalStatus === "completed"
                  ? "Kurukoo completed this objective."
                  : goalStatus === "cancelled"
                    ? "You asked Kurukoo to stop following up on this objective."
                    : goalStatus === "failed"
                      ? "Kurukoo paused this objective for review."
                      : "Kurukoo is checking this objective.";
    if (doing) doing.textContent = doingText;
    const needsText =
      goalStatus === "needs_user"
        ? currentSummary || "Your approval or a response on the next step."
        : goalStatus === "waiting_on_dependency"
          ? "Nothing from you right now. The next step depends on earlier work."
          : goalStatus === "waiting"
            ? "Nothing from you right now. Kurukoo is waiting for a confirmed update."
            : goalStatus === "blocked"
              ? "More verified information is needed before this can continue."
              : goalStatus === "active"
                ? goal.plan?.requiredInputs?.length
                  ? goal.plan.requiredInputs.join(", ")
                  : "Nothing from you right now."
                : "No further input is required right now.";
    if (needs) needs.textContent = needsText;
    const nextText =
      goalStatus === "needs_user"
        ? "Reply in Chat when you are ready to decide."
        : goalStatus === "waiting_on_dependency"
          ? "Kurukoo will continue when the earlier work is complete."
          : goalStatus === "waiting"
            ? "Kurukoo will continue when the next confirmed update arrives."
            : goalStatus === "active"
              ? "Kurukoo is continuing this objective."
              : goalStatus === "blocked"
                ? "Review the blocker in Chat before continuing."
    ...
    (Array.isArray(events) ? events.slice(-4) : []).forEach((event) => {
      ...
      const activity =
        result === "success"
          ? "A confirmed step was recorded."
          : result === "waiting"
            ? "Kurukoo is waiting for the next update."
            : result === "needs_user"
              ? "Your input is needed before the next step."
              : result === "blocked"
                ? "Kurukoo paused safely while it waits for more information."
                : result === "failed"
                  ? "A step needs review before continuing."
    ...
    if (!list.childElementCount)
      list.appendChild(
        makeElement("div", "empty-state", "Kurukoo will show confirmed activity here."),
      );
    ...
    const paused =
      ...
      String(goal.summary || "")
        .toLowerCase()
        .startsWith("paused at your request");
        ...
        renderAgentGoal(
          {
            id: goal.id,
            status:
              result.canonicalFacts?.status ||
              (action === "pause" ? "waiting" : action === "cancel" ? "cancelled" : "active"),
            summary: result.canonicalFacts?.objective || goal.summary,
          },
          events,
        );
      } catch (error) {
        setInspectorFeedback(error.message || failureMessage, "error");
      } finally {
    ...
    pause.onclick = () =>
      submitGoalAction(
        paused ? "resume" : "pause",
        paused ? "Could not resume follow-up." : "Could not pause follow-up.",
      );
  ...
  async function loadAgentGoal() {
    try {
      ...
      if (
        state.canonicalContextAction?.objectType === "agent_goal" &&
        state.canonicalContextAction?.objectId
      ) {
        const goalId = String(state.canonicalContextAction.objectId).slice(0, 120);
        const detail = await fetch(`/api/agent/goals/${encodeURIComponent(goalId)}`, {
          credentials: "same-origin",
        });
        if (detail.ok) {
          ...
          if (detailData.goal) {
            renderAgentGoal(
              detailData.goal,
              Array.isArray(detailData.events) ? detailData.events : [],
            );
            return;
          }
        }
      ...
      if (!response.ok) {
        ...
        const activeGoal = goals.find((g) =>
          ["active", "waiting", "needs_user", "blocked"].includes(String(g.status || "")),
        );
  ...
  async function loadReminders() {
    try {
      ...
      const card = $("reminders-card");
      const list = $("reminder-list");
      if (!res.ok || !card || !list) return;
      const data = await res.json();
      const reminders = Array.isArray(data.reminders) ? data.reminders : [];
      state.nativeAssistance.reminders = reminders;
      updateNativeAssistanceStatus();
      card.hidden = false;
      list.replaceChildren();
      if (!reminders.length) {
        list.textContent = "No active reminders.";
        return;
      }
      reminders.forEach((reminder) => {
        const row = document.createElement("div");
        row.className = "reminder-list-item";
        const title = document.createElement("strong");
        title.textContent = String(reminder.title || "Reminder");
        const due = document.createElement("span");
        const parsed = reminder.due_at ? new Date(reminder.due_at) : null;
        due.textContent =
        ...
        cancel.addEventListener("click", async () => {
          cancel.disabled = true;
          try {
            await executeCanonicalAction("reminder", "cancel", reminder.id);
          } catch (error) {
            setInspectorFeedback(error.message || "Could not cancel reminder.", "error");
          } finally {
            loadReminders();
          }
        });
        row.append(title, due, cancel);
        list.appendChild(row);
      });
    } catch {}
  }

  function setInspectorFeedback(message, kind = "info") {
    const feedback = $("inspector-feedback");
    if (!feedback) return;
  ...
  async function executeCanonicalAction(capability, action, canonicalObjectId, args = {}) {
    ...
    const idempotencyKey =
      `chat:${state.conversationId || "draft"}:${capability}:${action}:${canonicalObjectId || "none"}:${JSON.stringify(args).slice(0, 120)}`.slice(
        0,
        180,
      );
    const response = await fetch("/api/chat/action", {
      ...
      body: JSON.stringify({
        capability,
        action,
        canonicalObjectId: canonicalObjectId == null ? undefined : String(canonicalObjectId),
        arguments: args,
        conversationId: state.conversationId || undefined,
        idempotencyKey,
      }),
    ...
    if (!response.ok || !data.success)
      throw new Error(
        data.error || data.result?.message || "Canonical action could not be completed.",
      );
    return data.result;
  }
  async function nativeAction(url, options = {}) {
  ...
  async function loadSafety() {
    try {
      ...
      const contactsList = $("safety-contact-list");
      const checkInsList = $("safety-checkin-list");
      if (!contactsResponse.ok || !checkInsResponse.ok || !card || !contactsList || !checkInsList)
        return;
      ...
      state.nativeAssistance.checkIns = checkIns;
      updateNativeAssistanceStatus();
      card.hidden = false;
      contactsList.replaceChildren();
      checkInsList.replaceChildren();

      const contactHeading = document.createElement("strong");
      contactHeading.textContent = contacts.length ? "Contacts" : "No safety contacts yet.";
      contactsList.appendChild(contactHeading);
      contacts.forEach((contact) => {
        const row = document.createElement("div");
        row.className = "safety-list-item";
        const label = document.createElement("span");
        label.textContent = `${contact.name} · ${contact.status}`;
        row.appendChild(label);
        if (contact.status === "pending") {
          const activate = document.createElement("button");
          activate.type = "button";
          activate.className = "text-btn";
          activate.textContent = "Activate";
          activate.addEventListener("click", async () => {
            activate.disabled = true;
            try {
              await nativeAction(
                `/api/safety/contacts/${encodeURIComponent(contact.id)}/activate`,
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
              ...
              setInspectorFeedback(e.message, "error");
            }
          });
        ...
        if (contact.status !== "revoked") {
          ...
          revoke.type = "button";
          revoke.className = "text-btn text-btn-danger";
          revoke.textContent = "Revoke";
          revoke.addEventListener("click", async () => {
            revoke.disabled = true;
            try {
              await nativeAction(`/api/safety/contacts/${encodeURIComponent(contact.id)}/revoke`, {
                method: "POST",
              });
              await loadSafety();
            } catch (e) {
  ...
  async function refreshHistory() {
    ...
    try {
      ...
      if (
        ...
        chatContent.querySelectorAll(".message").length === 0
      ) {
        renderMessages(data.messages);
        if (!sessionStorage.getItem("kurukoo_returning_shown")) {
          sessionStorage.setItem("kurukoo_returning_shown", "1");
          window.KurukooTurnState?.showReturningUser?.();
        }
      }
    } catch {
      setConnection(false, "Offline");
  ...
  async function reconcileHistoricalStorefrontCards(entries) {
    await Promise.all(
      entries.map(async ({ messageEl, cardData }) => {
        if (!cardData?.requestId || !messageEl?.querySelector("[data-storefront-field]")) return;
        try {
          const response = await fetch(
            `/api/chat/economic-requests/${encodeURIComponent(cardData.requestId)}`,
            { credentials: "same-origin" },
          );
          if (!response.ok) return;
          const data = await response.json().catch(() => ({}));
          ...
          if (status && status !== "requested") {
            markStorefrontStepSuperseded(
              messageEl,
              `Request is ${status.replace(/_/g, " ")} — review the latest state below.`,
            );
          }
        } catch {}
      }),
  ...
  function renderMessages(messages) {
    ...
    messages.forEach((m) => {
      ...
      if (m.card_data)
        try {
          cardData = JSON.parse(m.card_data);
        } catch {}
  ...
  document.addEventListener("click", (event) => {
    const action = event.target.closest("[data-surface-view]");
    if (action && action.dataset.surfaceWired !== "1") {
      event.preventDefault();
      renderWorkspaceSurface(action.dataset.surfaceView);
    }
  });
  ...
  function setInspectorOpen(open, target = null) {
    ...
    $("notification-toggle")?.setAttribute(
      "aria-expanded",
      String(open && target === "notifications-card"),
    );
    if (target) {
      inspector.querySelectorAll(".inspector-card").forEach((card) => (card.hidden = true));
      const targetCard = $(target);
      if (targetCard) targetCard.hidden = false;
    } else if (open) {
  ...
  $("memory-toggle")?.addEventListener("click", () =>
    setInspectorOpen(!$("chat-inspector")?.classList.contains("open")),
  );
  ...
  $("inspector-collapse")?.addEventListener("click", () =>
    setInspectorCollapsed(!$("chat-inspector")?.classList.contains("is-collapsed")),
  );
  setInspectorCollapsed(localStorage.getItem("kurukoo_chat_inspector_collapsed") === "1");

  function closeOverflowMenu() {
    const menu = $("chat-overflow-menu");
    const toggle = $("chat-overflow-toggle");
  ...
  async function deleteCurrentConversation() {
    if (state.isGuest) {
      alert("Please sign in to delete conversations.");
      return;
    }
    if (!state.conversationId) {
      setInspectorFeedback("There is no saved conversation to delete.", "info");
      return;
    }
    ...
    try {
      ...
      state.messages = [];
      state.pinnedMessages = [];
      state.activeStorefrontId = null;
      chatContent.replaceChildren();
      renderWelcome();
      loadPinnedMessages();
      refreshHistory();
      setInspectorOpen(false);
      setInspectorFeedback("Conversation deleted.", "info");
    } catch (error) {
      setInspectorFeedback(error.message || "Unable to delete conversation.", "error");
    }
  }
  function pinLatestConversationMessage() {
    ...
    if (!latest) {
      ...
      setInspectorFeedback("There is no message to pin yet.", "info");
      return;
    }
  ...
  function renderAttachmentPreview(file) {
    ...
    details.className = "attachment-name";
    ...
    remove.appendChild(makeIcon("close", "Remove attachment"));
  ...
  $("file-input")?.addEventListener("change", (event) => {
    ...
    state.attached = file || null;
    renderAttachmentPreview(state.attached);
  });
  input?.addEventListener("input", () => {
    input.style.height = "auto";
    input.style.height = `${Math.min(input.scrollHeight, 180)}px`;
    if (!state.busy && send) send.disabled = !input.value.trim();
    saveComposerDraft();
  });
  ...
  $("new-chat")?.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch("/api/chat/conversation", {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "same-origin",
        body: JSON.stringify({ channel: "web", title: "New conversation" }),
      });
      const data = await res.json();
  ...
  function hydrateChatDeepLink() {
    ...
    if (conversationId) {
      state.conversationId = conversationId.slice(0, 160);
      localStorage.setItem("kurukoo_conversation_id", state.conversationId);
    }
    if (discoveryEntityId)
      state.discoveryContextAction = {
        type: "open_discovery_entity",
        entityId: discoveryEntityId.slice(0, 180),
      };
    if (canonicalAction && objectType && objectId) {
      state.canonicalContextAction = {
        type: "resume_canonical_context",
        contextId: contextId?.slice(0, 180),
        conversationId: conversationId?.slice(0, 180),
        canonicalAction: canonicalAction.slice(0, 120),
        objectType: objectType.slice(0, 80),
        objectId: objectId.slice(0, 180),
      };
      state.resumeCanonicalContextOnLoad = [
        "economic_request.open",
        "agent.goal.review",
        "agent.goal.resume",
        "task.open",
        "reminder.open",
    ...
    if (contactCompose && input) {
      try {
        ...
        if (
          ...
          candidate.contactId.length <= 180 &&
          candidate.body.trim()
        ) {
          contactMessageDraft = candidate;
          state.canonicalContextAction = {
            type: "resume_canonical_context",
            contextId: `contact:${candidate.contactId}`.slice(0, 180),
            canonicalAction: "communication.compose",
            objectType: "contact",
            objectId: candidate.contactId.slice(0, 180),
          };
        ...
        /* A private local draft is optional; Chat remains usable without it. */
      }
    }
    const contextParts = [];
    ...
    if (contextBanner && contextParts.length) {
      contextBanner.textContent = `${contextParts.join(" · ")}. Kurukoo will keep this context with the conversation.`;
      contextBanner.hidden = false;
    }
    if (prompt && input && !contactMessageDraft) {
      input.value = prompt.slice(0, 12000);
      input.dispatchEvent(new Event("input", { bubbles: true }));
    }
  }
  ...
  async function loadAgentBrief() {
    try {
      ...
      const visible = Array.isArray(brief.items)
        ? brief.items.filter((item) => item?.attention !== "SILENT")
        : [];
      ...
      const message = createMessage(
        "assistant",
        String(brief.presentation?.text || "Kurukoo has an update for you."),
        null,
        null,
        false,
      );
      ...
      state.messages.push({
        role: "assistant",
        text: String(brief.presentation?.text || ""),
        id: null,
      });
      if (brief.presentation?.shouldSpeak)
        speakAssistantResponse(String(brief.presentation.text || ""), null);
      scroll.scrollTop = scroll.scrollHeight;
    } catch {}
  }
  ...
  function renderWelcome() {
    ...
    const qa = makeElement("div", "quick-actions welcome-quick-actions");
    qa.id = "quick-actions";
    [
      ["Help me get somewhere", "Get somewhere"],
      ["Remind me about something", "Set a reminder"],
      ["Check a device or connection", "Check a device"],
      ["Find someone to help with this", "Find help"],
    ].forEach(([p, l]) => {
      const b = makeElement("button", "", l);
      b.dataset.prompt = p;
      qa.appendChild(b);
    });
    chatContent.appendChild(qa);
    wireQuickActions(qa);
    if (state.isGuest) {
      void startGuestAuth().then((step) => renderWelcomeAuth(step));
  ...
  function applyWorkspaceIdentityState() {
    ...
    const presence = $("sidebar-presence");
    if (presence) presence.textContent = state.isGuest ? "Guest" : "Available";
    const memory = $("sidebar-memory-status");
    if (memory) memory.textContent = state.isGuest ? "Not connected" : "In use";
    const channel = $("sidebar-channel-status");
    if (channel) channel.textContent = state.isGuest ? "Web only" : "Web";
    const context = $("memory-context");
    if (context)
      context.textContent = state.isGuest
        ? "Start with a conversation. Your private context appears after you establish your account."
        : "Your conversations, reminders and saved context stay connected to your profile.";
  }
  ...
  async function registerCurrentDeviceTrust() {
    ...
    try {
      ...
      if (!deviceId) {
        ...
        localStorage.setItem("kurukoo_device_id", deviceId);
      }
