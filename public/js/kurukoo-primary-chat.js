(() => {
  ...
  function sanitizeHtml(html) {
    ...
    doc.querySelectorAll('script,iframe,object,embed,style,link,form').forEach(node => node.remove());
    doc.querySelectorAll('*').forEach(node => [...node.attributes].forEach(attr => {
      if (/^on/i.test(attr.name) || /^(javascript|data):/i.test(attr.value)) node.removeAttribute(attr.name);
    }));
    return doc.body.innerHTML;
  }
  function renderMarkdown(text) {
    if (!window.marked) return String(text || '').replace(/[&<>]/g, ch => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;' }[ch])).replace(/\n/g, '<br>');
    marked.setOptions({ breaks: true, gfm: true });
    return sanitizeHtml(marked.parse(text || ''));
  }
  function enhanceCode(root) { root.querySelectorAll('pre code').forEach(block => { if (window.hljs) hljs.highlightElement(block); }); }
  ...
  function createMessage(role, text = '') {
    ...
    wrap.querySelector('.message-actions').addEventListener('click', event => {
